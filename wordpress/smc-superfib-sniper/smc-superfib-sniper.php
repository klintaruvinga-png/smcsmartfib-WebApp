...
final class SMC_SuperFib_Sniper_REST {
        ...
        if (isset($payload['tick'])) {
            ...
            $wpdb->replace(
                ...
                array(
                    ...
                    'source' => 'mt5',
                    // HARDENING: state must be 'live' for MT5-sourced data so downstream
                    // freshness checks in get_cached_price() do not silently return 'offline'.
                    'state' => 'live',
                    'updated_at' => $this->now_mysql(),
                ),
        ...
        if (isset($payload['candle_m1'])) {
            ...
            $wpdb->replace(
                ...
                array(
                    ...
                    'symbol' => $normalized_symbol,
                    'timeframe' => $timeframe,
                    'candle_time' => isset($candle['timestamp']) ? $this->parse_mt5_timestamp($candle['timestamp']) : $this->now_mysql(),
    ...
    public function get_market_data_authority(WP_REST_Request $request) {
        ...
        // BUGFIX: snapshot returns 'prices', not 'symbols' — use the correct key.
        $snapshot = $this->ensure_engine_snapshot($user_id);
        $watched  = array_column($snapshot['prices'] ?? array(), 'symbol');
        $result   = array();
