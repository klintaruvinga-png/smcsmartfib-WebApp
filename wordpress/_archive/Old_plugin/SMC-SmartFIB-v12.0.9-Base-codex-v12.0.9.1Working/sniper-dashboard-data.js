...
(function (root, factory) {
  ...
  var ICT_PD_TOLERANCE_PCT = 0.10;
  var ICT_BIAS_GATE = 0.10;
  ...
  function normalizeSessionTf(sessionTf) {
    var value = String(sessionTf || '').trim().toUpperCase();
    if (value === 'DAILY' || value === 'DAY' || value === 'D' || value === '1D') return 'Daily';
    if (value === 'WEEKLY' || value === 'WEEK' || value === 'W' || value === '1W') return 'Weekly';
    if (value === 'MONTHLY' || value === 'MONTH' || value === 'M' || value === '1M') return 'Monthly';
    if (value === 'YEARLY' || value === 'YEAR' || value === 'Y' || value === '1Y' || value === 'ANNUAL') return 'Yearly';
  ...
  function isAnchorTraceEnabled(options) {
    ...
    if (root && (root.SNIPER_DASHBOARD_DEBUG_ANCHOR_TRACE === true || root.__ANCHOR_TRACE__ === true)) return true;
    return false;
  }
  ...
  function buildCompletedSessions(candles, sessionTf) {
    var norm = candles.map(normalizeCandle).filter(Boolean).sort(function (a, b) { return a.timeMs - b.timeMs; });
  ...
  function computeCgBestBodyPct(candles, options) {
    ...
    var minBodyPct = options.cgMinBodyPct == null ? 0.40 : Number(options.cgMinBodyPct);
  ...
  function computeFibAnchors(candles, opts) {
    ...
    var cgMinRange = (pipType === 'JPY' ? cgMinRangeJPY : cgMinRangeUSD) * pipMult(pipType) * 10;
    var lockedRange = (lockedHigh != null && lockedLow != null) ? Math.abs(lockedHigh - lockedLow) : null;
    var cgValidRange = lockedRange != null && lockedRange >= cgMinRange;

    var cg = computeCgBestBodyPct(candles || [], options);
    var allowFib = !!(cgValidRange && cg.cgValidDisp && lockedHigh != null && lockedLow != null);
  ...
  function isValidFib(fib) {
    return !!(fib && Number.isFinite(fib.high) && Number.isFinite(fib.low) && fib.high > fib.low);
  }
  ...
  function computeSweepMssSequence(candles, opts) {
    ...
    var sweepConfirmBars = Number(options.sweepConfirmBars == null ? 2 : options.sweepConfirmBars);
    ...
    for (var i = 0; i < norm.length; i++) {
      ...
      var eqThresh = bosThresh(pipType, bosMinPips) * 0.50;
      var eqhActive = lastPH1 != null && lastPH2 != null && Math.abs(lastPH1 - lastPH2) <= eqThresh;
      var eqlActive = lastPL1 != null && lastPL2 != null && Math.abs(lastPL1 - lastPL2) <= eqThresh;
      ...
      if (swpCandDir === 1 && i > swpCandBar && (i - swpCandBar) <= sweepConfirmBars && c.close < swpCandPrice) {
        ...
        swpCandBar = null;
        swpAcceptCount = 0;
      } else if (swpCandDir === -1 && i > swpCandBar && (i - swpCandBar) <= sweepConfirmBars && c.close > swpCandPrice) {
      ...
      var dispScoreBull = (dispBodyRatio >= 0.55 ? 1 : 0) + (dispRangeATR >= 1.3 ? 1 : 0) + (dispFvgBull ? 1 : 0) + (dispExtendBull ? 1 : 0);
      var dispScoreBear = (dispBodyRatio >= 0.55 ? 1 : 0) + (dispRangeATR >= 1.3 ? 1 : 0) + (dispFvgBear ? 1 : 0) + (dispExtendBear ? 1 : 0);
      ...
      var bullLegRange = (bullLegHigh != null && bullLegLow != null) ? Math.abs(bullLegHigh - bullLegLow) : 0;
      var bearLegRange = (bearLegHigh != null && bearLegLow != null) ? Math.abs(bearLegHigh - bearLegLow) : 0;
      ...
      var bullNarrativeValid = !!(hasBullSweepContext &&
        mss_bullish === true &&
        bullLegRange >= efLegMin &&
        bullLegLow != null &&
        (c.close - bullLegLow) >= atr14 * efDispMult);
      var bearNarrativeValid = !!(hasBearSweepContext &&
        mss_bearish === true &&
      ...
      var efOteLo = null;
      var efOteHi = null;
      if (Number.isFinite(efAnchorHigh) && Number.isFinite(efAnchorLow) && efAnchorHigh > efAnchorLow) {
      ...
      if (efOteLo != null && efOteHi != null && c.close >= efOteLo && c.close <= efOteHi) barsInZone += 1;
      else barsInZone = 0;

      var sequenceStatus;
      if (sweep_bar === null) sequenceStatus = 'AWAIT SWEEP';
      else if (mss_bar === null || mss_bar <= sweep_bar) {
        if (mss_sweep_bar === null && (i - sweep_bar) > mssConfirmBars) sequenceStatus = 'STALE';
      ...
      var pdaRankBonusMap = { EQH: 10, EQL: 10, PDH: 8, PDL: 8, PWH: 6, PWL: 6, BSL: 3, SSL: 3 };
      var pdaRankBonus = pdaRankBonusMap[liquidityType] || 0;
      var liqBonus = liquidityType !== 'NONE' ? (10 + pdaRankBonus) : 0;
      var narrativeOk = liquidityType !== 'NONE' && (mss_bullish || mss_bearish) && sequenceStatus === 'READY';
      ...
      var htfTrendBonus = ((pz === -1 && htfTrend === 1) || (pz === 1 && htfTrend === -1)) ? 1 : 0;
      var dwellPenalty = barsInZone > 6 ? -15 : barsInZone > 3 ? 0 : barsInZone > 0 ? 10 : 0;
      var reactionBonus = reactionCandleQuality ? 10 : 0;
      var setupQualityBase = Number(raw.setup_quality == null ? (raw.setupQ == null ? 0 : raw.setupQ) : raw.setup_quality);
      var execQualityBase = Number(raw.execution_quality == null ? (raw.execQ == null ? 0 : raw.execQ) : raw.execution_quality);
      var setupQuality = setupQualityBase + liqBonus + narrativeBonus + htfFvgBonus + htfTrendBonus;
      var executionQuality = execQualityBase + dwellPenalty + reactionBonus;
      var rrOk = raw.rr_ok != null ? !!raw.rr_ok : true;
      var poiOk = efAnchorDir === 1 ? bullPoiFresh : (efAnchorDir === -1 ? bearPoiFresh : (bullPoiFresh || bearPoiFresh));
      ...
      var ef_tp_narrative_valid = !!(ef_is_narrative || (state === 'ACTIVE' && ladderDir === efAnchorDir));
  ...
  function applyStarsForSide(sfLevels, startIdx, endIdx, step, sfAnchorHigh, sfAnchorLow, validCount) {
    var runningMax = 0;
  ...
  function computeSfEngine(f1, f2, f3) {
    ...
    var sfAnchorHigh = fibs.reduce(function (acc, fib) { return acc + fib.high; }, 0) / validCount;
    var sfAnchorLow = fibs.reduce(function (acc, fib) { return acc + fib.low; }, 0) / validCount;
    ...
    applyStarsForSide(sfLevels, idx50 + 1, sfLevels.length, 1, sfAnchorHigh, sfAnchorLow, validCount);
  ...
  function buildProfile(rawTimeframe) {
    switch (normalizeFibTimeframeKey(rawTimeframe)) {
      case 'DAILY':
        return { key: 'DAILY', label: 'Day Trader', fib_timeframe: 'DAILY', candleInterval: '1h', interval: '1h', historyDepth: 160, outputSize: 160, outputsize: 160, proximityThreshold: 12, strategyHorizon: '1 Day' };
      case 'MONTHLY':
        return { key: 'MONTHLY', label: 'Positional', fib_timeframe: 'MONTHLY', candleInterval: '1day', interval: '1day', historyDepth: 180, outputSize: 180, outputsize: 180, proximityThreshold: 35, strategyHorizon: '1 Month' };
      case 'YEARLY':
        return { key: 'YEARLY', label: 'Institutional', fib_timeframe: 'YEARLY', candleInterval: '1week', interval: '1week', historyDepth: 220, outputSize: 220, outputsize: 220, proximityThreshold: 50, strategyHorizon: '1 Year' };
      case 'WEEKLY':
      default:
        return { key: 'WEEKLY', label: 'Swing Trader', fib_timeframe: 'WEEKLY', candleInterval: '4h', interval: '4h', historyDepth: 140, outputSize: 140, outputsize: 140, proximityThreshold: 20, strategyHorizon: '1 Week' };
    }
  }
  ...
  function computeStructureRegime(norm, opts) {
    ...
    var bos = bosThresh(pipType, Number(options.bosMinPips == null ? bosDefault : options.bosMinPips));
    var lb = Number(options.swingLookback == null ? 10 : options.swingLookback);
    if (!norm.length) {
      return { trend: 0, prevTrend: 0, regime: 'RANGING', pivots: { highs: [], lows: [] }, labels: [] };
    }
    var highs = [];
    var lows = [];
    ...
    for (var idx = 0; idx < limit; idx++) {
      var isHh = idx > 0 && idx < highs.length && highs[idx].price > (highs[idx - 1].price + bos);
      var isLh = idx > 0 && idx < highs.length && highs[idx].price < (highs[idx - 1].price - bos);
    ...
    var regime = regimeState === 1 ? 'TREND UP' : (regimeState === -1 ? 'TREND DOWN' : (regimeState === 2 ? 'REVERSAL ZONE' : 'RANGING'));
    return {
      trend: trend,
      prevTrend: prevTrend,
      regime: regime,
      pivots: { highs: highs.slice(-6), lows: lows.slice(-6) },
      labels: [latest.hh ? 'HH' : (latest.lh ? 'LH' : null), latest.hl ? 'HL' : (latest.ll ? 'LL' : null)].filter(Boolean)
    };
  }
  ...
  function collectPivotLiquidity(norm, lookback, maxCount) {
    ...
    for (var i = lb; i < norm.length - lb; i++) {
      var ph = pivotHigh(norm, i, lb);
      if (ph != null) highs.push({ price: ph, timeMs: norm[i].timeMs, source: 'LOCAL_PIVOT_HIGH' });
      var pl = pivotLow(norm, i, lb);
      if (pl != null) lows.push({ price: pl, timeMs: norm[i].timeMs, source: 'LOCAL_PIVOT_LOW' });
    }
  ...
  function collectEqualLiquidity(norm, pipType, majorOnly) {
    ...
    for (var i = 1; i < candles.length; i++) {
      ...
      if (Math.abs(cur.high - prev.high) <= thresh) {
        eqh.push({ price: (cur.high + prev.high) * 0.5, timeMs: cur.timeMs, source: majorOnly ? 'MAJOR_EQH' : 'EQH' });
      }
      if (Math.abs(cur.low - prev.low) <= thresh) {
        eql.push({ price: (cur.low + prev.low) * 0.5, timeMs: cur.timeMs, source: majorOnly ? 'MAJOR_EQL' : 'EQL' });
      }
    }
    return {
  ...
  function buildHtfLiquidityPools(profile, norm, candles, pipType) {
    ...
    var pivots = collectPivotLiquidity(norm, profile.timeframe_profile === 'POSITION' ? 8 : 5, profile.timeframe_profile === 'POSITION' ? 4 : 6);
    highs = highs.concat(pivots.highs.map(function (h) {
      return { price: h.price, source: profile.timeframe_profile === 'POSITION' ? 'MAJOR_PIVOT_HIGH' : 'LOCAL_PIVOT_HIGH', timeMs: h.timeMs };
    }));
    lows = lows.concat(pivots.lows.map(function (l) {
      return { price: l.price, source: profile.timeframe_profile === 'POSITION' ? 'MAJOR_PIVOT_LOW' : 'LOCAL_PIVOT_LOW', timeMs: l.timeMs };
    }));

    return { highs: highs.filter(function (v) { return v && Number.isFinite(v.price); }), lows: lows.filter(function (v) { return v && Number.isFinite(v.price); }) };
  }
  ...
  function resolveHtfDealingRange(candles, matrixTf) {
    ...
    var hi = window.reduce(function (acc, s) { return Math.max(acc, Number(s.high)); }, -Infinity);
    var lo = window.reduce(function (acc, s) { return Math.min(acc, Number(s.low)); }, Infinity);
  ...
  function fibPressureComponent(price, fibHigh, fibLow) {
    if (!Number.isFinite(price) || !Number.isFinite(fibHigh) || !Number.isFinite(fibLow) || fibHigh === fibLow) {
  ...
  function buildPdArrayAssessment(seqBar, context) {
    ...
    var freshBullPoi = seqBar && typeof seqBar.bullPoiFresh === 'boolean'
      ? seqBar.bullPoiFresh
      : (lastBullAge != null && lastBullAge <= 12);
    var freshBearPoi = seqBar && typeof seqBar.bearPoiFresh === 'boolean'
      ? seqBar.bearPoiFresh
    ...
    var efNearEntryBuy = !!(efDir === 1 && toNumber(seqBar && seqBar.efOteLo) != null && toNumber(seqBar && seqBar.efOteHi) != null && toNumber(ctx.lastClose) >= toNumber(seqBar.efOteLo) && toNumber(ctx.lastClose) <= toNumber(seqBar.efOteHi));
    var efNearEntrySell = !!(efDir === -1 && toNumber(seqBar && seqBar.efOteLo) != null && toNumber(seqBar && seqBar.efOteHi) != null && toNumber(ctx.lastClose) >= toNumber(seqBar.efOteLo) && toNumber(ctx.lastClose) <= toNumber(seqBar.efOteHi));

    var bullFvgLow = Math.min(htfHigh3 == null ? Infinity : htfHigh3, htfLow1 == null ? Infinity : htfLow1);
    var bullFvgHigh = Math.max(htfHigh3 == null ? -Infinity : htfHigh3, htfLow1 == null ? -Infinity : htfLow1);
    var bearFvgLow = Math.min(htfHigh1 == null ? Infinity : htfHigh1, htfLow3 == null ? Infinity : htfLow3);
    var bearFvgHigh = Math.max(htfHigh1 == null ? -Infinity : htfHigh1, htfLow3 == null ? -Infinity : htfLow3);

    var bullPdSupport = (
      (htfBullFvg && lastLow != null && lastLow <= (bullFvgHigh + pdTol) && lastLow >= (bullFvgLow - pdTol)) ||
      (freshBullPoi && bullPoiHigh != null && bullPoiLow != null && lastLow != null && lastLow <= (bullPoiHigh + pdTol) && lastLow >= (bullPoiLow - pdTol)) ||
      efNearEntryBuy
    );
    var bearPdSupport = (
      (htfBearFvg && lastHigh != null && lastHigh <= (bearFvgHigh + pdTol) && lastHigh >= (bearFvgLow - pdTol)) ||
      (freshBearPoi && bearPoiHigh != null && bearPoiLow != null && lastHigh != null && lastHigh <= (bearPoiHigh + pdTol) && lastHigh >= (bearPoiLow - pdTol)) ||
      efNearEntrySell
    );
    var bullishSupport = bullPdSupport || (efDir === 1);
    var bearishSupport = bearPdSupport || (efDir === -1);
    var pdArrayDir = bullPdSupport && !bearPdSupport ? 1 : (bearPdSupport && !bullPdSupport ? -1 : 0);
    var mixed = !(bullishSupport && !bearishSupport) && !(bearishSupport && !bullishSupport);
    var label = mixed ? 'MIXED PDA' : (pdArrayDir === 1 ? 'BULLISH PDA' : (pdArrayDir === -1 ? 'BEARISH PDA' : 'MIXED PDA'));
    return {
  ...
  function computeInstrumentSnapshot(pair, candles, runtimeContext) {
    ...
    var structure = computeStructureRegime(norm, { pipType: pipType, swingLookback: 10, bosMinPips: ctx.bosMinPips });
    ...
    if (last && liqPools.highs.length) {
      ...
      if (!upPool) {
        upPool = liqPools.highs.reduce(function (best, pool) {
          if (!best) return pool;
          return Math.abs(pool.price - last.close) < Math.abs(best.price - last.close) ? pool : best;
        }, null);
      }
    }
    ...
    var htfDolDir = htfDolScoreUp > htfDolScoreDown ? 1 : (htfDolScoreDown > htfDolScoreUp ? -1 : 0);
    ...
    if (matrixRange && last) {
      ...
      var rangeSpan = Math.max(Math.abs(matrixRange.high - matrixRange.low), pipMult(pipType) * 10);
    ...
    var pdTol = matrixRange ? (Math.abs(matrixRange.high - matrixRange.low) * ICT_PD_TOLERANCE_PCT) : ((seqBar.dispRangeATR || 0) * 0.25);
    ...
    var sfComp = (sf && sf.sfValid && last) ? fibPressureComponent(last.close, sf.sfAnchorHigh, sf.sfAnchorLow) : { bull: 0.5, bear: 0.5 };
    var f1Comp = isValidFib(anchorsRaw.f1) && last ? fibPressureComponent(last.close, anchorsRaw.f1.high, anchorsRaw.f1.low) : { bull: 0, bear: 0 };
    var f2Comp = isValidFib(anchorsRaw.f2) && last ? fibPressureComponent(last.close, anchorsRaw.f2.high, anchorsRaw.f2.low) : { bull: 0, bear: 0 };
    var f3Comp = isValidFib(anchorsRaw.f3) && last ? fibPressureComponent(last.close, anchorsRaw.f3.high, anchorsRaw.f3.low) : { bull: 0, bear: 0 };
    ...
    var pressureBias = pressureBiasValue > ICT_BIAS_GATE ? 'BULLISH' : (pressureBiasValue < -ICT_BIAS_GATE ? 'BEARISH' : 'NEUTRAL');
    matrixPressureLabel = pressureBiasValue > ICT_BIAS_GATE ? 'DISC PRESS' : (pressureBiasValue < -ICT_BIAS_GATE ? 'PREM PRESS' : 'BALANCED');
    ...
    var execBullOk = !!(seqBar.confirmed_sweep_down && seqBar.mss_bullish && seqBar.dispScoreBull >= 2);
    var execBearOk = !!(seqBar.confirmed_sweep_up && seqBar.mss_bearish && seqBar.dispScoreBear >= 2);
    ...
    var rawFinalBiasState = finalBiasDelta >= 35 ? (inHtfPremium ? 2 : 1) : (finalBiasDelta <= -35 ? (inHtfDiscount ? -2 : -1) : (Math.abs(finalBiasDelta) < 15 ? 0 : 3));
    ...
    if (ICT_BIAS_CONFIRM_EXEC) {
      ...
      else if (rawFinalBiasState === -1 && !hardFlipToBear) finalBiasState = inHtfDiscount ? -2 : 3;
    }
    var finalBias = mapFinalBiasStateToLabel(finalBiasState);
    var hardFlipAllowed = hardFlipToBull || hardFlipToBear;
    var pressureBias = bullPressure > bearPressure ? 'BULLISH' : (bearPressure > bullPressure ? 'BEARISH' : 'NEUTRAL');
    ...
    if (structure.regime === 'TREND UP' && (finalBias === 'BULL_EXP' || finalBias === 'BULL_PB')) {
      gate = 'BUY';
      gateReason = 'BIAS_ALIGNED';
    } else if (structure.regime === 'TREND DOWN' && (finalBias === 'BEAR_EXP' || finalBias === 'BEAR_RALLY')) {
      gate = 'SELL';
      gateReason = 'BIAS_ALIGNED';
    ...
    return {
      ...
      prev_regime: structure.prevTrend === 1 ? 'TREND UP' : (structure.prevTrend === -1 ? 'TREND DOWN' : 'RANGING'),
      structure: {
      ...
      matrix: { matrix_state: matrixState, matrix_pressure_label: matrixPressureLabel, dealing_range: matrixRange, equilibrium_buffer: matrixEquilibriumBuffer },
      matrix_tf: profile.matrix_tf,
      pd_array: Object.assign({ pd_array_dir: pdArrayDir, pd_array_label: pdArrayLabel }, pdArrayAssessment),
      pd_tf: profile.pd_tf,
      ...
      sweep_state: seqBar.confirmed_sweep_up ? 'SWEEP_UP_CONFIRMED' : (seqBar.confirmed_sweep_down ? 'SWEEP_DOWN_CONFIRMED' : 'NO_SWEEP'),
      mss_state: seqBar.mss_bullish ? 'BULLISH_MSS' : (seqBar.mss_bearish ? 'BEARISH_MSS' : 'NO_MSS'),
    ...
    return root.fetch(base + path, {
      ...
      if (!response.ok) {
        if (root && root.console && typeof root.console.warn === 'function') {
          root.console.warn('[SniperDashboardData] GET ' + path + ' failed with HTTP ' + response.status);
        }
        return null;
      }
      ...
      if (root && root.console && typeof root.console.warn === 'function') {
        root.console.warn('[SniperDashboardData] GET ' + path + ' failed:', err && err.message ? err.message : err);
      }
      return null;
    });
  }
  ...
  function canonicalState(rawState, validityBarsRemaining) {
    ...
    var state = String(rawState || '').trim().toUpperCase().replace(/[_-]+/g, ' ');
    if (['EXPIRED', 'INVALID', 'WATCHLIST', 'READY', 'ACTIVE'].indexOf(state) > -1) return state;
    if (state === 'PENDING' || state === 'AWAIT MSS' || state === 'AWAIT SWEEP') return 'WATCHLIST';
  ...
  function canonicalPair(rawPair) {
    var value = String(rawPair || '').trim().toUpperCase();
    if (!value) return '';
  ...
  function canonicalSignal(raw) {
    ...
    var profile = buildProfile(input.fib_timeframe || input.session_tf || getBoot().fib_timeframe);
    ...
    if (!Number.isFinite(validityBarsRemaining) || validityBarsRemaining < 0) validityBarsRemaining = 1;
    var totalLot = Number(input.total_lot_size != null ? input.total_lot_size : input.total_lots);
    if (!Number.isFinite(totalLot)) {
      totalLot = Array.isArray(input.entries)
        ? input.entries.reduce(function (sum, entry) { return sum + (Number(entry && entry.lot) || 0); }, 0)
        : null;
    }
    return {
      signal_id: String(input.signal_id || input.signal_hash || [pair, input.direction || 'NA', input.updated_at || 'na'].join('|')),
      pair: pair,
      direction: String(input.direction || '').toUpperCase(),
      state: canonicalState(input.state || input.signal_state || input.status, validityBarsRemaining),
      entry: {
        zone_price: toNumber(input.zone_price != null ? input.zone_price : input.entry_zone_price),
        label: input.zone_label || input.entry_zone_label || null,
        ladder: Array.isArray(input.entries)
          ? input.entries.map(function (entry) {
      ...
      lot_size: {
        stages: Array.isArray(input.entries)
          ? input.entries.map(function (entry) { return Number(entry && entry.lot); }).filter(function (lot) { return Number.isFinite(lot); })
          : [],
  ...
  function seedSignalsFromRuntime() {
    ...
    var liveSignals = runtime && Array.isArray(runtime.liveSignals)
      ? runtime.liveSignals
      : (root && Array.isArray(root.liveSignals) ? root.liveSignals : null);
    if (liveSignals && liveSignals.length) {
      runtimeCache.signals = liveSignals.map(canonicalSignal);
      runtimeCache.hydratedSignals = true;
      return;
    }
    var signals = runtime && Array.isArray(runtime.signals)
      ? runtime.signals
      : (root && Array.isArray(root.signals) ? root.signals : null);
  ...
  function seedRegimesFromRuntime() {
    var runtime = getCoreRuntimeState();
    var regimes = runtime && runtime.savedRegimes && typeof runtime.savedRegimes === 'object'
      ? runtime.savedRegimes
      : (root && root.savedRegimes && typeof root.savedRegimes === 'object' ? root.savedRegimes : null);
    if (regimes) {
  ...
  function candidateSymbols() {
    var pairs = runtimeCache.signals.map(function (signal) { return signal.pair; }).filter(Boolean);
    if (!pairs.length && root && Array.isArray(root.PAIRS)) {
      pairs = root.PAIRS.slice();
    }
    if (!pairs.length) {
      pairs = ['GBP/USD', 'AUD/USD', 'USD/JPY', 'AUD/JPY', 'EUR/USD', 'XAU/USD', 'US30', 'NAS100'];
    }
    var mapped = [];
    pairs.forEach(function (pair) {
      ...
      if (canonical === 'BTC/USD') mapped.push('BINANCE:BTCUSDT', 'BITSTAMP:BTCUSD', 'BTCUSD');
      if (canonical === 'ETH/USD') mapped.push('BINANCE:ETHUSDT', 'BITSTAMP:ETHUSD', 'ETHUSD');
  ...
  var bridge = {
    getProfile: function () {
      var boot = getBoot();
      var liveTimeframe = root && root.FIB_TIMEFRAME
        ? root.FIB_TIMEFRAME
        : (boot && boot.fib_timeframe ? boot.fib_timeframe : '');
    ...
    getRegime: function (pair) {
      ...
      return runtimeCache.regimes[normalized] || runtimeCache.regimes[canonicalPair(pair)] || null;
    },
    isChopCondition: function (pairOrSignal, price) {
      var signal = pairOrSignal && typeof pairOrSignal === 'object' ? pairOrSignal : null;
      if (signal && signal.enrichment_meta && signal.enrichment_meta.chop && signal.enrichment_meta.chop.active) {
        return true;
      }
      if (root && typeof root.getChopBand === 'function') {
        var pair = signal ? signal.pair : pairOrSignal;
        var band = root.getChopBand(pair);
        var marketPrice = Number(signal && signal.market_price != null ? signal.market_price : price);
        return !!(band && Number.isFinite(marketPrice) && marketPrice >= band.lo && marketPrice <= band.hi);
      }
      return false;
    }
  };
