...
test('Pine source retains the audited F1/F2/F3 timeframe and draw wiring', () => {
  ...
  assert.match(source, /session_tf\s*=\s*session_tf_override_on\s*\?\s*session_tf_override\s*:\s*_auto_session_tf/);
  ...
  assert.match(source, /data_tf\s*=\s*session_tf\s*==\s*"Yearly"\s*\?\s*"D"\s*:\s*session_tf\s*==\s*"Monthly"\s*\?\s*"D"\s*:\s*session_tf\s*==\s*"Weekly"\s*\?\s*"60"\s*:\s*"15"/);
  assert.match(source, /\\"session_tf\\":\\""\s*\+\s*session_tf\s*\+\s*"\\",\\"fib_timeframe\\":\\""\s*\+\s*session_tf/);
  ...
  assert.match(source, /if\s+_same_dir\s+and\s+_full_rng\s*>\s*0\s+and\s+\(_swap_rng\s*\/\s*_full_rng\)\s*<\s*_f2_expand_thresh/);
  ...
  assert.match(source, /lbl_text\s*=\s*fib_name\s*\+\s*" "\s*\+\s*level_pct\s*\+\s*" @ "\s*\+\s*str\.tostring\(price,\s*format\.mintick\)/);
  assert.match(source, /draw_fib_bi\(_xb_f1_smart,\s*f1_high,\s*f1_low,\s*false,\s*f1_bull,\s*3,\s*f1_name,\s*_xbl_f1_smart\)/);
  assert.match(source, /draw_fib_bi\(_xb_f2_smart,\s*f2_high,\s*f2_low,\s*true,\s*f2_is_bull,\s*2,\s*f2_name,\s*_xbl_f2_smart\)/);
  assert.match(source, /draw_fib_bi\(_xb_f3_smart,\s*f3_high,\s*f3_low,\s*false,\s*f3_bull,\s*1,\s*f3_name,\s*_xbl_f3_smart\)/);
});

test('Pine source tracks redraw objects for fib arrows, EF labels, and SF overlays', () => {
  const source = loadPineSource();

  assert.match(source, /draw_vert_arrow_bi[\s\S]*array\.push\(_fib_lines,\s*_shaft\)[\s\S]*array\.push\(_fib_lines,\s*_tip_l\)[\s\S]*array\.push\(_fib_lines,\s*_tip_r\)[\s\S]*array\.push\(_fib_lines,\s*_base\)/);
  assert.match(source, /show_ef_ote_zone[\s\S]*array\.push\(_fib_boxes,\s*_ote_box\)/);
  assert.match(source, /show_labels and show_ef_labels[\s\S]*array\.push\(_fib_labels,\s*_l0\)[\s\S]*array\.push\(_fib_labels,\s*_l100\)/);
  assert.match(source, /show_ef_targets and show_ext and _ef_tp_draw_ok[\s\S]*array\.push\(_fib_labels,\s*_lm1\)[\s\S]*array\.push\(_fib_labels,\s*_lm2\)/);
...
test('Pine parity harness matches fixtures and the dashboard anchor engine', () => {
  ...
  loadAnchorFixtures().forEach((fixture) => {
    const pine = computePineFibAnchors(fixture.candles, Object.assign({}, fixture.options, { nowMs: fixedNow }));
    ...
    assertFib({ high: pine.f2.high, low: pine.f2.low, bull: pine.f2.bull }, fixture.expected.f2, fixture.name + ' F2');
    ...
    assertFib({ high: pine.f2.high, low: pine.f2.low, bull: pine.f2.bull, name: pine.f2.name }, dash.f2, fixture.name + ' F2 parity');
...
test('Pine fib level math produces plotted anchor endpoints and label prices', () => {
  ...
  const pine = computePineFibAnchors(fixture.candles, Object.assign({}, fixture.options, {
