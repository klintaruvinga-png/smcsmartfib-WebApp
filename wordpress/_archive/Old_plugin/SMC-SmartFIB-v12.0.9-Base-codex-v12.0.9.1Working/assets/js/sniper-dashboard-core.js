...
const API = {
    BASE:
        (window.SNIPER && window.SNIPER.rest_url) ||
        "https://trader.stokvelsociety.co.za/wp-json/sniper/v1/",
            ...
            return r.text().then(function (raw) {
                ...
                if (raw) {
                    try {
                        ...
                        detail =
                            parsed && (parsed.error || parsed.message || parsed.code)
                                ? String(parsed.error || parsed.message || parsed.code)
                                : "";
                    } catch (_err) {
                        detail = raw;
                    }
            ...
            return r.text().then(function (raw) {
                ...
                if (raw) {
                    try {
                        ...
                        detail =
                            parsed &&
                            (parsed.reason || parsed.error || parsed.message || parsed.code)
                                ? String(
                                      parsed.reason ||
                                          parsed.error ||
                                          parsed.message ||
...
function getSectionPanelId(sectionId) {
    ...
    return (
        PRIMARY_SECTION_PANEL_IDS[normalized] ||
        LEGACY_SECTION_PANEL_IDS[normalized] ||
        PRIMARY_SECTION_PANEL_IDS[SECTION_DEFAULT]
    );
}
...
function bindAccountTabs() {
    ...
    document.addEventListener("click", function (event) {
        ...
        document
            .querySelectorAll("#tab-account-overview, #tab-risk-profile")
...
function notifyTdCreditsExhausted(kind) {
    ...
    if (
        !tdCreditsToastState.active ||
        now - tdCreditsToastState.lastAt >= TD_CREDITS_TOAST_COOLDOWN_MS
    ) {
        xtoast(msg, "warn");
    }
...
function plUSC(v) {
    return fmtAccountAmount(v, true);
}
function plZAR(v) {
    return "";
} // deprecated — kept to avoid call-site errors; callers should use fmtAccountAmount
function plBoth(v, cls) {
    if (v == null || isNaN(v)) return '<span style="color:var(--mu)">-</span>';
    var col = v >= 0 ? "var(--gr)" : "var(--re)";
    var ec = cls ? " " + cls : "";
    return (
        '<span class="mn' + ec + '" style="color:' + col + '">' + fmtAccountAmount(v) + "</span>"
    );
}
function plBothText(v) {
    return fmtAccountAmount(v);
}
...
function isPhase4aPair(raw) {
    return !!toPhase4aPair(raw);
}
function isPairJPY(pair) {
    return String(pair || "").indexOf("JPY") > -1;
}
function regimeGateLabel(regime) {
    return regime === "TREND DOWN"
        ? "SELL"
        : regime === "TREND UP"
          ? "BUY"
          : regime === "REVERSAL ZONE"
            ? "BOTH"
            : "NONE";
}
...
var savedSeqStatus = {};
function seqBadge(pair) {
    return seqPill(savedSeqStatus[pair]);
}
...
function syncDashboardStateWatchlist() {
...
function watchlistMutationInProgress() {
    ...
    return (
        WATCHLIST_LAST_MUTATION_AT > 0 &&
        Date.now() - WATCHLIST_LAST_MUTATION_AT < WATCHLIST_GUARD_MS
    );
}
function persistPairList() {
    syncDashboardStateWatchlist();
    if (!USER_SYNC || !USER_SYNC.authenticated) return;
    apiPost("user/watchlist", { watchlist: PAIRS.slice() }).catch(function (e) {
...
function applyWatchlist(nextWatchlist, source) {
    ...
    if (source !== "watchlist_mutation_confirmed" && watchlistMutationInProgress()) {
        if (DEBUG_TRACE)
            console.log("[WATCHLIST_GUARD]", {
                skipped: true,
                source: source || "unknown",
                depth: WATCHLIST_MUTATION_DEPTH,
            });
        return false;
    }
    ...
    var seen = {};
    incoming = incoming.filter(function (pair) {
    ...
    PAIRS.forEach(function (pair) {
        if (!PAIR_SYMBOLS[pair]) PAIR_SYMBOLS[pair] = pair;
    });
    Object.keys(savedPrices).forEach(function (pair) {
        if (PAIRS.indexOf(pair) === -1) delete savedPrices[pair];
    });
    Object.keys(savedRegimes).forEach(function (pair) {
        if (PAIRS.indexOf(pair) === -1) delete savedRegimes[pair];
    });
    Object.keys(savedSeqStatus).forEach(function (pair) {
        if (PAIRS.indexOf(pair) === -1) delete savedSeqStatus[pair];
    });
    Object.keys(computedSignals).forEach(function (pair) {
        if (PAIRS.indexOf(pair) === -1) delete computedSignals[pair];
    });
    Object.keys(regimeMetaByPair).forEach(function (pair) {
        if (PAIRS.indexOf(pair) === -1) delete regimeMetaByPair[pair];
    });
    Object.keys(lastSuccessfulMarketSymbolByPair).forEach(function (pair) {
        if (PAIRS.indexOf(pair) === -1) delete lastSuccessfulMarketSymbolByPair[pair];
    });
    Object.keys(PAIR_SYMBOLS).forEach(function (pair) {
        if (PAIRS.indexOf(pair) === -1) delete PAIR_SYMBOLS[pair];
    });
    liveSignals = (liveSignals || []).filter(function (sig) {
        return sig && PAIRS.indexOf(sig.pair) > -1;
    });
    tradeQueue = (tradeQueue || []).filter(function (bp) {
        return bp && PAIRS.indexOf(toPairDisplay(bp.pair)) > -1;
    });
    rebuildLiveSignalMap();
    renderTickerStrip();
    var addedPairs = PAIRS.filter(function (pair) {
        return prevPairs.indexOf(pair) === -1;
    });
    if (addedPairs.length && MARKET_DATA_READY && signalEngineStatus !== "COMPUTING") {
        setTimeout(function () {
            runSignalEngineNow({
                manual: true,
                reason: "watchlist_added_pairs",
                pairs: addedPairs.slice(),
            });
        }, 250);
    }
    return true;
...
function bosThresh(pipType, minPips) {
    return minPips * pipMult(pipType);
}

var EF_LEVELS = {};
function efLevel(pair, ratio) {
    var ef = EF_LEVELS[pair];
    if (!ef || !ef.fibHigh || !ef.fibLow) return null;
    return +sfKey(ef.fibHigh, ef.fibLow, ratio).toFixed(pair.indexOf("JPY") > -1 ? 2 : 5);
}
...
var SFL_ANCHORS = {};
function normalizeSflAnchorPayload(
    payload,
    fallbackHigh,
    fallbackLow,
    fallbackSource,
    ...
    var out = {
        fibHigh:
            fallbackHigh != null
                ? parseFloat(fallbackHigh)
                : anchor.fibHigh != null
                  ? parseFloat(anchor.fibHigh)
                  : null,
        fibLow:
            fallbackLow != null
                ? parseFloat(fallbackLow)
                : anchor.fibLow != null
                  ? parseFloat(anchor.fibLow)
                  : null,
    ...
    if (anchor.timeframes && typeof anchor.timeframes === "object") {
        ...
        Object.keys(anchor.timeframes).forEach(function (tf) {
            ...
            out.timeframes[tf] =
                tfAnchor && tfAnchor.fibHigh != null && tfAnchor.fibLow != null
...
function getAuthoritySFAnchor(pair) {
    ...
    if (a.authority && a.authority.fibHigh != null && a.authority.fibLow != null) {
        return {
            fibHigh: a.authority.fibHigh,
            fibLow: a.authority.fibLow,
            source: a.authority.source || "local_fib_composite",
        };
    }
...
function getTimeframeSFAnchor(pair, timeframe) {
    ...
    if (
        ...
        tfKey &&
        a.timeframes[tfKey] &&
        a.timeframes[tfKey].fibHigh != null &&
        a.timeframes[tfKey].fibLow != null
    ) {
        return {
            fibHigh: a.timeframes[tfKey].fibHigh,
            fibLow: a.timeframes[tfKey].fibLow,
            source: tfKey,
        };
    }
...
function publishRuntimeState() {
    DASHBOARD_STATE.watchlist = PAIRS.slice();
    DASHBOARD_STATE.pricesBySymbol =
        savedPrices && typeof savedPrices === "object" ? Object.assign({}, savedPrices) : {};
    DASHBOARD_STATE.regimesBySymbol =
        savedRegimes && typeof savedRegimes === "object" ? Object.assign({}, savedRegimes) : {};
    DASHBOARD_STATE.signalsBySymbol =
        computedSignals && typeof computedSignals === "object"
            ? Object.assign({}, computedSignals)
            : {};
    DASHBOARD_STATE.plannerBySymbol = Array.isArray(tradeQueue)
        ? tradeQueue.reduce(function (map, row) {
              if (row && row.pair) {
                  map[toPairDisplay(row.pair)] = row;
              }
              return map;
          }, {})
        : {};
    runtimeStateSnapshot = {
        ...
        savedRegimes:
            savedRegimes && typeof savedRegimes === "object" ? Object.assign({}, savedRegimes) : {},
        regimeMetaByPair:
            regimeMetaByPair && typeof regimeMetaByPair === "object"
                ? Object.assign({}, regimeMetaByPair)
                : {},
        computedSignals:
            computedSignals && typeof computedSignals === "object"
                ? Object.assign({}, computedSignals)
...
function getRuntimeState() {
    ...
    return {
        ...
        acct:
            snapshot.acct && typeof snapshot.acct === "object"
                ? Object.assign({}, snapshot.acct)
                : snapshot.acct,
...
function refreshRuntimeAfterTimeframeChange() {
    var hasPrices = Object.keys(savedPrices).some(function (k) {
        return savedPrices[k] > 0;
    });
    if (hasPrices && MARKET_DATA_READY) {
        return runSignalEngineNow({ manual: true, reason: "timeframe_change" }).finally(
            function () {
                generatePlan();
            },
        );
...
function getAnchorSet(pair, timeframe) {
    ...
    if (!anchor || anchor.fibHigh == null || anchor.fibLow == null) {
        if (DEBUG_TRACE)
            console.log("[ANCHOR_TRACE:GET_ANCHOR_SET_MISS]", pair, {
...
function levelsFromAnchor(pair, anchor, label) {
    ...
    var dp = isJPY ? 2 : 5;
    function lvl(r) {
        return +sfKey(anchor.fibHigh, anchor.fibLow, r).toFixed(dp);
    }
    var defaultSl = isJPY ? 1.2 : 0.003;
    return [
...
function getChopBand(pair) {
    ...
    if (
        resolved &&
        resolved.chop_band &&
        resolved.chop_band.low != null &&
        resolved.chop_band.high != null
    ) {
        return {
...
function getAllLevels(pair) {
    ...
    return sfl.concat(ef).filter(function (level) {
        ...
        var key = [
            level.fib || "",
            level.pct || "",
            Number(level.price).toFixed(pair.indexOf("JPY") > -1 ? 2 : 5),
        ].join("|");
...
function primaryLivePrice(sig) {
    if (sig && sig.entries && sig.entries.length && sig.entries[0].price != null)
        return sig.entries[0].price;
...
function normalizeLiveSignal(raw) {
    var pair =
        toPairDisplay(raw.pair || raw.display_symbol || raw.symbol || raw.instrument_id) ||
        String(raw.pair || raw.display_symbol || raw.symbol || raw.instrument_id || "")
            .trim()
            .toUpperCase();
    ...
    return {
        ...
        entries: Array.isArray(raw.entries)
            ? raw.entries.map(function (en) {
                  return {
                      level: en.level || "",
                      price: en.price != null ? parseFloat(en.price) : null,
                      status: en.status || "",
                  };
              })
            : [],
        ...
        chop_band:
            raw.chop_band && typeof raw.chop_band === "object"
                ? {
...
function normalizeRegimeMetaRow(raw) {
    ...
    var chopBand =
        raw.chop_band && raw.chop_band.low != null && raw.chop_band.high != null
            ? {
...
function livePoiZone(sig) {
    ...
    return {
        ...
        slBuf: sig.pair && isPairJPY(sig.pair) ? 1.2 : 0.003,
...
function mergePhase4aFromLiveSignal(sig) {
    ...
    var regime =
        sig.regime ||
        (sig.direction === "BUY" ? "TREND UP" : sig.direction === "SELL" ? "TREND DOWN" : null);
    if (regime) savedRegimes[sig.pair] = regime;

    if (sig.sequence_status)
        savedSeqStatus[sig.pair] = normalizeSequenceStatus(sig.sequence_status);

    // Only let backend prices win when they are fresher than the local price source.
    if (
        sig.market_price != null &&
        sig.market_price > 0 &&
        (!(savedPrices[sig.pair] > 0) ||
            shouldAcceptBackendPrice(sig.updated_at || sig.last_signal_at))
    ) {
        savedPrices[sig.pair] = sig.market_price;
    }

    if (sig.ef && sig.ef.fibHigh != null && sig.ef.fibLow != null)
        updateEF(sig.pair, sig.ef.mode || "Range", sig.ef.fibHigh, sig.ef.fibLow);
...
function rebuildLiveSignalMap() {
    ...
    liveSignals.forEach(function (sig) {
        liveSignalMap[sig.instrument_id] = sig;
    });
}
function computeLiveSignalRank(sig) {
    ...
    var inChop = !!(
        sig.market_price != null &&
        ch &&
        sig.market_price >= ch.lo &&
        sig.market_price <= ch.hi
    );
...
function renderLiveSignals() {
    ...
    var runtimeRows = PAIRS.map(function (pair) {
        ...
        var seq = normalizeSequenceStatus(
            resolved.sequence_status || fallbackLive.sequence_status || savedSeqStatus[pair] || "",
        );
        var state = normalizeSignalState(
            resolved.signal_state || fallbackLive.signal_state || fallbackLive.state || "",
            seq,
        );
        var updatedTs =
            resolved.updated_at || fallbackLive.updated_at || fallbackLive.last_signal_at || null;
        var staleMins = minutesSince(updatedTs);
        var runtimeSig = resolved.runtime_signal || null;
        var score =
            runtimeSig && runtimeSig.confluence_score != null
                ? Number(runtimeSig.confluence_score)
                : fallbackLive.score != null
                  ? Number(fallbackLive.score)
                  : null;
        var direction = resolved.direction || fallbackLive.direction || null;
        var rankBase =
            (resolved.setup_quality != null ? Number(resolved.setup_quality) : 0) * 0.7 +
            (resolved.execution_quality != null ? Number(resolved.execution_quality) : 0) * 0.3;
        if (score != null) rankBase += Math.max(0, Number(score));
        if (seq === "READY") rankBase += 10;
        if (state === "ACTIVE") rankBase += 8;
        if (resolved.rr_estimate != null)
            rankBase += Math.min(Math.max(Number(resolved.rr_estimate), 0), 5) * 4;
        var backendSeq = normalizeSequenceStatus(fallbackLive.sequence_status || "");
        var backendState = normalizeSignalState(
            fallbackLive.signal_state || fallbackLive.state || "",
            backendSeq,
        );
        var backendAuthoritative = !!(
            fallbackLive &&
            (backendState === "ACTIVE" || backendSeq === "READY")
        );
        var hasLocalState = !!(
            resolved.has_fresh_local_signal &&
            (state === "ACTIVE" || state === "WATCHLIST") &&
        ...
        return {
            ...
            direction:
                direction ||
                (resolved.final_bias && String(resolved.final_bias).indexOf("BULL") === 0
                    ? "BUY"
                    : resolved.final_bias && String(resolved.final_bias).indexOf("BEAR") === 0
                      ? "SELL"
                      : ""),
            ...
            setup_quality:
                resolved.setup_quality != null
                    ? Number(resolved.setup_quality)
                    : fallbackLive.setup_quality != null
                      ? Number(fallbackLive.setup_quality)
                      : null,
            execution_quality:
                resolved.execution_quality != null
                    ? Number(resolved.execution_quality)
                    : fallbackLive.execution_quality != null
                      ? Number(fallbackLive.execution_quality)
                      : null,
            rr_estimate:
                resolved.rr_estimate != null
                    ? Number(resolved.rr_estimate)
                    : fallbackLive.rr_estimate != null
                      ? Number(fallbackLive.rr_estimate)
                      : null,
            score: score,
            ...
            provenance: hasLocalState
                ? "JS_ENGINE_PENDING_SYNC"
                : fallbackLive.provenance || fallbackLive.source || "BACKEND",
        };
    });
    var ranked = runtimeRows
        .filter(function (sig) {
            return (
                !!sig.direction || sig.signal_state === "ACTIVE" || sig.signal_state === "WATCHLIST"
            );
        })
        .sort(function (a, b) {
    ...
    if (liveSyncEl)
        liveSyncEl.textContent =
            priceEngineMeta && priceEngineMeta.updated_at
                ? formatSastTime(priceEngineMeta.updated_at)
                : "--";
    if (liveMetaEl)
        liveMetaEl.textContent =
            priceEngineMeta && priceEngineMeta.updated_at
                ? "Last Update: " +
                  formatSastDateTime(priceEngineMeta.updated_at) +
                  " - Source: Price Engine"
                : "Waiting for Price Engine";
    if (sidebarSyncEl) {
        sidebarSyncEl.textContent =
            priceEngineMeta && priceEngineMeta.updated_at
                ? formatSastSidebarStamp(priceEngineMeta.updated_at)
                : "--";
        var isRecent = false;
    ...
    if (!ranked.length) {
        topEl.innerHTML =
            '<div style="text-align:center;color:var(--mu);padding:30px;font-family:var(--mo);font-size:11px">No rankable live candidates yet - waiting for backend context or a viable watchlist setup</div>';
    } else {
        topEl.innerHTML =
            '<div class="g2">' +
            ranked
                ...
                .map(function (sig) {
                    ...
                    var dist =
                        sig.rankDistance != null
                            ? sig.pair
                                ? sig.rankDistance.toFixed(0) + "p"
                                : sig.rankDistance.toFixed(2)
                            : "-";
                    ...
                    var provenance =
                        sig.provenance ||
                        sig.source ||
                        (tag === "PINE_WEBHOOK" ? "PINE_WEBHOOK" : "JS_ENGINE_PENDING_SYNC");
                    return (
                        '<div class="card gb"><div class="clbl">' +
                        sig.display_symbol +
                        "</div>" +
                        ...
                        '<div class="' +
                        (sig.direction === "BUY" ? "pg2" : "pr2") +
                        ' pill">' +
                        (sig.direction || "WATCH") +
                        "</div></div>" +
                        statePill(sig.signal_state) +
                        "</div>" +
                        '<div class="kv"><span class="kvl">Rank</span><span class="kvv" style="color:var(--ac)">' +
                        sig.rank.toFixed(1) +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">Setup Grade</span><span class="kvv">' +
                        (sig.setup_class || "WATCH") +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">Regime / Seq</span><span class="kvv">' +
                        (sig.regime || "-") +
                        " / " +
                        (sig.sequence_status || "-") +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">Setup / Exec</span><span class="kvv">' +
                        (sig.setup_quality != null ? sig.setup_quality : "-") +
                        " / " +
                        (sig.execution_quality != null ? sig.execution_quality : "-") +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">Score / R:R</span><span class="kvv">' +
                        (sig.score != null ? sig.score : "-") +
                        " / " +
                        rr +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">Anchor</span><span class="kvv">' +
                        (anchor != null ? (dp ? anchor.toFixed(dp) : anchor) : "-") +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">Distance</span><span class="kvv">' +
                        dist +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">Freshness</span><span class="kvv">' +
                        (stale || '<span class="pgy pill live-pulse">LIVE</span>') +
                        "</span></div>" +
                        '<div class="kv"><span class="kvl">POI / Tag</span><span class="kvv">' +
                        ((sig.poi && sig.poi.type) || "-") +
                        " - " +
                        tag +
                        "</span></div>" +
                        ((sig.setup_class || "") === "BLOCKED"
                            ? '<div style="margin-top:8px;font-size:11px;color:var(--am)">Blocked: ' +
                              (sig.blocked_reason || "unknown reason") +
                              "</div>"
                            : "") +
                        '<div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn btg bts" onclick="openLiveChart(\'' +
                        String(sig.pair || sig.display_symbol).replace(/'/g, "\\'") +
                        "')\">Open Chart</button></div></div>"
                    );
                })
                .join("") +
            "</div>";
    ...
    tableEl.innerHTML = runtimeRows.length
        ? runtimeRows
              .slice()
              .sort(function (a, b) {
              ...
              .map(function (sig) {
                  var rr = sig.rr_estimate != null ? "1:" + sig.rr_estimate.toFixed(2) : "-";
                  var updated = sig.updated_at
                      ? new Date(sig.updated_at).toLocaleTimeString("en-ZA", {
                  ...
                  var stale = stalePill(sig.rankStaleMins);
                  return (
                      '<tr><td class="mn"><button class="btn btg bts" type="button" onclick="openLiveChart(\'' +
                      String(sig.pair || sig.display_symbol).replace(/'/g, "\\'") +
                      "')\">" +
                      sig.display_symbol +
                      "</button></td><td>" +
                      ...
                      '</td><td class="mn">' +
                      updated +
                      (stale ? '<div style="margin-top:4px">' + stale + "</div>" : "") +
                      "</td></tr>"
                  );
              })
              .join("")
        : '<tr><td colspan="9" style="text-align:center;color:var(--mu);padding:20px;font-family:var(--mo);font-size:11px">No live feed yet</td></tr>';
...
function fetchLiveSignals(manual) {
    ...
    return fetch(API.BASE + "live-signals", {
        method: "GET",
        headers: apiGetHeaders(),
        credentials: "include",
    })
        ...
        .then(function (data) {
            ...
            liveSignals = rows.map(normalizeLiveSignal).filter(function (sig) {
                return sig && PAIRS.indexOf(sig.pair) > -1;
            });
            ...
            try {
                ...
                console.warn(
                    "[SNIPER] buildPriceInputs failed after live signal fetch:",
                    eInputs && eInputs.message ? eInputs.message : eInputs,
                );
            }
            renderSeqCards();
            renderSessionBrief();
            ...
            try {
                ...
                console.warn(
                    "[SNIPER] generatePlan failed after live signal fetch:",
                    ePlan && ePlan.message ? ePlan.message : ePlan,
                );
            }
            ...
            return finishRefreshAction(refreshAction, {
                msg:
                    "Live feed refreshed - " +
                    rows.length +
                    " signal" +
        ...
        .catch(function (err) {
            ...
            return finishRefreshAction(refreshAction, {
                msg: "Refresh Live failed - " + err.message,
                type: "warn",
            }).then(function () {
                return false;
            });
...
function persistTradesLocal() {
    ...
    try {
        lsSet("sn_sig", signals);
    } catch (e) {
        xtoast("⚠ Storage limit reached — export data before continuing", "err");
    }
    lsSet("sn_snap", snaps);
...
function buildUserPayload(bucket) {
    if (bucket === "trades")
        return { signals: signals, snapshots: snaps, closed_trades: closedTrades };
    if (bucket === "account")
        return { account: acct, account_info: acctInfo, baseline: baseline, positions: curPos };
...
function queueUserSync(bucket, delay) {
    ...
    USER_SYNC.timers[bucket] = setTimeout(
        function () {
            userFetch(bucket, "POST", buildUserPayload(bucket))
                .then(function (data) {
                    USER_SYNC.lastSavedAt =
                        data && data.saved_at ? data.saved_at : new Date().toISOString();
...
function applyRiskProfile(data) {
    ...
    if (data.usd_to_account_rate != null && !isNaN(parseFloat(data.usd_to_account_rate)))
        USD_TO_ACCOUNT_RATE = parseFloat(data.usd_to_account_rate);
    if (data.instrument_overrides && typeof data.instrument_overrides === "object")
        INSTRUMENT_OVERRIDES = data.instrument_overrides;
    persistSettingsLocal();
}
...
function smcLogout() {
    var url =
        API.USER && API.USER.logout_url
            ? API.USER.logout_url
            : API.WP + "/wp-login.php?action=logout";
    window.location.href = url;
}
function smcLogin() {
    window.location.href =
        API.WP + "/wp-login.php?redirect_to=" + encodeURIComponent(window.location.href);
}
...
function buildMarketDataSymbolCandidates(pair, symbol) {
    var seen = {};
    var ordered = [];
    var normalizedPair = String(pair || "")
        .trim()
        .toUpperCase();
    ...
    if (
        normalizedPair === "GBP/USD" ||
        normalizedPair === "EUR/USD" ||
        normalizedPair === "AUD/USD" ||
        normalizedPair === "USD/JPY" ||
        normalizedPair === "AUD/JPY"
    ) {
    ...
    if (normalizedPair === "BTC/USD")
        aliases.push("BINANCE:BTCUSDT", "BITSTAMP:BTCUSD", "BTC/USD", "BTCUSD");
    if (normalizedPair === "ETH/USD")
        aliases.push("BINANCE:ETHUSDT", "BITSTAMP:ETHUSD", "ETH/USD", "ETHUSD");
...
function parseMarketDataErrorKind(err) {
    ...
    if (/api key is not configured|missing api key|no api key|key is not configured/.test(msg))
        return "missing_key";
    ...
    if (/http 502|http 503|http 504|timeout|temporar|upstream|gateway/.test(msg))
...
function extractRetryAfterSeconds(err) {
    ...
    var match =
        msg.match(/retry[_\s-]?after\s*=\s*(\d+)/i) ||
        msg.match(/retry[_\s-]?after\s*[:\s]\s*(\d+)/i);
...
function hasRenderableLocalEngineState(pairs) {
    ...
    function isRenderableState(row) {
        ...
        var regime = String(row.regime || "")
            .toUpperCase()
            .trim();
        var gate = String(row.gate || "")
            .toUpperCase()
            .trim();
        if (
            (regime === "RANGING" || regime === "STALE" || regime === "UNKNOWN") &&
            (gate === "NONE" || gate === "NO DATA" || gate === "STALE")
        )
            return false;
        return true;
    }
    return keys.some(function (pair) {
        ...
        var runtimeSnap =
            computedSnapshots && computedSnapshots[pair] ? computedSnapshots[pair] : null;
...
function scheduleEngineRetry(delayMs, reason, pairs) {
    ...
    pendingEngineRetryTimer = setTimeout(
        function () {
            pendingEngineRetryTimer = null;
...
function normalizeF3AnchorForPost(pairKey, sig, snapshot) {
    if (DEBUG_TRACE)
        console.log("[ANCHOR_TRACE:F3_SOURCES]", pairKey, {
            ...
            snapshotActiveFibF3:
                snapshot && snapshot.activeFib ? snapshot.activeFib.f3 || null : null,
    ...
    if (normalizedAnchors.f3) {
        if (DEBUG_TRACE)
            console.log("[ANCHOR_TRACE:F3_MATCH]", pairKey, {
    ...
    var sources = [
        ...
        {
            name: "snapshot.anchors.f3",
            value: snapshot && snapshot.anchors ? snapshot.anchors.f3 : null,
        },
        ...
        {
            name: "snapshot.activeFib.f3",
            value: snapshot && snapshot.activeFib ? snapshot.activeFib.f3 : null,
        },
...
async function postEngineToBackend(signalResults, computedRegimes, runSnapshots) {
    if (DEBUG_TRACE)
        console.log("[ENGINE_TRACE:POST_CALL]", {
            pairKeys: Object.keys(signalResults || {}),
            nonNullSignals: Object.keys(signalResults || {}).filter(function (k) {
                return !!signalResults[k];
            }),
    ...
    PAIRS.forEach(function (pair) {
        ...
        var levelsPayload =
            sig && sig.levels ? sig.levels : snapshot && snapshot.levels ? snapshot.levels : null;
        var gateReason = fieldFromSignalOrSnapshot(sig, snapshot, "gate_reason", null);
        var blockedReason =
            fieldFromSignalOrSnapshot(sig, snapshot, "blocked_reason", null) ||
            gateReason ||
            "NO_CANDIDATE";
        if (DEBUG_TRACE)
            console.log("[ANCHOR_TRACE:POST_FINAL]", pairKey, {
                hasAnchors: !!normalizedAnchors,
                anchors: normalizedAnchors,
                hasF3: !!(normalizedAnchors && normalizedAnchors.f3),
                f3High:
                    normalizedAnchors && normalizedAnchors.f3 ? normalizedAnchors.f3.high : null,
        ...
        payload.pairs[pairKey] = {
            ...
            regime:
                computedRegimes[pair] || fieldFromSignalOrSnapshot(sig, snapshot, "regime", null),
            market_price: savedPrices[pair] || 0,
            sequence_status: fieldFromSignalOrSnapshot(
                sig,
                snapshot,
                "sequence_status",
                "AWAIT SWEEP",
            ...
            fib_disagreement_penalty: fieldFromSignalOrSnapshot(
                sig,
                snapshot,
                "fib_disagreement_penalty",
                null,
            ),
        ...
        if (DEBUG_TRACE)
            console.log("[ANCHOR_TRACE:POST]", pairKey, {
                pairDisplay: pair,
    ...
    try {
        ...
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:POST_SKIP]", {
                reason: "post_engine_batch_failed",
...
function fetchRegimes(manual) {
    ...
    return fetch(API.BASE + "regimes", {
        ...
        .then(function (data) {
            ...
            Object.keys(metaData).forEach(function (wpPair) {
                ...
                if (normalizedMeta) {
                    ...
                    if (
                        normalizedMeta.chop_band &&
                        normalizedMeta.chop_band.low != null &&
                        normalizedMeta.chop_band.high != null
                    )
                        hasChopBand = true;
                }
            });
            ...
            if (data.ef_levels) {
                Object.keys(data.ef_levels).forEach(function (pair) {
                    ...
                    if (ef && ef.fibHigh && ef.fibLow)
                        updateEF(pair, ef.mode || "Range", ef.fibHigh, ef.fibLow);
                });
            }
            if (data.sfl_anchors && typeof data.sfl_anchors === "object") {
                Object.keys(data.sfl_anchors).forEach(function (pair) {
                    ...
                    if (DEBUG_TRACE)
                        console.log("[ANCHOR_TRACE:FETCH_REGIMES]", pair, {
                            ...
                            hasStored: !!(
                                dp &&
                                SFL_ANCHORS[dp] &&
                                Number.isFinite(SFL_ANCHORS[dp].fibHigh) &&
                                Number.isFinite(SFL_ANCHORS[dp].fibLow)
                            ),
                        });
                });
            }
            ...
            renderSeqCards();
            {
                ...
                lastRegimeFetch =
                    latestMeta && latestMeta.updated_at
                        ? parseSyncDate(latestMeta.updated_at)
                        : new Date();
                try {
                    buildPriceInputs();
                } catch (eInputs) {
                    console.warn(
                        "[SNIPER] buildPriceInputs failed after regime fetch:",
                        eInputs && eInputs.message ? eInputs.message : eInputs,
                    );
                }
                renderLiveSignals();
                ...
                try {
                    ...
                    console.warn(
                        "[SNIPER] generatePlan failed after regime fetch:",
                        ePlan && ePlan.message ? ePlan.message : ePlan,
                    );
                }
                if (!DATA_HYDRATION.firstHydrationComplete) {
                ...
                if (updated > 0 && !manual) {
                    var ts = lastRegimeFetch.toLocaleTimeString("en-ZA", {
                        timeZone: "Africa/Johannesburg",
                        hour: "2-digit",
                        minute: "2-digit",
                    });
                    xtoast(
                        "Regimes auto-updated from indicator: " + updated + " pairs - " + ts,
                        "ok",
                    );
                }
                var shouldRunFromRegimeHydration =
                    hasFreshMeta ||
                    hasChopBand ||
                    priceChanged ||
                    signalEngineStatus === "STALE" ||
                    signalEngineStatus === "OFFLINE" ||
                    signalEngineStatus === "NOT_READY";
                if (shouldRunFromRegimeHydration && shouldReconcileLocalEngineState(PAIRS)) {
                    if (isTdProxyDeferred())
                        scheduleEngineRetry(
                            (getTdProxyRemainingSeconds() + 1) * 1000,
                            "regimes_reconcile_td_deferred",
                            PAIRS,
                        );
        ...
        .catch(function (err) {
            ...
            if (metaEl)
                metaEl.textContent = "Last Update: Price Engine sync failed (" + err.message + ")";
            return finishRefreshAction(refreshAction, {
                msg: "Refresh Regimes failed - " + err.message,
                type: "warn",
            }).then(function () {
                return false;
            });
...
function fmtLocalClockTime(ms) {
    try {
        ...
        return [sast.getUTCHours(), sast.getUTCMinutes(), sast.getUTCSeconds()]
            .map(function (n) {
...
function setPriceEngineMeta(updatedAt) {
    ...
    var current =
        window._lastWpRegimeMeta && window._lastWpRegimeMeta.updated_at
            ? parseSyncDate(window._lastWpRegimeMeta.updated_at)
            : null;
...
function ingestPriceEngineMetaFromRegimes(data) {
    ...
    Object.keys(meta).forEach(function (key) {
        var candidate =
            meta[key] && meta[key].updated_at ? parseSyncDate(meta[key].updated_at) : null;
...
function getPriceEngineMeta() {
    return window._lastWpRegimeMeta && window._lastWpRegimeMeta.updated_at
        ? window._lastWpRegimeMeta
        : null;
}
...
function renderTickerStrip() {
    ...
    var baseRows = PAIRS.map(function (pair, idx) {
        ...
        return (
            '<div class="smc-ticker__item"><span class="smc-ticker__pair">' +
            pair +
            '</span><span class="smc-ticker__price" id="tick-' +
            key +
            '" data-tick-key="' +
            key +
            '">--</span><span class="smc-ticker__chg" data-chg-key="' +
            key +
            '">--</span></div>' +
...
function updatePriceDisplays() {
    ...
    var liveCount = PAIRS.filter(function (pair) {
        return savedPrices[pair] && savedPrices[pair] > 0;
    }).length;
    ...
    if (hasAnyPrice) {
        var ts = lastFetchTime
            ? lastFetchTime.toLocaleTimeString("en-ZA", {
                  timeZone: "Africa/Johannesburg",
                  hour: "2-digit",
                  minute: "2-digit",
                  second: "2-digit",
    ...
    var BLOCK_STATES = [
        ...
        "Rate limited",
        "No price data",
        "Log in to enable authenticated market data",
    ];
...
function fetchPrices(manual) {
    ...
    if (isTdProxyDeferred()) {
        ...
        if (manual)
            return finishRefreshAction(refreshAction, { msg: deferMsg, type: "warn" }).then(
                function () {
                    return false;
                },
            );
    ...
    if (!MARKET_DATA_READY) {
        ...
        if (manual)
            return finishRefreshAction(refreshAction, {
                msg: "Log in to use authenticated market data routes",
                type: "warn",
            }).then(function () {
                return false;
            });
        showToastThrottled(
            "⚠ WARNING Log in to use authenticated market data routes",
            "warn",
            "market_data_login_required",
            180000,
            "low",
        );
        return Promise.resolve(false);
    ...
    if (fetchPrices._pending) {
        if (manual)
            return finishRefreshAction(refreshAction, {
                msg: "Price refresh already in progress",
                type: "info",
            }).then(function () {
    ...
    if (!manual && fetchPrices._lastCall && now - fetchPrices._lastCall < 30000) {
        ...
        showToastThrottled(
            "⚠ WARNING Rate limit guard: wait " + wait + "s before refreshing again",
            "warn",
            "market_data_rate_guard",
            180000,
            "low",
        );
        return Promise.resolve(false);
    ...
    if (planStatus) {
        planStatus.textContent = "Refreshing...";
        planStatus.style.color = "var(--am)";
    }
    var inlineStatus = document.getElementById("live-price-status-inline");
    if (inlineStatus) {
        inlineStatus.textContent = "Refreshing...";
        inlineStatus.style.color = "var(--am)";
    }
    ...
    var request = marketDataGet({ kind: "prices", symbols: symbols })
        .then(function (data) {
            ...
            var hasDataRateLimit = errors.some(function (e) {
                return /credits|rate.limit|429|per minute/i.test(e);
            });
            if (hasDataRateLimit) {
                var rlSeconds = 15;
                ...
                if (planStatus) {
                    planStatus.textContent = "Rate limited";
                    planStatus.style.color = "var(--re)";
                }
                if (inlineStatus) {
                    inlineStatus.textContent = "Rate limited";
                    inlineStatus.style.color = "var(--re)";
                }
                return finishRefreshAction(refreshAction, {
                    msg:
                        "Price refresh rate-limited — API credits exhausted. Cooldown: " +
                        rlSeconds +
                        "s",
                    type: "warn",
                }).then(function () {
            ...
            var ts = lastFetchTime.toLocaleTimeString("en-ZA", {
                timeZone: "Africa/Johannesburg",
                hour: "2-digit",
                minute: "2-digit",
                second: "2-digit",
            ...
            if (updated === PAIRS.length) {
                ...
                if (planStatus) {
                    planStatus.textContent = okText;
                    planStatus.style.color = "var(--gr)";
                }
                if (inlineStatus) {
                    inlineStatus.textContent = updated + "/" + PAIRS.length + " Live";
                    inlineStatus.style.color = "var(--gr)";
                }
                updatePriceDisplays();
                    ...
                    xtoast(
                        "Prices loaded: " +
                            PAIRS.map(function (p) {
                                var isJPY = p.indexOf("JPY") > -1;
                                return p + " " + savedPrices[p].toFixed(isJPY ? 3 : 5);
                            }).join(" - "),
                        "ok",
                    );
                renderSessionBrief();
                if (typeof window.generatePlan === "function") {
                    try {
                        ...
                        console.warn(
                            "[SNIPER] generatePlan failed after price fetch:",
                            ePlan && ePlan.message ? ePlan.message : ePlan,
                        );
                    }
                }
                ...
                return finishRefreshAction(refreshAction, {
                    msg: "Live prices refreshed for " + updated + " pairs",
                    type: "ok",
                }).then(function () {
                    return true;
                });
                ...
                if (status) {
                    status.textContent = warnText;
                    status.style.color = "var(--am)";
                }
                if (planStatus) {
                    planStatus.textContent = warnText;
                    planStatus.style.color = "var(--am)";
                }
                if (inlineStatus) {
                    inlineStatus.textContent = updated + "/" + PAIRS.length + " Live";
                ...
                if (shouldKickstartEngine) {
                    if (isTdProxyDeferred()) {
                        scheduleEngineRetry(
                            (getTdProxyRemainingSeconds() + 1) * 1000,
                            "prices_partial_kickstart_td_deferred",
                            PAIRS,
                        );
                    } else {
                ...
                if (!manual && errors.length)
                    showToastThrottled(
                        "⚠ WARNING Price fetch issues: " + errors.join(", "),
                        "warn",
                        "market_data_price_issues",
                        180000,
                        "low",
                    );
                return finishRefreshAction(refreshAction, {
                    msg: "Price refresh completed with warnings",
                    type: "warn",
                }).then(function () {
                    return false;
                });
        ...
        .catch(function (err) {
            ...
            MARKET_DATA_KEY_MISSING =
                MARKET_DATA_KEY_MISSING || /api key|no[_\\s-]?key|missing[_\\s-]?key/i.test(errMsg);
            if (!manual && btn) {
                btn.classList.remove("is-loading");
                btn.textContent = "Refresh Prices";
                btn.disabled = false;
            ...
            if (planStatus) {
                planStatus.textContent = "Feed offline";
                planStatus.style.color = "var(--re)";
            }
            var inlineStatusErr = document.getElementById("live-price-status-inline");
            if (inlineStatusErr) {
                inlineStatusErr.textContent = "Feed offline";
                inlineStatusErr.style.color = "var(--re)";
            }
            updatePriceDisplays();
                    ...
                    errorKind === "missing_key"
                        ...
                        : errorKind === "rate_limited"
                          ...
                          : errorKind === "upstream_failure"
                            ...
                            : errorKind === "no_data"
                              ? "no candle/price data returned"
                              : "general backend error";
                showToastThrottled(
                    "⚠ WARNING Live price fetch failed (" +
                        reasonText +
                        "): " +
                        err.message +
                        " - enter prices manually",
                    "warn",
                    "market_data_live_fetch_failed_" + reasonText,
                    120000,
                    "fatal",
                );
            }
            return finishRefreshAction(refreshAction, {
                msg: "Refresh Prices failed - " + err.message,
                type: "warn",
            }).then(function () {
                return false;
            });
...
function estimateFallbackSequenceBar(candles) {
    ...
    var baseQuality = hasTrend
        ? Math.min(80, Math.max(45, Math.round(Math.abs(delta) * 10000)))
        : 50;
    return {
...
function computeSequenceFromDataModule(pair, candles) {
    ...
    if (!dataBridge || typeof dataBridge.computeSweepMssSequence !== "function") {
        if (!dataBridgeMissingNotified) {
            ...
            console.warn(
                "[SniperDashboard] Data bridge missing computeSweepMssSequence; falling back to local sequence estimate.",
            );
            if (typeof xtoast === "function")
                xtoast("⚠ WARNING Data bridge unavailable - using local signal fallback", "warn");
        }
    ...
    if (!sequence || !sequence.bars || !sequence.bars.length)
        return estimateFallbackSequenceBar(candles);
...
function isPredictiveLevelForDirection(level, direction, marketPrice) {
    if (!level || !direction || !isFinite(Number(level.price)) || !isFinite(Number(marketPrice)))
        return false;
...
function getStageStopData(entryPrice, dir, levels, dp, options) {
    ...
    if (opts.excludeEf) {
        directional = directional.filter(function (lv) {
            return !isEfEntryLevel(lv);
        });
    }
    if (opts.preferEf) {
        var efDirectional = directional.filter(function (lv) {
            return isEfEntryLevel(lv);
        });
...
function buildSignalForPair(pair, candles, currentPrice, providedRegime) {
    ...
    var sequenceStatus = hasSnapshot
        ...
        : normalizeSequenceStatus(
              sequenceBar.sequenceStatus || sequenceBar.sequence_status || "AWAIT SWEEP",
          );
    ...
    var direction = null;
    if (hasSnapshot) {
        if (snapshot.final_bias === "BULL_EXP" || snapshot.final_bias === "BULL_PB")
            direction = "BUY";
        else if (snapshot.final_bias === "BEAR_EXP" || snapshot.final_bias === "BEAR_RALLY")
            direction = "SELL";
    ...
    if (!direction && !hasSnapshot)
        direction = sequenceBar.mss_bullish ? "BUY" : sequenceBar.mss_bearish ? "SELL" : null;
    ...
    if (direction && levels.length) {
        ...
        var efCandidates = eligibleLevels.filter(function (lv) {
            return isEfEntryLevel(lv);
        });
        var nonEfCandidates = eligibleLevels.filter(function (lv) {
            return !isEfEntryLevel(lv);
        });
    ...
    var chop =
        hasSnapshot && snapshot.chop_band
            ? { lo: snapshot.chop_band.low, hi: snapshot.chop_band.high }
            : getChopBand(pair);
    var inChop = !!(
        chop &&
        currentPrice != null &&
        currentPrice >= chop.lo &&
        currentPrice <= chop.hi
    );
    var blockedReason = "";
    ...
    if (direction && entryZone && levels.length) {
        ...
        var slBufBase = entryDp === 2 ? 1.5 : 0.004;
        ...
        executionStagePlan = buildStageEntryFramework(
            entryZone,
            direction,
            levels,
            entryDp,
        ...
        executionSlLevels = executionStagePlan.entries.map(function (stageEntry) {
            var stageStop = isEfEntryLevel(entryZone)
                ? getStageStopData(stageEntry.entry, direction, levels, entryDp, {
                      preferEf: true,
                      fallbackToAny: false,
                  })
                : getStageStopData(stageEntry.entry, direction, levels, entryDp, {
            ...
            return {
                ...
                level_label: stageStop.level
                    ? stageStop.level.pct || stageStop.level.fib || null
                    : null,
                level_price: stageStop.level
                    ? +Number(stageStop.level.price).toFixed(entryDp)
                    : null,
            };
        });
        executionSlRule = isEfEntryLevel(entryZone) ? "STAGE_EF_NEXT_LEVEL" : "LEGACY_NEXT_LEVEL";
        if (!fallbackReason && executionStagePlan.fallbackReason)
            fallbackReason = executionStagePlan.fallbackReason;
    }
    ...
    if (resolvedChopBand) {
        resolvedChop = {
            active: !!(
                currentPrice != null &&
                currentPrice >= resolvedChopBand.low &&
                currentPrice <= resolvedChopBand.high
            ),
        ...
        snapshot &&
        snapshot.chop &&
        snapshot.chop.low != null &&
        snapshot.chop.high != null
    ) {
    ...
    return {
        ...
        ede_stars:
            sequenceBar.setup_quality != null
                ? Math.max(1, Math.min(5, Math.round(Number(sequenceBar.setup_quality) / 20)))
                : null,
        confluence_score:
            sequenceBar.execution_quality != null
                ? Math.max(0, Math.min(5, Math.round(Number(sequenceBar.execution_quality) / 20)))
                : 0,
        ...
        execution_quality:
            sequenceBar.execution_quality != null ? Number(sequenceBar.execution_quality) : null,
...
async function fetchCandles(symbol, pair, apiKey) {
    ...
    if (cache && cache.profileKey === cacheKey && Date.now() - cache.fetchedAt < staleMs) {
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:CANDLES_CACHE_HIT]", pair, {
    ...
    try {
        ...
        for (var idx = 0; idx < candidates.length; idx++) {
            ...
            if (data.status === "error" || !data.values) {
                if (data && data.message) {
                    ...
                    if (msgLower.indexOf("credits") !== -1) {
                        if (
                            msgLower.indexOf("minute") !== -1 ||
                            msgLower.indexOf("per minute") !== -1
                        ) {
                ...
                if (DEBUG_TRACE)
                    console.log("[ENGINE_TRACE:CANDLE_CANDIDATE_FAIL]", pair, {
            ...
            if (DEBUG_TRACE)
                console.log("[ENGINE_TRACE:CANDLE_CANDIDATE_OK]", pair, {
                    candidate: candidates[idx],
                    duration_ms: durationMs,
        ...
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:CANDLES_FAIL]", pair, {
...
async function fetchDailyOpen(pair, symbol) {
    try {
        ...
        if (
            meta &&
            meta.day_key === todayKey &&
            meta.symbol === symbol &&
            savedDailyOpens[pair] != null
        ) {
        ...
        if (data.status === "error" || !data.values || data.values.length < 1) {
            if (DEBUG_TRACE)
                console.log("[DAILY_OPEN_FAIL]", pair, { symbol: symbol, reason: "no_data" });
            return null;
        }
        ...
        if (isNaN(openPrice)) {
            if (DEBUG_TRACE)
                console.log("[DAILY_OPEN_FAIL]", pair, { symbol: symbol, reason: "invalid_open" });
            return null;
        }
        if (DEBUG_TRACE) console.log("[DAILY_OPEN_OK]", pair, { symbol: symbol, open: openPrice });
        savedDailyOpenMeta[pair] = {
            day_key: todayKey,
            symbol: symbol,
...
function updateSignalEngineUI() {
    ...
    if (sideLabel) {
        ...
        if (effectiveStatus === "LIVE" || effectiveStatus === "SYNCED")
            sideLabel.classList.add("live-pulse-text");
        ...
        src === "BACKEND_BLUEPRINT"
            ...
            : src === "HYBRID_RUNTIME"
              ...
              : src === "JS_ONLY"
                ...
                : "var(--mu)";
    return (
        '<span class="pgy pill" style="color:' + color + '">' + src.replace(/_/g, " ") + "</span>"
    );
}
function renderComputedSignalCards() {
    var el = document.getElementById("computed-signal-cards");
    if (!el) return;
    var hasSigs = Object.keys(computedSignals).some(function (k) {
        return computedSignals[k] !== null;
    });
    if (!hasSigs) {
        ...
        el.innerHTML =
            '<div style="color:var(--mu);font-family:var(--mo);font-size:11px;padding:12px">' +
            emptyMsg +
            "</div>";
        return;
    }
    var freshestTs =
        PAIRS.map(function (pair) {
            ...
            return sig ? sig.freshness_ts || sig.generated_at : null;
        })
            .filter(Boolean)
            .sort(function (a, b) {
                return b - a;
    ...
    var cards = PAIRS.map(function (pair) {
        ...
        if (!sig)
            return (
                '<div class="card" style="opacity:.5"><div class="clbl">' +
                pair +
                '</div><div style="font-family:var(--mo);font-size:11px;color:var(--mu);margin-top:6px">No signal</div></div>'
            );
        var dirClass = sig.direction === "BUY" ? "pg2" : "pr2";
        var regClass =
            sig.regime === "TREND UP"
                ? "rup"
                : sig.regime === "TREND DOWN"
                  ? "rdn"
                  : sig.regime === "REVERSAL ZONE"
                    ? "rrv"
                    : "rrg";
        ...
        return (
            ...
            '<div class="clbl" style="margin:0">' +
            pair +
            "</div>" +
            '<div style="display:flex;gap:6px;align-items:center">' +
            provenanceTag(sig.source || "JS_ENGINE") +
            '<span class="' +
            dirClass +
            ' pill">' +
            sig.direction +
            ...
            "</span>" +
            (sig.kz_active
                ? '<span class="pac pill">' + sig.kz_label + "</span>"
                : '<span class="pgy pill">Outside KZ</span>') +
            "</div>" +
            '<div class="kv"><span class="kvl">Score</span><span class="kvv">' +
            sig.confluence_score +
            "/5</span></div>" +
            '<div class="kv"><span class="kvl">EDE Stars</span><span class="kvv">' +
            (sig.ede_stars !== null ? sig.ede_stars + "*" : "-") +
            "</span></div>" +
            '<div class="kv"><span class="kvl">Sweep</span><span class="kvv">' +
            (sig.sweep_confirmed ? "OK" : "-") +
            "</span></div>" +
            '<div class="kv"><span class="kvl">Sequence</span><span class="kvv ' +
            (sig.sequence_status === "READY" ? "pos" : "wrn") +
            '">' +
            (sig.sequence_status || "-") +
            "</span></div>" +
            '<div class="kv"><span class="kvl">State</span><span class="kvv ' +
            (sig.signal_state === "ACTIVE" ? "pos" : "wrn") +
            '">' +
            sig.signal_state +
            "</span></div>" +
            (sig.setup_class === "BLOCKED"
                ? '<div style="margin-top:8px;font-size:11px;color:var(--am)">Blocked: ' +
                  ...
                  "</div>"
                : "") +
            '<div style="margin-top:8px;font-size:10px;color:var(--mu)">Freshness: ' +
            freshness +
            "</div>" +
            '<div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">' +
            '<button class="btn btg bts" onclick="openComputedSignalChart(\'' +
            pair +
            "')\">Open Chart</button>" +
            (sig.sequence_status === "READY"
                ? '<button class="btn bta bts" onclick="selectPairForPlan(\'' +
                  pair +
                  "')\">Generate Plan -></button>"
                : '<span style="font-family:var(--mo);font-size:10px;color:var(--mu)">' +
                  sig.sequence_status +
    ...
    el.innerHTML =
        '<div style="font-size:10px;color:var(--mu);font-family:var(--mo);margin-bottom:8px">' +
        metaLine +
        '</div><div class="g3">' +
        cards +
        "</div>";
...
function buildExecutionSignalIdentity(payload) {
    ...
    var zonePrice =
        payload.entry_zone_price != null
            ? Number(payload.entry_zone_price).toFixed(pair.indexOf("JPY") > -1 ? 2 : 5)
            : "0";
    var sl =
        payload.sl != null ? Number(payload.sl).toFixed(pair.indexOf("JPY") > -1 ? 2 : 5) : "0";
    var tp1 =
        payload.tp1 != null ? Number(payload.tp1).toFixed(pair.indexOf("JPY") > -1 ? 2 : 5) : "0";
    var tp2 =
        payload.tp2 != null ? Number(payload.tp2).toFixed(pair.indexOf("JPY") > -1 ? 2 : 5) : "0";
    var regime = String(payload.regime || "")
        .toUpperCase()
        .replace(/\s+/g, "_");
...
async function postExecuteSignals(signalResults) {
    ...
    Object.keys(signalResults).forEach(function (pair) {
        ...
        var payload = {
            ...
            entry_levels: Array.isArray(sig.entry_levels)
                ? deepCloneSignalPayload(sig.entry_levels)
                : null,
        ...
        if (data.blueprints && data.blueprints.length) {
            ...
            xtoast(
                "OK " +
                    ...
                    " blueprint" +
                    (data.blueprints.length > 1 ? "s" : "") +
                    " calculated - Trade Queue updated",
                "ok",
            );
        }
    } catch (e) {
...
function renderServerBlueprintPlan(context, blueprints) {
    ...
    var ddWarnings = blueprints.filter(function (bp) {
        return !!bp.dd_warning;
    });
    var totalRisk = blueprints.reduce(function (sum, bp) {
        return sum + (Number(bp.total_risk_usc) || 0);
    }, 0);
    var equity =
        blueprints[0] && blueprints[0].equity_at_calc
            ? Number(blueprints[0].equity_at_calc)
            : context.equity || 0;
    var riskPct = equity > 0 ? ((totalRisk / equity) * 100).toFixed(1) : "--";
    var ddBanner = "";
    if (ddWarnings.length) {
        ddBanner =
            '<div class="alert ared" style="margin-bottom:10px"><div>!</div><div><strong>Drawdown warning.</strong> ' +
            ddWarnings.length +
            " blueprint" +
            (ddWarnings.length > 1 ? "s carry" : " carries") +
            " backend DD warnings. Review before execution.</div></div>";
    }
    ...
    verdictEl.innerHTML =
        ...
        context.verdictClass +
        '">' +
        '<div class="verdict-lbl" style="color:' +
        (context.verdict.indexOf("EXECUTE") === 0
            ? "var(--gr)"
            : context.verdict.indexOf("NO") === 0
              ? "var(--re)"
              : "var(--am)") +
        '">' +
        context.verdict +
        ...
        provenanceTag("BACKEND_BLUEPRINT") +
        " Live plan</span></div></div>";
    checklistEl.innerHTML = renderChecklist(context.checklist || []);

    laddersEl.innerHTML = blueprints
        .map(function (bp) {
            ...
            var stageRows =
                ...
                ["Entry", "SL", "TP1", "TP2", "Lots", "SL Pips", "Risk USC", "Risk ZAR", "R:R"]
                    .map(function (h) {
                        return '<div class="entry-head entry-cell">' + h + "</div>";
                    })
                    .join("") +
                "</div>";
            ...
            (bp.entries || []).forEach(function (entry, i) {
                ...
                var stageRiskUsc = riskPickNum(stage, [
                    "riskUsc",
                    "riskUSC",
                    "risk_usc",
                    "riskAmount",
                ...
                var stageSl =
                    stage && stage.sl != null
                        ? stage.sl
                        : bp.stage_sls && bp.stage_sls[i] != null
                          ? bp.stage_sls[i]
                          : bp.sl;
                var stageLabelBits = [];
                ...
                stageRows +=
                    '<div class="entry-row">' +
                    '<div class="entry-cell" style="color:var(--mu)">' +
                    (stageLabels[i] || i + 1) +
                    "</div>" +
                    '<div class="entry-cell mn">' +
                    (entry != null ? Number(entry).toFixed(dp) : "--") +
                    (stageLabelBits.length
                        ? '<div style="font-size:10px;color:var(--mu)">' +
                          cleanDisplayText(stageLabelBits.join(" · ")) +
                          "</div>"
                        : "") +
                    "</div>" +
                    '<div class="entry-cell mn neg">' +
                    (stageSl != null ? Number(stageSl).toFixed(dp) : "--") +
                    (stageSlBits.length
                        ? '<div style="font-size:10px;color:var(--mu)">' +
                          cleanDisplayText(stageSlBits.join(" · ")) +
                          "</div>"
                        : "") +
                    "</div>" +
                    '<div class="entry-cell mn pos">' +
                    (bp.tp1 != null ? Number(bp.tp1).toFixed(dp) : "--") +
                    "</div>" +
                    '<div class="entry-cell mn">' +
                    (bp.tp2 != null ? Number(bp.tp2).toFixed(dp) : "--") +
                    "</div>" +
                    '<div class="entry-cell mn">' +
                    (stage
                        ? stage.lot === 0
                            ? "TOO SMALL"
                            : Number(stage.lot).toFixed(2)
                        : "--") +
                    "</div>" +
                    '<div class="entry-cell mn">' +
                    (stage ? stage.sl_pips : "--") +
                    "</div>" +
                    '<div class="entry-cell mn wrn">' +
                    (stageRiskUsc == null ? "--" : riskFmtNum(stageRiskUsc, 2, "--")) +
                    "</div>" +
                    '<div class="entry-cell mn">' +
                    (stageRiskZar == null ? "--" : "R" + riskFmtNum(stageRiskZar, 2, "--")) +
                    "</div>" +
                    '<div class="entry-cell mn" style="color:' +
                    ((bp.rr1 || 0) >= 2
                        ? "var(--gr)"
                        : (bp.rr1 || 0) >= 1.5
                          ? "var(--am)"
                          : "var(--re)") +
                    ';font-weight:700">' +
                    (bp.rr1 != null ? "1:" + Number(bp.rr1).toFixed(2) : "--") +
                    "</div>" +
                    "</div>";
            });
            ...
            return (
                ...
                "<div>" +
                '<span class="ladder-title" style="color:' +
                dirColor +
                '">' +
                pair +
                " " +
                (bp.direction || "UNKNOWN") +
                " BLUEPRINT</span>" +
                "&nbsp;&nbsp;" +
                ...
                "Zone: " +
                (bp.zone_price != null ? Number(bp.zone_price).toFixed(dp) : "--") +
                " &nbsp;|&nbsp; Market: " +
                (bp.market_price != null ? Number(bp.market_price).toFixed(dp) : "--") +
                " &nbsp;|&nbsp; Total risk: " +
                (riskPickNum(bp, [
                    "totalRiskUsc",
                    "totalRiskUSC",
                ...
                "</div></div>" +
                stageRows +
                ((bp.setup_class || "") === "BLOCKED"
                    ? '<div style="margin:8px 14px 0;font-size:11px;color:var(--am)">Blocked: ' +
                      (bp.blocked_reason || "unknown reason") +
                      "</div>"
                    : "") +
                (rb.available
                    ? '<div style="background:var(--bg3);border-top:1px solid var(--bd2);padding:12px 14px;font-family:var(--mo);font-size:12px">Total if all filled: <span style="color:var(--am)">' +
                      riskFmtNum(
                          riskPickNum(rb, [
                              "totalRiskUsc",
                              "totalRiskUSC",
                      ...
                      " USC</span> / <span>" +
                      (riskPickNum(rb, ["totalRiskZar", "totalRiskZAR", "total_risk_zar"]) == null
                          ? "--"
                          : "R" +
                            riskFmtNum(
                                riskPickNum(rb, ["totalRiskZar", "totalRiskZAR", "total_risk_zar"]),
                                2,
                                "--",
                            )) +
                      '</span> &nbsp;|&nbsp; DD Impact: <span style="color:' +
                      (rb.dd_warning ? "var(--re)" : "var(--gr)") +
                      '">' +
                      riskFmtNum(
                          riskPickNum(rb, ["ddImpactPct", "ddImpact", "dd_impact_pct"]),
                          2,
                          "--",
                      ) +
                      "%</span></div>"
                    : "") +
                (bp.dd_warning
                    ? '<div style="margin:10px 14px 14px;padding:8px 12px;background:rgba(245,65,79,.08);border:1px solid rgba(245,65,79,.3);border-radius:4px;font-size:12px;color:var(--re)">' +
                      bp.dd_warning_msg +
                      "</div>"
                    : "") +
                "</div>"
    ...
    riskEl.innerHTML =
        '<div class="card gb"><div class="clbl">Blueprints</div><div class="cval" style="color:var(--ac)">' +
        blueprints.length +
        '</div><div class="csub">Backend-authoritative plans</div></div>' +
        '<div class="card"><div class="clbl">Total Max Risk</div><div class="cval wrn">' +
        totalRisk.toFixed(2) +
        '</div><div class="csub">USC / R' +
        (totalRisk * ZAR).toFixed(2) +
        " / " +
        riskPct +
        "% equity</div></div>" +
        '<div class="card"><div class="clbl">Equity Used</div><div class="cval pos">' +
        (equity ? equity.toLocaleString() : "--") +
        '</div><div class="csub">USC at blueprint calc</div></div>' +
        '<div class="card"><div class="clbl">Queue Status</div><div class="cval" style="color:var(--bl)">' +
        (tradeQueue || []).length +
        '</div><div class="csub">Stored trade queue items</div></div>';

    if (gatesEl) {
        var gateRowsArr = context.gateResults || [];
        if (gateRowsArr.length) {
            gatesEl.innerHTML = gateRowsArr
                .map(function (gr) {
                    var runtimeTruth = gr.runtimeTruth || resolvePairRuntimeSignal(gr.pair);
                    var resolvedRegime = runtimeTruth.regime || gr.regime;
                    var regCls =
                        resolvedRegime === "TREND DOWN"
                            ? "rdn"
                            : resolvedRegime === "TREND UP"
                              ? "rup"
                              : resolvedRegime === "REVERSAL ZONE"
                                ? "rrv"
                                : "rrg";
                    var isJPY = gr.pair.indexOf("JPY") > -1;
                    var dp = isJPY ? 2 : 5;
                    var zoneStr = gr.bestZone
                        ? gr.bestZone.pct + " @ " + gr.bestZone.price.toFixed(dp)
                        : "None";
                    var distStr = gr.bestZone ? gr.bestDist.toFixed(0) + "p" : "--";
                    var ch =
                        runtimeTruth.chop_band &&
                        runtimeTruth.chop_band.low != null &&
                        runtimeTruth.chop_band.high != null
                            ? { lo: runtimeTruth.chop_band.low, hi: runtimeTruth.chop_band.high }
                            : getChopBand(gr.pair);
                    var chopStr = ch
                        ? gr.mkt >= ch.lo && gr.mkt <= ch.hi
                            ? '<span class="pr2 pill">IN CHOP</span>'
                            : '<span class="pg2 pill">Clear</span>'
                        : '<span class="pgy pill">N/A</span>';
                    ...
                    var gateStateColor =
                        gateStateSrc === "OPEN"
                            ? "var(--gr)"
                            : gateStateSrc === "BLOCKED"
                              ? "var(--re)"
                              : "var(--mu)";
                    var gateStatePill =
                        '<span class="pgy pill" style="color:' +
                        gateStateColor +
                        '">' +
                        gateStateSrc +
                        "</span>";
                    ...
                    var rowClass =
                        qualState === "PASS" ? "rg" : gateStateSrc === "OPEN" ? "" : "rr";
                    return (
                        '<tr class="' +
                        rowClass +
                        '">' +
                        ...
                        regCls +
                        '">' +
                        (resolvedRegime || "NO DATA") +
                        "</span></td>" +
                        '<td class="mn">' +
                        (gr.mkt != null ? gr.mkt.toFixed(dp) : "--") +
                        "</td>" +
                        '<td class="mn" style="color:' +
                        (gr.targetSide === "PREMIUM" ? "var(--re)" : "var(--gr)") +
                        '">' +
                        gr.targetSide +
                        (gr.storedGate
                            ? ' <span class="pgy pill" style="font-size:9px">' +
                              gr.storedGate +
                              "</span>"
                            : "") +
                        "</td>" +
                        ...
                        "</td>" +
                        '<td class="mn">' +
                        (gr.zoneStars != null ? gr.zoneStars + " star" : "--") +
                        "</td>" +
                        '<td class="mn" title="' +
                        (gr.edeLabel || "EDE (SF)") +
                        '">' +
                        (gr.edeDistance != null ? gr.edeDistance.toFixed(3) : "--") +
                        "</td>" +
                        "</tr>"
                    );
                })
    ...
    return (checklist || [])
        .map(function (ci) {
            ...
            return (
                ...
                icon +
                "</div>" +
                '<div class="ci-body"><strong>' +
                (ci.type === "cancel"
                    ? '<span style="color:var(--re)">'
                    : ci.type === "place"
                      ? '<span style="color:var(--gr)">'
                      : ci.type === "add" || ci.type === "review"
...
function hasValidFibAnchor(anchor) {
    return !!(
        ...
        anchor.high != null &&
        anchor.low != null &&
        isFinite(Number(anchor.high)) &&
        isFinite(Number(anchor.low))
    );
}
...
function renderUiMetaSummary(entrySource, slRule, fallbackReason) {
    var parts = [
        '<span class="kvl">Entry Source</span><span class="kvv">' +
            cleanDisplayText(entrySource || "FIB") +
            "</span>",
        '<span class="kvl">SL Rule</span><span class="kvv">' +
            cleanDisplayText(slRule || "LEGACY_NEXT_LEVEL") +
            "</span>",
    ];
    if (fallbackReason) {
        parts.push(
            '<span class="kvl">Fallback</span><span class="kvv wrn">' +
                cleanDisplayText(fallbackReason) +
                "</span>",
        );
    }
    var html = "";
...
function renderTradeQueue() {
    ...
    if (!tradeQueue || !tradeQueue.length) {
        el.innerHTML =
            '<div style="color:var(--mu);font-family:var(--mo);font-size:11px;padding:14px">' +
    ...
    var cards = tradeQueue
        .map(function (bp) {
            ...
            if (rb.available && rb.stages) {
                rb.stages.forEach(function (s, i) {
                    ...
                    stageRows +=
                        ...
                        stageLabels[i] +
                        " @ " +
                        riskFmtNum(s.entry, dp, "--") +
                        "</span>" +
                        '<span class="kvv"><span style="color:' +
                        lotColor +
                        '">' +
                        (lot === 0 ? "TOO SMALL" : riskFmtNum(lot, 2, "--") + " lots") +
                        "</span>" +
                        " &middot; " +
                        s.sl_pips +
                        "p &middot; " +
                        '<span style="color:var(--am)">' +
                        (riskUsc == null ? "--" : riskFmtNum(riskUsc, 2, "--")) +
                        " USC</span>" +
                        (stageDetail.length
                            ? ' &middot; <span style="color:var(--mu)">' +
                              cleanDisplayText(stageDetail.join(" · ")) +
                              "</span>"
                            : "") +
                        "</span></div>";
                });
                ...
                stageRows =
                    '<div style="color:var(--mu);font-size:11px;padding:4px 0">' +
                    (rb.reason || "Upload broker report for lot sizing") +
                    "</div>";
            }
            ...
            var genTime = bp.generated_at
                ? new Date(bp.generated_at).toLocaleTimeString("en-ZA", {
                      timeZone: "Africa/Johannesburg",
                      hour: "2-digit",
                      minute: "2-digit",
                  })
            ...
            return (
                ...
                "</div>" +
                "</div>" +
                '<div class="kv"><span class="kvl">Zone</span><span class="kvv wrn">' +
                (bp.zone_price ? bp.zone_price.toFixed(dp) : "--") +
                " &middot; " +
                zoneLabel +
                "</span></div>" +
                '<div class="kv"><span class="kvl">SL</span><span class="kvv neg">' +
                (bp.sl ? bp.sl.toFixed(dp) : "--") +
                " (" +
                (bp.sl_pips_shallow || "--") +
                "p)</span></div>" +
                '<div class="kv"><span class="kvl">R:R (shallow)</span><span class="kvv" style="color:' +
                rrColor +
                '">' +
                rrStr +
                "</span></div>" +
                ...
                stageRows +
                (rb.available
                    ? '<div class="kv"><span class="kvl">Total risk</span><span class="kvv wrn">' +
                      riskFmtNum(
                          riskPickNum(rb, [
                              "totalRiskUsc",
                              "totalRiskUSC",
                    ...
                    : "") +
                (bp.dd_warning
                    ? '<div style="background:rgba(245,65,79,.07);border:1px solid rgba(245,65,79,.25);border-radius:4px;padding:7px 10px;margin-top:8px;font-size:11px;color:var(--re)">' +
                      bp.dd_warning_msg +
                      "</div>"
                    : "") +
                ((bp.setup_class || "") === "BLOCKED"
                ...
                '<span style="font-family:var(--mo);font-size:9px;color:var(--mu)">Freshness: ' +
                formatFreshnessAgo(bp.updated_at || bp.generated_at) +
                " · " +
                genTime +
                " SAST / " +
                (bp.equity_at_calc
                    ? bp.equity_at_calc.toLocaleString() + " USC equity"
                    : "no equity") +
                "</span>" +
                '<div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn btg bts" onclick="openTradeQueueChart(\'' +
                pair +
                '\')">Open Chart</button><button class="btn bta bts" onclick="selectPairForPlan(\'' +
                pair +
                "')\">Generate Plan &rarr;</button></div>" +
                "</div>" +
                "</div>"
            );
        })
...
function clearTradeQueue() {
    ...
    if (USER_SYNC.authenticated) {
        fetch(API.BASE + "user/trade-queue", {
            ...
            console.warn("clearTradeQueue sync failed:", e.message);
        });
    }
}
...
function buildPriceInputs() {
    ...
    el.innerHTML = PAIRS.map(function (p) {
        ...
        return (
            ...
            p +
            "</span>" +
            '<input class="ppr-price" type="number" step="' +
            (isJPY ? "0.01" : "0.00001") +
            '" id="pr-' +
            pId +
            '" value="' +
            ...
            regOpts
                ...
                .join("") +
            "</select>" +
            '<button class="btn btd bts ppr-remove" type="button" onclick="removeInstrument(\'' +
            p +
            "')\">Remove</button>" +
            "</div>"
        );
...
function addInstrument() {
    ...
    if (!pair) {
        xtoast("⚠ WARNING Enter a valid pair, e.g. ETH/USD", "warn");
        return false;
    }
    if (PAIRS.indexOf(pair) > -1) {
        xtoast("⚠ WARNING " + pair + " already in watchlist", "warn");
        return false;
    }
    if (!USER_SYNC.authenticated) {
        xtoast("⚠ WARNING Login required to update watchlist", "warn");
        return false;
    }
    return queueWatchlistMutation(function () {
        return apiPost("user/watchlist/add", { symbol: pair })
            .then(function (res) {
...
function removeInstrument(raw) {
    ...
    return queueWatchlistMutation(function () {
        return apiPost("user/watchlist/remove", { symbol: pair })
            .then(function (res) {
                var payload = res && res.data ? res.data : res;
                if (!(payload && Array.isArray(payload.watchlist))) {
                ...
                fetch(API.BASE + "user/trade-queue", {
                    ...
                    console.warn("removeInstrument trade queue sync failed:", e.message);
                });
...
function resolvePairFreshness(pair) {
    ...
    freshnessMinutes =
        pairFreshnessMinutes !== null ? pairFreshnessMinutes : globalFreshnessMinutes;
    return {
        ...
        freshness_source:
            pairFreshnessMinutes !== null
                ? "pair_signal"
                : globalFreshnessMinutes !== null
                  ? "price_engine"
                  : "none",
    };
}
...
function isValidChopBand(band) {
    return !!(
        ...
        band.low != null &&
        band.high != null &&
        isFinite(Number(band.low)) &&
        isFinite(Number(band.high))
    );
}
function resolvePairRuntimeSignal(pair) {
    ...
    var nowTs = Date.now();
    var localFreshTs = runtimeSig
        ? Number(runtimeSig.freshness_ts || runtimeSig.generated_at || runtimeSig.updated_at || 0)
        : 0;
    var localIsFresh = !!(
        hasLocalSignal &&
        localFreshTs &&
        isFinite(localFreshTs) &&
        nowTs - localFreshTs <= 3 * 60 * 60 * 1000
    );
    var backendState = liveSig
        ? normalizeSignalState(liveSig.signal_state || liveSig.state, liveSig.sequence_status)
        : null;
    var runtimeState = runtimeSig
        ? normalizeSignalState(
              runtimeSig.signal_state || runtimeSig.state,
              runtimeSig.sequence_status,
          )
        : null;
    ...
    var resolved = {
            ...
            runtimeSig && isValidChopBand(runtimeSig.chop_band)
                ...
                : runtimeSnap && isValidChopBand(runtimeSnap.chop_band)
                  ...
                  : liveSig && isValidChopBand(liveSig.chop_band)
                    ...
                    : liveSig &&
                        liveSig.chop &&
                        liveSig.chop.low != null &&
                        liveSig.chop.high != null
                      ? {
                      ...
                      : regimeMeta && isValidChopBand(regimeMeta.chop_band)
                        ...
                        : regimeMeta &&
                            regimeMeta.chop &&
                            regimeMeta.chop.low != null &&
                            regimeMeta.chop.high != null
                          ? {
        ...
        setup_quality:
            runtimeSig && runtimeSig.setup_quality != null
                ? runtimeSig.setup_quality
                : liveSig && liveSig.setup_quality != null
                  ? liveSig.setup_quality
                  : null,
        execution_quality:
            runtimeSig && runtimeSig.execution_quality != null
                ? runtimeSig.execution_quality
                : liveSig && liveSig.execution_quality != null
                  ? liveSig.execution_quality
                  : null,
        rr_estimate:
            runtimeSig && runtimeSig.rr_estimate != null
                ? runtimeSig.rr_estimate
                : liveSig && liveSig.rr_estimate != null
                  ? liveSig.rr_estimate
                  : null,
        updated_at:
            pickFirstNonEmpty(
                runtimeSig && runtimeSig.updated_at,
                runtimeSnap && runtimeSnap.updated_at,
            ) ||
            (liveSig && (liveSig.updated_at || liveSig.last_signal_at)
                ? liveSig.updated_at || liveSig.last_signal_at
                : null),
        source: runtimeSig
            ? "computedSignals"
            : runtimeSnap
    ...
    if (DEBUG_TRACE)
        console.log("[FIX_TRACE:RUNTIME_TRUTH]", pair, {
...
function resolvePairChopState(pair) {
    ...
    if (
        ...
        return {
            band: {
                lo: parseFloat(resolved.chop_band.low),
                hi: parseFloat(resolved.chop_band.high),
            },
            source: resolved.chop_band.source || resolved.source || "runtime",
            has_chop: true,
        };
...
function buildPairStateContext(pair) {
    ...
    var chopState =
        resolvedRuntime.chop_band &&
        resolvedRuntime.chop_band.low != null &&
        resolvedRuntime.chop_band.high != null
            ? {
                  band: {
                      lo: parseFloat(resolvedRuntime.chop_band.low),
                      hi: parseFloat(resolvedRuntime.chop_band.high),
                  },
                  source: resolvedRuntime.chop_band.source || resolvedRuntime.source || "runtime",
                  has_chop: true,
              }
    ...
    var inChop = !!(
        chopState.has_chop &&
        hasPrice &&
        mkt >= chopState.band.lo &&
        mkt <= chopState.band.hi
    );
    var isHydrating = !DATA_HYDRATION.firstHydrationComplete;
    var hasAnyDynamicState =
        hasPrice || hasRegime || hasAnchor || chopState.has_chop || freshness.has_meta;
    var statusReason;
    if (isHydrating && !hasAnyDynamicState) statusReason = "AWAITING_SYNC";
    else if (!freshness.has_meta && !hasRegime) {
        if (
            DATA_HYDRATION.firstHydrationComplete &&
            (!DATA_HYDRATION.pricesLoaded ||
                !DATA_HYDRATION.regimesLoaded ||
                !DATA_HYDRATION.liveLoaded)
        )
            statusReason = "HYDRATION_FAILED";
    ...
    else if (statusReason === "NO_DATA" || statusReason === "NO_REGIME_DATA")
        regimeDisplay = "NO DATA";
    ...
    var storedGate =
        resolvedRuntime.gate != null
            ? String(resolvedRuntime.gate).toUpperCase()
            : pairSig && pairSig.gate
              ? String(pairSig.gate).toUpperCase()
              : null;
    var gateDisplay = {
        value: "UNAVAILABLE",
        mode: "none",
        reason: resolvedRuntime.gate_reason || null,
    };
    if (statusReason === "AWAITING_SYNC") gateDisplay.value = "LOADING";
    else if (statusReason === "NO_DATA" || statusReason === "NO_REGIME_DATA")
        gateDisplay.value = "NO DATA";
    ...
    else if (storedGate === "SELL" || storedGate === "BUY" || storedGate === "BOTH") {
        gateDisplay.value = storedGate;
        gateDisplay.mode = "strict";
    } else if ((resolvedRuntime.gate_reason || "").toUpperCase() === "IN_CHOP_BAND") {
        gateDisplay.value = "CHOP";
        gateDisplay.mode = "strict_reason";
    } else if ((regimeValue || "").toUpperCase() === "RANGING") {
        gateDisplay.value = "RANGING";
        gateDisplay.mode = "strict_reason";
    } else if (
        (storedGate === "NONE" || !storedGate) &&
        regimeValue === "TREND UP" &&
        statusReason === "OK"
    ) {
        gateDisplay.value = "BUY";
        gateDisplay.mode = "display_fallback";
    } else if (
        (storedGate === "NONE" || !storedGate) &&
        regimeValue === "TREND DOWN" &&
        statusReason === "OK"
    ) {
        gateDisplay.value = "SELL";
        gateDisplay.mode = "display_fallback";
    } else if (storedGate === "NONE") {
        gateDisplay.value = "NONE";
        gateDisplay.mode = "strict";
    }

    var sigState;
    if (resolvedRuntime.signal_state) {
        sigState = normalizeSignalState(
            resolvedRuntime.signal_state,
            resolvedRuntime.sequence_status || savedSeqStatus[pair],
        );
    } else if (pairSig) {
        sigState = normalizeSignalState(
            pairSig.signal_state || pairSig.state,
            pairSig.sequence_status || savedSeqStatus[pair],
        );
    } else if (statusReason === "AWAITING_SYNC") {
        sigState = "PENDING";
    } else if (
        statusReason === "STALE_PRICE_ENGINE" ||
        statusReason === "NO_DATA" ||
        statusReason === "NO_REGIME_DATA" ||
        statusReason === "HYDRATION_FAILED"
    ) {
        sigState = "INVALID";
    ...
    if (DEBUG_TRACE)
        console.log(
            "[REGIME_BRIEF] " +
                ...
                chopState.source +
                " fresh_mins=" +
                (freshness.freshness_minutes !== null
                    ? freshness.freshness_minutes.toFixed(1)
                    : "null") +
                " reason=" +
                statusReason +
                " }",
    ...
    if (
        DEBUG_TRACE &&
        (readinessTier === "NOT_READY" || readinessTier === "PENDING" || readinessTier === "WAIT")
    ) {
        var _blocker =
            statusReason !== "OK"
                ...
                : !hasPrice
                  ...
                  : !hasRegime
                    ? "missing_regime"
                    : !gateIsResolved
                      ? "gate_unresolved(gate=" + gateDisplay.value + ")"
                      : !chopIsPresent
                        ? "chop_absent"
                        : "unknown";
    ...
    return {
        ...
        pairSig: pairSig,
        seq_status:
            resolvedRuntime.sequence_status ||
            (pairSig ? pairSig.sequence_status || savedSeqStatus[pair] : savedSeqStatus[pair]),
    };
}
function renderSessionBrief() {
    ...
    var rowsHtml = PAIRS.map(function (pair) {
        ...
        var dp = isJPY ? 2 : 5;
        var regimeHtml;
        if (ctx.status_reason === "AWAITING_SYNC")
            regimeHtml = '<span class="pgy pill">LOADING</span>';
        else if (ctx.status_reason === "STALE_PRICE_ENGINE")
            regimeHtml = '<span class="pgy pill">STALE</span>';
        else if (ctx.status_reason === "HYDRATION_FAILED")
            regimeHtml = '<span class="pgy pill">UNAVAILABLE</span>';
        else if (ctx.status_reason === "NO_DATA" || ctx.status_reason === "NO_REGIME_DATA")
            regimeHtml = '<span class="pgy pill">NO DATA</span>';
        else {
        ...
        var gateHtml;
        var gateValue =
            ctx.gate_display && ctx.gate_display.value ? ctx.gate_display.value : "UNAVAILABLE";
        var gateMode = ctx.gate_display && ctx.gate_display.mode ? ctx.gate_display.mode : "none";
        var gateReason =
            ctx.gate_display && ctx.gate_display.reason ? String(ctx.gate_display.reason) : "";
        var gateTitleText = gateReason ? gateReason + " · " + gateMode : gateMode;
        var gateTitle = ' title="' + escapeHtmlAttr(gateTitleText) + '"';
        if (gateValue === "SELL") {
            gateHtml = '<span class="pr2 pill"' + gateTitle + ">SELL</span>";
        } else if (gateValue === "BUY") {
            gateHtml = '<span class="pg2 pill"' + gateTitle + ">PASS</span>";
        } else if (gateValue === "BOTH") {
            gateHtml = '<span class="pg2 pill"' + gateTitle + ">PASS</span>";
        } else if (gateValue === "UNAVAILABLE") {
            gateHtml = '<span class="pgy pill">BLOCKED</span>';
        } else if (gateValue === "LOADING") {
            gateHtml = '<span class="pgy pill">LOADING</span>';
        } else if (gateValue === "STALE") {
            gateHtml = '<span class="pgy pill">STALE</span>';
        } else if (gateValue === "NO DATA") {
            gateHtml = '<span class="pgy pill">BLOCKED</span>';
        } else if (gateValue === "PENDING") {
            gateHtml = '<span class="pgy pill">PENDING</span>';
        } else if (gateValue === "NOT READY") {
            gateHtml = '<span class="pa2 pill">NOT READY</span>';
        } else if (gateValue === "PARTIAL") {
            gateHtml = '<span class="pa2 pill">PARTIAL</span>';
        } else if (gateValue === "CHOP") {
            gateHtml = '<span class="pr2 pill"' + gateTitle + ">BLOCKED</span>";
        } else if (gateValue === "RANGING") {
            gateHtml = '<span class="pgy pill"' + gateTitle + ">BLOCKED</span>";
        } else {
            gateHtml =
                '<span class="pgy pill"' +
                gateTitle +
                ">" +
                escapeHtmlAttr(gateValue || "BLOCKED") +
                "</span>";
        }
        ...
        var allChecksPassed =
            ...
            ctx.has_price &&
            ctx.has_regime &&
            ctx.gate_is_resolved &&
            ctx.chop_state.has_chop;
        ...
        if (ctx.freshness.freshness_minutes !== null && ctx.freshness.freshness_minutes > 120) {
            ...
            var staleLabel =
                staleMins > 1440 ? Math.round(staleMins / 60) + "h" : Math.round(staleMins) + "m";
            statusHtml =
                '<span class="pgy pill" title="last update ' + staleLabel + ' ago">OFF</span>';
        }
    ...
    if (metaEl) {
        if (wpMeta && wpMeta.updated_at) {
            ...
            var freshLabel =
                freshMins > 120
                    ? '<span style="color:var(--re)">STALE - ' +
                      formatSastDateTime(wpMeta.updated_at) +
                      "</span>"
...
function renderSeqCards() {
    var el = document.getElementById("seq-status-cards");
    if (!el) return;
    el.innerHTML = PAIRS.map(function (pair) {
        var s = normalizeSequenceStatus(savedSeqStatus[pair]),
            label,
            bg,
            tc;
        if (s === "READY") {
            label = "READY";
            bg = "rgba(31,219,122,0.10)";
            tc = "var(--gr)";
        } else if (s === "AWAIT MSS") {
            label = "AWAIT MSS";
            bg = "rgba(245,166,35,0.10)";
            tc = "var(--am)";
        } else if (s === "AWAIT SWEEP") {
            label = "AWAIT SWEEP";
            bg = "rgba(244,65,79,0.10)";
            tc = "var(--re)";
        } else if (s === "STALE") {
            label = "STALE";
            bg = "rgba(136,153,176,0.08)";
            tc = "var(--mu)";
        } else {
            label = "NO DATA";
            bg = "rgba(136,153,176,0.06)";
            tc = "var(--mu)";
        }
        return (
            '<div style="background:' +
            bg +
            ';border:1px solid var(--bd);border-radius:6px;padding:10px 12px"><div style="font-family:var(--mo);font-size:9px;letter-spacing:1.5px;color:var(--mu);margin-bottom:4px">' +
            pair +
            '</div><div style="font-family:var(--mo);font-size:12px;font-weight:700;color:' +
            tc +
            '">' +
            label +
            "</div></div>"
...
function markNoFill(posId) {
    var sig = signals.filter(function (s) {
...
function getTPs(ld) {
    ...
    if (dir === "SELL") {
        targets = sorted
            .filter(function (lv) {
                return lv.price < zonePrice - 0.0001;
            })
            .sort(function (a, b) {
    ...
    return [
        {
            ...
            fill: "1 of 3",
            tp: tps.tp3,
            label: "TP3 major counter zone (all fills)",
        },
...
function buildTPManagement(ld) {
    ...
    html +=
        '<div style="font-family:var(--mo);font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--mu);margin-bottom:12px">TP Assignment - v7.3 Simplified - 1 Position = 1 TP - Broker Rule</div>';
    var scenarios = [
        {
            label: "IF 3 FILL",
            color: "var(--gr)",
    ...
    scenarios.forEach(function (sc) {
        html +=
            '<div style="background:var(--bg2);border-radius:5px;padding:10px 12px;margin-bottom:8px;border-left:3px solid ' +
            sc.color +
            '">';
        html +=
            '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">';
        html +=
            '<span style="font-family:var(--mo);font-size:11px;font-weight:700;color:' +
            sc.color +
            '">' +
            sc.label +
            "</span>";
        ...
        [
            [e1, sc.p1tp, "Shallow (P1)"],
            [e2, sc.p2tp, "Mid (P2)"],
            [e3, sc.p3tp, "Deep (P3)"],
        ].forEach(function (row) {
            html += '<div style="background:var(--bg3);border-radius:3px;padding:6px 8px">';
            html +=
                '<div style="font-family:var(--mo);font-size:9px;color:var(--mu);margin-bottom:2px">' +
                row[2] +
                "</div>";
            if (row[1]) {
                html +=
                    '<div style="font-family:var(--mo);font-size:10px;color:var(--gr);font-weight:700">' +
                    tpStr(row[1]) +
                    "</div>";
                html +=
                    '<div style="font-family:var(--mo);font-size:9px;color:var(--dm)">' +
                    pips(row[0], row[1]) +
                    " | " +
                    rr(row[0], row[1], sl) +
                    " | " +
                    usc(row[0], row[1], lots) +
                    "</div>";
            } else {
    ...
    html +=
        '<div style="font-family:var(--mo);font-size:9px;color:var(--mu);margin-top:6px;line-height:1.6">';
...
function fetchPendingLimits() {
    xtoast(
        "⚠ NOTICE Pending limits: check indicator alerts for proximity (EF levels within 15 pips).",
        "info",
    );
}
...
function _buildRiskSection(ld, rb) {
    ...
    function _fmtNum(v, dp, fallback) {
        var n = _numOrNull(v);
        return n == null ? (fallback == null ? "&mdash;" : fallback) : n.toFixed(dp);
    }
    ...
    if (rb.available) {
        ...
        html +=
            '<div style="font-family:var(--mo);font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--mu);margin-bottom:10px">Risk Breakdown - Account Profile Sizing</div>';
        html +=
        ...
        rb.stages.forEach(function (s, i) {
            ...
            html +=
                '<tr><td style="padding:6px 10px;font-family:var(--mo)">' +
                stageLabels[i] +
                "</td>";
            html +=
                '<td style="padding:6px 10px;font-family:var(--mo)">' +
                _fmtNum(entry, entry > 10 ? 3 : 5, "&mdash;") +
                "</td>";
            html +=
                '<td style="padding:6px 10px;font-family:var(--mo);color:' +
                (lot === 0 ? "var(--re)" : "var(--gr)") +
                '">' +
                (lot === 0 ? "TOO SMALL" : _fmtNum(lot, 2, "&mdash;")) +
                "</td>";
            html +=
                '<td style="padding:6px 10px;font-family:var(--mo)">' +
                (slPips == null ? "&mdash;" : slPips) +
                "</td>";
            html +=
                '<td style="padding:6px 10px;font-family:var(--mo);color:var(--am)">' +
                (_numOrNull(riskUsc) == null
                    ? "&mdash;"
                    : _fmtNum(riskUsc, 2, "&mdash;") + " USC") +
                "</td>";
            html +=
                '<td style="padding:6px 10px;font-family:var(--mo)">' +
                (_numOrNull(riskZar) == null ? "&mdash;" : "R" + _fmtNum(riskZar, 2, "&mdash;")) +
                "</td></tr>";
        });
        html += "</tbody></table></div>";
        var totalRiskUsc = _pickNum(rb, [
            "totalRiskUsc",
            "totalRiskUSC",
            "total_risk_usc",
            "totalRiskAmount",
        ...
        html +=
            '<div style="display:flex;gap:24px;margin-top:10px;font-family:var(--mo);font-size:12px">';
        html +=
            '<span>Total if all filled: <b style="color:var(--am)">' +
            (_numOrNull(totalRiskUsc) == null
                ? "&mdash;"
                : _fmtNum(totalRiskUsc, 2, "&mdash;") + " USC") +
            "</b> / <b>" +
            (_numOrNull(totalRiskZar) == null
                ? "&mdash;"
                : "R" + _fmtNum(totalRiskZar, 2, "&mdash;")) +
            "</b></span>";
        html +=
            '<span>DD Impact: <b style="color:' +
            (rb.ddWarning ? "var(--re)" : "var(--gr)") +
            '">' +
            (_numOrNull(ddImpactPct) == null
                ? "&mdash;"
                : _fmtNum(ddImpactPct, 2, "&mdash;") + "%") +
            "</b></span>";
        html += "</div>";
        if (rb.ddWarning) {
            html +=
                '<div style="margin-top:8px;padding:8px 12px;background:rgba(245,65,79,.08);border:1px solid rgba(245,65,79,.3);border-radius:4px;font-size:12px;color:var(--re)">' +
                rb.ddWarningMsg +
                "</div>";
        }
        html += "</div>";
        ...
        html +=
            '<div style="padding:10px 14px;background:rgba(74,85,104,.15);border-top:1px solid var(--bd2);font-size:12px;color:var(--dm);font-family:var(--mo)">Risk sizing unavailable: ' +
            rb.reason +
            " - configure Account Profile above to enable lot sizing</div>";
    }
    return html;
}
...
function buildLegacyPlanContext() {
    ...
    var prices = synced.prices,
        regimes = synced.regimes;
    ...
    var _staleHTML = "";
    if (_regimeMins !== null && _regimeMins > 30)
        _staleHTML +=
            '<div class="alert aw" style="margin-bottom:8px">WARN Regimes last updated ' +
            Math.floor(_regimeMins) +
            " min ago - may be stale. Refresh before executing.</div>";
    if (_priceMins !== null && _priceMins > 25)
        _staleHTML +=
            '<div class="alert aw" style="margin-bottom:8px">WARN Prices last fetched ' +
            Math.floor(_priceMins) +
            " min ago - refresh before executing.</div>";
    ...
    if (_manualTs > 0 && _manualStaleHrs > 4 && !lastFetchTime) {
        _staleHTML +=
            '<div class="alert aw" style="margin-bottom:8px">WARN Prices last manually set ' +
            Math.round(_manualStaleHrs) +
            "h ago - verify before executing.</div>";
    }
    ...
    PAIRS.forEach(function (p) {
        ...
        if (regime === "RANGING") {
            ...
            var hasSMCOverride =
                (liveSignalMap[p] && liveSignalMap[p].sequence_status === "READY") ||
                (liveSignalMap[pNoSlash] && liveSignalMap[pNoSlash].sequence_status === "READY") ||
                savedSeqStatus[p] === "READY";
            if (!hasSMCOverride) {
                blocked = true;
                blockReason = "REGIME: RANGING - no entries";
            }
        }
        ...
        var smcSig =
            ...
            getBestLiveSignalForPair(p, allowedDir === "BOTH" ? null : allowedDir) ||
            pairSigState;
        var bestZone = null,
            bestDist = 99999,
            bestZoneScore = -9999;
        ...
        if (smcZone) {
            var smcPredictive =
                (smcSig.direction === "SELL" && smcZone.price > mkt) ||
                (smcSig.direction === "BUY" && smcZone.price < mkt);
            if (
                smcPredictive &&
                (smcSig.setup_class === "A+" ||
                    smcSig.setup_class === "A" ||
                    smcSig.setup_class === "B")
            ) {
                bestZone = smcZone;
                bestDist = Math.abs(mkt - smcZone.price) * mult;
                bestZoneScore =
                    ((runtimeTruth.setup_quality != null
                        ? runtimeTruth.setup_quality
                        : smcSig.setup_quality) || 0) +
                    ((runtimeTruth.execution_quality != null
                        ? runtimeTruth.execution_quality
                        : smcSig.execution_quality) || 0) *
                        0.25;
            }
        }
        if (!bestZone) {
            var directionalCandidates = levels.filter(function (lv) {
                ...
                else if (allowedDir === "BOTH" && (lv.side === "PREMIUM" || lv.side === "DISCOUNT"))
                    dirMatch = true;
                if (!dirMatch) return false;
                if (allowedDir === "BOTH") {
                    return (
                        (lv.side === "PREMIUM" && lv.price > mkt) ||
                        (lv.side === "DISCOUNT" && lv.price < mkt)
                    );
                }
                return isPredictiveLevelForDirection(lv, allowedDir, mkt);
            });
            var efDirectional = directionalCandidates.filter(function (lv) {
                return isEfEntryLevel(lv);
            });
            if (efDirectional.length) {
                bestZone =
                    sortStageLevels(efDirectional, allowedDir === "SELL" ? "SELL" : "BUY")[0] ||
                    null;
                if (bestZone) {
        ...
        gateResults.push({
            ...
            gatePass: gatePass,
            targetSide:
                allowedDir === "SELL" ? "PREMIUM" : allowedDir === "BUY" ? "DISCOUNT" : "BOTH",
            smcSignal: smcSig,
            smcReady: smcReady,
        ...
        gateResults[gateResults.length - 1].edeDistance =
            bestZone && _edeAnchor
                ? computeEDEDistance(bestZone.price, _edeAnchor.fibHigh, _edeAnchor.fibLow)
                : 0;
        gateResults[gateResults.length - 1].edeLabel =
            _edeAnchor && _edeAnchor.source === "htf_authority_sf" ? "EDE (HTF SF)" : "EDE (SF)";
    ...
    gateResults.forEach(function (gr) {
        ...
        var levels = getAllLevels(p);
        var dir =
            gr.targetSide === "PREMIUM"
                ? "SELL"
                : gr.targetSide === "DISCOUNT"
                  ? "BUY"
                  : zone.side === "PREMIUM"
                    ? "SELL"
                    : "BUY";
        var pv = getPipValueAccount(p, true) || 0.1;
        var pvPerLot = pv * 100;
        ...
        var slBase =
            nextLevelSL !== null
                ...
                : dir === "SELL"
                  ...
                  : +(zone.price - zone.slBuf).toFixed(dp);
        var isJpy = isJPY;
        var minSlDist = isJpy ? 0.4 : 0.004;
        if (Math.abs(slBase - zone.price) < minSlDist) {
            slBase =
                dir === "SELL"
                    ? +(zone.price + minSlDist).toFixed(dp)
                    : +(zone.price - minSlDist).toFixed(dp);
        }
        ...
        sortedLevels = sortedLevels.filter(function (lv) {
            return !lv.fib || lv.fib.indexOf("EF") === -1;
        });
        var tp1 = null,
            tp2 = null,
            tp3 = null;
        if (dir === "SELL") {
            var below = sortedLevels.filter(function (lv) {
                return lv.price < zone.price && lv.side !== "CHOP";
            });
            ...
            var above = sortedLevels.filter(function (lv) {
                return lv.price > zone.price && lv.side !== "CHOP";
            });
        ...
        var ALLOC = useStaggered ? [0.2, 0.3, 0.5] : [1 / 3, 1 / 3, 1 / 3];
        var entryFramework = buildStageEntryFramework(zone, dir, levels, dp, mkt);
        var entries = [];
        var lotsPerEntry = 0;
        for (var i = 0; i < 3; i++) {
            var entryStage = entryFramework.entries[i];
            var entryPrice = entryStage
                ? entryStage.entry
                : buildLegacySpreadEntries(zone.price, dir, dp)[i];
            var stageStop = isEfEntryLevel(zone)
                ? getStageStopData(entryPrice, dir, levels, dp, {
                      preferEf: true,
                      fallbackToAny: false,
                  })
            ...
            if (Math.abs(entryPrice - sl_i) < minSlDist) {
                sl_i =
                    dir === "SELL"
                        ? +(entryPrice + minSlDist).toFixed(dp)
                        : +(entryPrice - minSlDist).toFixed(dp);
            }
            var slPips = Math.abs(entryPrice - sl_i) * mult;
            var riskForEntry = riskPerLadder * ALLOC[i];
            var entryLots = Math.max(
                Math.round(riskForEntry / (slPips * pvPerLot) / 0.01) * 0.01,
                0.01,
            );
            ...
            entries.push({
                ...
                sl_level_label: stageStop.level
                    ? stageStop.level.pct || stageStop.level.fib || null
                    : null,
                sl_level_price: stageStop.level ? +Number(stageStop.level.price).toFixed(dp) : null,
            });
        }
        var entryPriceArray = entries.map(function (en) {
            return en ? en.entry : null;
        });
        var stageSls = entries.map(function (en) {
            return en ? en.sl : null;
        });
        ...
        var finalStages =
            finalStagePlan && Array.isArray(finalStagePlan.stages_final)
                ? finalStagePlan.stages_final
                : [];
        var stageLots =
            finalStagePlan && Array.isArray(finalStagePlan.stage_lots)
                ? finalStagePlan.stage_lots
                : [];
        ...
        finalStages.forEach(function (stage, idx) {
            ...
            var stageLot =
                stageLots[idx] != null ? Number(stageLots[idx]) : Number(entries[idx].lots || 0);
            ...
            entries[idx].slPips = isFinite(stageSlPips)
                ? +stageSlPips.toFixed(1)
                : entries[idx].slPips;
            ...
            entries[idx].riskUSC = +(entries[idx].slPips * (entries[idx].lots / 0.01) * pv).toFixed(
                2,
            );
        ...
        if (rrFailures.length || (finalStagePlan && finalStagePlan.rr_validation_pass === false)) {
            gateResults.forEach(function (gr2) {
                if (gr2.pair === p) {
                    ...
                    gr2.blockReason =
                        (gr2.blockReason ? gr2.blockReason + " | " : "") +
                        "Stage R:R gate fail: " +
                        rrFailures.join(" | ");
                }
            });
        ...
        var lotsTotal = +entries
            .reduce(function (s, e) {
                return s + e.lots;
            }, 0)
            .toFixed(2);
        var totalRisk = +entries
            .reduce(function (s, e) {
                return s + e.riskUSC;
            }, 0)
            .toFixed(2);
        ladders.push({
            ...
            zone: zone,
            entries: entries,
            lotsPerEntry: lotsPerEntry,
            lotsTotal: lotsTotal,
            regime: gr.regime,
            tp1: tp1,
            tp2: tp2,
            tp3: tp3,
            sl: entries[0] ? entries[0].sl : slBase,
            slPipsBase: Math.abs(entries[0].entry - entries[0].sl) * mult,
            totalRisk: totalRisk,
            stageSls: stageSls,
    ...
    scanOrders.forEach(function (o) {
        ...
        if (!o.sl) {
            ...
            if (!addSlGroups[groupKey])
                addSlGroups[groupKey] = { pair: o.pair, type: o.type, orders: [] };
        ...
        cancelOrders.forEach(function (o) {
            checklist.push({
                ...
                body:
                    "Entry " +
                    o.entry +
                    " sits inside F3 yearly chop zone (37.5%-62.5%). Hard block Phase 1. Cancel before session opens.",
            });
        });
    }
        ...
        if (matchingOrders.length === 0) {
            checklist.push({
                ...
                title:
                    ...
                    " LADDER (" +
                    ld.lotsTotal +
                    " lots across 3 entries)",
                body:
                    ld.zone.pct +
                    " zone (" +
                    ld.zone.price.toFixed(dp) +
                    ") | " +
                    ld.zone.fib +
                    " | " +
                    ld.regime +
                    ...
                    tp3str +
                    " | Total risk: " +
                    ld.totalRisk.toFixed(2) +
                    " USC",
            });
        } else {
            var issues = [];
            var totalExistingLots = matchingOrders.reduce(function (s, o) {
                return s + o.lots;
            }, 0);
            var expectedLots = ld.lotsTotal;
            if (Math.abs(totalExistingLots - expectedLots) > 0.02) {
                var riskActual = matchingOrders[0]
                    ? (
                          Math.abs(matchingOrders[0].entry - (matchingOrders[0].sl || ld.sl)) *
                          mult *
                          (matchingOrders[0].lots / 0.01) *
                          pv
                      ).toFixed(2)
                    : 0;
                issues.push(
                    "Lots: " +
                        totalExistingLots.toFixed(2) +
                        " placed vs " +
                        expectedLots.toFixed(2) +
                        " required by 1% rule (" +
            ...
            matchingOrders.forEach(function (o) {
                if (!o.sl) issues.push("Order #" + o.id + " has NO stop-loss");
                    ...
                    if (slDist < 5)
                        issues.push(
                            "Order #" + o.id + " SL only " + slDist.toFixed(0) + "p - very tight",
                        );
                }
            });
            if (matchingOrders.length < 3)
                issues.push("Only " + matchingOrders.length + "/3 ladder entries placed");
            if (issues.length > 0) {
                checklist.push({
                    type: "review",
                    num: checkNum++,
    ...
    if (tightenPositions.length > 5) {
        checklist.push({
            ...
            title:
                "TIGHTEN - " +
                tightenPositions.length +
                " open sell positions have SL > 300 pips from entry",
    ...
    gateResults
        ...
        .forEach(function (gr) {
            var reason = String(
                gr.qualificationReason || gr.blockReason || gr.gateReason || "",
            ).toUpperCase();
            var regimeLabel = String(gr.regime || "").toUpperCase();
            var bucket = "OTHER";
            if (
                reason.indexOf("CHOP") > -1 ||
                reason.indexOf("IN_CHOP") > -1 ||
                regimeLabel.indexOf("CHOP") > -1
            ) {
                bucket = "CHOP_BLOCKED";
            } else if (
                reason.indexOf("RANG") > -1 ||
                regimeLabel.indexOf("RANGING") > -1 ||
                reason.indexOf("NO_TREND") > -1
            ) {
                bucket = "RANGING";
            } else if (
                (String(gr.gateState || "").toUpperCase() === "OPEN" &&
                    reason.indexOf("NO_ZONE") > -1) ||
                reason.indexOf("OPEN_GATE_NO_ZONE") > -1
            ) {
                bucket = "OPEN_GATE_NO_ZONE";
            }
            if (monitorBuckets[bucket].indexOf(gr.pair) === -1)
                monitorBuckets[bucket].push(gr.pair);
        });
    ...
    if (monitorBuckets.OPEN_GATE_NO_ZONE.length) {
        checklist.push({
            ...
            num: checkNum++,
            title:
                "MONITOR ONLY &mdash; OPEN GATE, NO ZONE: " +
                monitorBuckets.OPEN_GATE_NO_ZONE.join(", "),
    ...
    if (ladders.length === 0) {
        ...
        var _openButNoZone = gateResults.filter(function (gr) {
            return gr.gateState === "OPEN" && gr.qualificationReason === "NO_ZONE";
        }).length;
        var _openButStarFail = gateResults.filter(function (gr) {
            return gr.gateState === "OPEN" && gr.qualificationReason === "STAR_FAIL";
        }).length;
        var _openButRRFail = gateResults.filter(function (gr) {
            return gr.gateState === "OPEN" && gr.qualificationReason === "RR_FAIL";
        }).length;
        var _blockedInChop = gateResults.filter(function (gr) {
            return gr.qualificationReason === "IN_CHOP";
        }).length;
        var _blockedByGate = gateResults.filter(function (gr) {
            return gr.qualificationReason === "GATE_NONE";
        }).length;
        var _ranging = gateResults.filter(function (gr) {
            return gr.qualificationReason === "RANGING_CONTEXT";
        }).length;
        var _openTotal = _openButNoZone + _openButStarFail + _openButRRFail;
        if (_openButRRFail > 0 && _openButRRFail >= _openTotal) {
            verdict = "NO ACTIONABLE LADDER";
            verdictClass = "verdict-no";
            verdictBody =
                "Structural gates are open on " +
                _openTotal +
                " pair" +
                (_openTotal > 1 ? "s" : "") +
                ", but current candidate ladders fail the minimum 2:1 R:R gate. Wait for price to reach a deeper qualifying zone.";
        } else if (_openButNoZone > 0 || _openButStarFail > 0) {
            verdict = "NO TRADE YET";
            verdictClass = "verdict-no";
            verdictBody =
                "Gates are open on " +
                _openTotal +
                " pair" +
                (_openTotal > 1 ? "s" : "") +
                ", but no qualifying SF zones are currently in play. Monitor and wait for price to approach a valid premium or discount zone.";
        } else if (_ranging > 0 || _blockedByGate > 0 || _blockedInChop > 0) {
            verdictBody =
                "All monitored pairs are currently blocked: " +
                _ranging +
                " ranging, " +
                _blockedByGate +
                " gate-blocked, " +
                _blockedInChop +
                " in chop. No entries until gate conditions change.";
        } else {
            verdictBody =
                "No valid SF zones qualify for entry. Wait for price to reach a premium or discount zone outside the chop band.";
        }
    } else if (inKZ) {
        verdict = "EXECUTE - KILL ZONE ACTIVE";
        verdictClass = "verdict-exec";
        verdictBody =
            "You are inside an active Kill Zone. " +
            ladders.length +
            " valid ladder" +
            (ladders.length > 1 ? "s" : "") +
            " identified. Place pending limit orders now. Orders sit at zones 440-2,340 pips from market - they will wait for price to reach them.";
    } else {
        verdict = "PLACE ORDERS - AWAIT KILL ZONE";
        verdictClass = "verdict-wait";
        verdictBody =
            ladders.length +
            " valid ladder" +
            (ladders.length > 1 ? "s" : "") +
            " identified. Place pending limit orders now so they are ready when price reaches the zone during a Kill Zone window. Next KZ: " +
            nextKZ() +
            ".";
    }
...
function generatePlan() {
    ...
    if (planner && typeof planner.renderPlanFromState === "function") {
        try {
            ...
            console.warn(
                "SniperDashboardPlanner.renderPlanFromState failed:",
...
function sourceWithBackendExecutionContract(runtimeTruth) {
    ...
    for (var i = 0; i < candidates.length; i++) {
        ...
        if (
            src.final_bias &&
            src.matrix &&
            typeof src.matrix === "object" &&
            src.pd_array &&
            typeof src.pd_array === "object"
        ) {
            return src;
        }
    }
...
function legacyExecutionPayloadFromLadder(ld, gateResult) {
    var runtimeTruth =
        gateResult && gateResult.runtimeTruth
            ? gateResult.runtimeTruth
            : resolvePairRuntimeSignal(ld.pair);
...
function legacyGeneratePlan(renderOnly) {
    ...
    if (!ctx) {
        ...
        document.getElementById("plan-verdict").innerHTML =
            '<div class="alert aw"><span style="font-size:18px">! </span><strong>Upload your IFX account report (.xlsx) to generate a signal plan.</strong> Equity is required to calculate lot sizes and risk.</div>';
    ...
    var _hasPrices = PAIRS.some(function (p) {
        return savedPrices[p] && savedPrices[p] > 0;
    });
    if (!_hasPrices)
        xtoast(
            "⚠ WARNING No prices set - enter current prices or fetch via Twelve Data before generating a plan",
            "warn",
        );
    document.getElementById("plan-equity-note").textContent =
        "Equity: " +
        ctx.equity.toLocaleString() +
        " USC - 1% risk = " +
        ctx.riskPerLadder.toFixed(2) +
        " USC per ladder";
    ...
    if (!renderOnly && _acctProfile && ladders.length > 0) {
        var gateByPair = {};
        gateResults.forEach(function (gr) {
            if (gr && gr.pair) gateByPair[gr.pair] = gr;
        });
        var _rawSignals = ladders
            .map(function (ld) {
        ...
        if (!_rawSignals.length) {
            if (typeof xtoast === "function")
                xtoast(
                    "Backend execution skipped - missing authoritative gate context for legacy plan fallback",
                    "warn",
                );
        }
        if (_rawSignals.length) {
        ...
        renderPlan(
            {
                ...
                blueprints: _activeBlueprints,
            },
            "BACKEND_BLUEPRINT",
        );
        return;
    }
    var _provenanceFreshMins = lastRegimeFetch
        ? Math.floor((new Date() - lastRegimeFetch) / 60000)
        : null;
    var _provenanceRegimeLabel =
        _provenanceFreshMins === null
            ? "unknown"
            : _provenanceFreshMins > 30
              ? "stale (" + _provenanceFreshMins + " min ago)"
              : "live";
    document.getElementById("plan-output").style.display = "block";
    document.getElementById("plan-verdict").innerHTML =
        _staleHTML +
        ...
        '">' +
        '<div class="verdict-lbl" style="color:' +
        (verdict.indexOf("EXECUTE") === 0
            ? "var(--gr)"
            : verdict.indexOf("NO") === 0
              ? "var(--re)"
              : "var(--am)") +
        '">' +
        verdict +
        ...
        verdictBody +
        '<br><span style="color:var(--ac)">' +
        provenanceTag(lastRegimeFetch ? "HYBRID_RUNTIME" : "JS_ONLY") +
        " " +
        (lastRegimeFetch
            ? "Backend regime " + _provenanceRegimeLabel + " · local qualification"
            : "Local engine only") +
        " </span></div></div>";
    document.getElementById("plan-checklist").innerHTML = renderChecklist(checklist);
    document.getElementById("plan-ladders").innerHTML = ladders
        .map(function (ld) {
            ...
            var regCls =
                ld.regime === "TREND DOWN"
                    ? "rdn"
                    : ld.regime === "TREND UP"
                      ? "rup"
                      : ld.regime === "REVERSAL ZONE"
                        ? "rrv"
                        : "rrg";
            ...
            ld.entries.forEach(function (en, i) {
                ...
                entriesHTML +=
                    ...
                    "</div>" +
                    '<div class="entry-cell mn">' +
                    en.entry.toFixed(ld.dp) +
                    "</div>" +
                    '<div class="entry-cell mn neg">' +
                    en.sl.toFixed(ld.dp) +
                    "</div>" +
                    '<div class="entry-cell mn pos">' +
                    (en.tp1 ? en.tp1.toFixed(ld.dp) : "&mdash;") +
                    "</div>" +
                    '<div class="entry-cell mn" style="color:var(--dm)">' +
                    (en.tp2 ? en.tp2.toFixed(ld.dp) : "&mdash;") +
                    "</div>" +
                    '<div class="entry-cell mn">' +
                    en.lots +
                    "</div>" +
            ...
            return (
                ...
                '&nbsp;&nbsp;<span class="' +
                starCls +
                '" style="font-family:var(--mo);font-weight:700">' +
                starDisp +
                "</span>" +
                '&nbsp;&nbsp;<span class="' +
                regCls +
                ...
                ld.zone.pct +
                " @ " +
                ld.zone.price.toFixed(ld.dp) +
                " &nbsp;|&nbsp; Market: " +
                ld.mkt.toFixed(ld.dp) +
                " &nbsp;|&nbsp; Total lots: " +
                ld.lotsTotal +
                " &nbsp;|&nbsp; Total risk: " +
                ld.totalRisk +
                ...
                "</div>" +
                entriesHTML +
                buildTPManagement(ld) +
                _buildRiskSection(ld, _riskBreakdowns[ld.pair + "_" + ld.dir]) +
                "</div>"
            );
        })
        .join("");
    var totalMaxRisk = ladders.reduce(function (s, ld) {
        return s + ld.totalRisk;
    }, 0);
    var riskPct = ((totalMaxRisk / equity) * 100).toFixed(1);
    document.getElementById("plan-risk").innerHTML =
        '<div class="card gb"><div class="clbl">Ladders</div><div class="cval" style="color:var(--ac)">' +
        ladders.length +
        '</div><div class="csub">Valid signal plans</div></div>' +
        '<div class="card"><div class="clbl">Total Max Risk</div><div class="cval wrn">' +
        totalMaxRisk.toFixed(2) +
        '</div><div class="csub">USC &middot; R' +
        (totalMaxRisk * ZAR).toFixed(2) +
        " &middot; " +
        riskPct +
        "% equity</div></div>" +
        '<div class="card"><div class="clbl">Equity</div><div class="cval pos">' +
        equity.toLocaleString() +
        '</div><div class="csub">USC &middot; Phase 1 Recovery</div></div>' +
        '<div class="card"><div class="clbl">Real Correlated Risk</div><div class="cval pos">~' +
        (totalMaxRisk / 3).toFixed(0) +
        '</div><div class="csub">USC (2-3 fills typical)</div></div>';
    document.getElementById("plan-gates").innerHTML = gateResults
        .map(function (gr) {
            var runtimeTruth = gr.runtimeTruth || resolvePairRuntimeSignal(gr.pair);
            var resolvedRegime = runtimeTruth.regime || gr.regime;
            var regCls =
            ...
            return (
                ...
                gr.targetSide +
                (gr.storedGate
                    ? ' <span class="pgy pill" style="font-size:9px">' + gr.storedGate + "</span>"
                    : "") +
                "</td>" +
                '<td class="mn wrn">' +
                zoneStr +
...
function buildExportTPManagement(ld) {
    ...
    var targets =
        dir === "SELL"
            ? sorted
                  .filter(function (lv) {
                      return lv.price < ld.zone.price - 0.0001;
                  })
                  .sort(function (a, b) {
                      return b.price - a.price;
                  })
    ...
    var tp1 = targets[0] || null,
        tp2 = targets[1] || null,
        tp3 = targets[2] || tp2 || null;
    function fmt(tp) {
        return tp ? tp.price.toFixed(dp) + " (" + tp.pct + ")" : "--";
    }
    function rr(e, tp, sl) {
        if (!tp) return "--";
    ...
    var e1 = ld.entries[0].entry,
        e2 = ld.entries[1] ? ld.entries[1].entry : e1,
        e3 = ld.entries[2] ? ld.entries[2].entry : e2;
    var sl = ld.sl,
        lots = ld.lotsPerEntry;
    var html = '<div style="background:#1a1d2a;border-top:1px solid #28334a;padding:14px">';
    html +=
        '<div style="font-family:monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#4a5568;margin-bottom:12px">TP Assignment - v7.3 Simplified - 1 Position = 1 TP - Broker Rule</div>';
    var scenarios = [
        {
            label: "IF 3 FILL",
            color: "#1fdb7a",
            desc: "Full ladder - all 3 positions open",
            p1: tp1,
            p2: tp2,
            p3: tp3,
        },
        ...
        },
        {
            label: "IF 1 FILL",
            color: "#3d8ef5",
            desc: "Shallow only - run it all the way",
            p1: tp3,
            p2: null,
            p3: null,
        },
    ...
    scenarios.forEach(function (sc) {
        html +=
            '<div style="background:#111620;border-radius:5px;padding:10px 12px;margin-bottom:8px;border-left:3px solid ' +
            sc.color +
            '">';
        ...
        html +=
            ...
            "</span>";
        html +=
            '<span style="font-family:monospace;font-size:10px;color:#4a5568">' +
            sc.desc +
            "</span></div>";
        html += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">';
        [
            ...
            html +=
                '<div style="font-family:monospace;font-size:9px;color:#4a5568;margin-bottom:2px">' +
                row[2] +
                "</div>";
            if (row[1]) {
                html +=
                    '<div style="font-family:monospace;font-size:10px;color:#1fdb7a;font-weight:700">' +
                    fmt(row[1]) +
                    "</div>";
                html +=
                    '<div style="font-family:monospace;font-size:9px;color:#8899b0">' +
                    usc(row[0], row[1], lots) +
                    " | " +
                    rr(row[0], row[1], sl) +
                    "</div>";
    ...
    var tp1str = fmt(tp1);
    html +=
        '<div style="font-family:monospace;font-size:9px;color:#4a5568;margin-top:6px;line-height:1.6">';
    html +=
        "BE PROTECTION: Shallow TP hit -> move Mid+Deep SL to entry | Mid TP hit -> move Deep SL to " +
        tp1str +
        " | Never widen";
    html += "</div></div>";
    return html;
...
function exportPlanHTML() {
    if (!window._lastPlan) {
        xtoast("⚠ WARNING Generate plan first", "warn");
        return;
    }
            ...
            var rows = ld.entries
                .map(function (en, i) {
                    ...
                    return (
                        ...
                        '<td style="padding:7px 10px;color:#4a5568;font-family:monospace">' +
                        (i + 1) +
                        "</td>" +
                        '<td style="padding:7px 10px;font-family:monospace">' +
                        en.entry.toFixed(ld.dp) +
                        "</td>" +
                        '<td style="padding:7px 10px;font-family:monospace;color:#f5414f">' +
                        en.sl.toFixed(ld.dp) +
                        "</td>" +
                        '<td style="padding:7px 10px;font-family:monospace;color:#1fdb7a">' +
                        (en.tp1 ? en.tp1.toFixed(ld.dp) : "—") +
                        "</td>" +
                        '<td style="padding:7px 10px;font-family:monospace;color:#8899b0">' +
                        (en.tp2 ? en.tp2.toFixed(ld.dp) : "—") +
                        "</td>" +
                        '<td style="padding:7px 10px;font-family:monospace">' +
                        en.lots +
                        "</td>" +
                        ...
                        en.slPips +
                        "p</td>" +
                        '<td style="padding:7px 10px;font-family:monospace;color:#f5a623">' +
                        en.riskUSC +
                        "</td>" +
                        '<td style="padding:7px 10px;font-family:monospace;color:#f5a623">R' +
                        en.riskZAR +
                        "</td>" +
                        '<td style="padding:7px 10px;font-family:monospace;color:#1fdb7a">' +
                        rr +
                ...
                .join("");
            return (
                '<div style="background:#111620;border:1px solid #1e2738;border-radius:6px;margin-bottom:14px;overflow:hidden">' +
                ...
                '<div><span style="font-family:monospace;font-size:13px;font-weight:700;color:' +
                dirColor +
                '">' +
                ld.pair +
                " " +
                ...
                " LADDER</span>" +
                '&nbsp;&nbsp;<span style="color:' +
                (ld.zone.stars === 3 ? "#ff8c00" : "#f5a623") +
                '">' +
                starDisp +
                "</span>" +
                '&nbsp;&nbsp;<span style="font-family:monospace;font-size:10px;background:rgba(74,85,104,.3);padding:2px 8px;border-radius:3px">' +
                ld.regime +
                "</span></div>" +
                '<div style="font-family:monospace;font-size:10px;color:#8899b0">Zone: ' +
                ld.zone.pct +
                " @ " +
                ld.zone.price.toFixed(ld.dp) +
                " (" +
                ...
                [
                    ...
                    .map(function (h) {
                        return (
                            '<th style="padding:6px 10px;font-family:monospace;font-size:9px;letter-spacing:1px;text-transform:uppercase;color:#4a5568;text-align:left;border-bottom:1px solid #28334a">' +
                            h +
                            "</th>"
                        );
                    })
                ...
                rows +
                "</tbody></table></div>" +
                buildExportTPManagement(ld) +
                "</div>"
            );
        })
        .join("");
    var checklistHTML = P.checklist
        .map(function (ci, i) {
            var col =
                {
                    cancel: "#f5414f",
            ...
            var icon =
                { cancel: "🚨", add: "⚠️", place: "✅", tighten: "🔧", note: "📋" }[ci.type] || "•";
            return (
                '<div style="display:flex;gap:12px;padding:10px 14px;border-bottom:1px solid #1e2738;align-items:flex-start">' +
                '<div style="font-size:16px;min-width:24px">' +
                icon +
                "</div>" +
                '<div><div style="font-weight:700;margin-bottom:2px;color:' +
                col +
                '">' +
                ci.title +
                "</div>" +
        ...
        .join("");
    var verdictBg =
        P.verdict.indexOf("EXECUTE") === 0
            ? "rgba(31,219,122,.05)"
            : P.verdict.indexOf("NO") === 0
              ? "rgba(245,65,79,.05)"
              : "rgba(245,166,35,.05)";
    var verdictBorder =
        P.verdict.indexOf("EXECUTE") === 0
            ...
            : P.verdict.indexOf("NO") === 0
              ? "#f5414f"
              : "#f5a623";
    var gateHTML = P.gateResults
        .map(function (gr) {
            var isJPY = gr.pair.indexOf("JPY") > -1,
                dp = isJPY ? 2 : 5;
            var gateStr = gr.gatePass
                ? '<span style="background:rgba(31,219,122,.12);color:#1fdb7a;padding:2px 8px;border-radius:3px;font-family:monospace;font-size:9px">EXECUTE</span>'
                : '<span style="background:rgba(245,65,79,.12);color:#f5414f;padding:2px 8px;border-radius:3px;font-family:monospace;font-size:9px">BLOCKED</span>';
            var zoneStr = gr.bestZone
                ? gr.bestZone.pct + " @ " + gr.bestZone.price.toFixed(dp)
                : "None";
            return (
                '<tr style="border-bottom:1px solid #1e2738">' +
                '<td style="padding:6px 10px;font-family:monospace;font-weight:700">' +
                gr.pair +
                "</td>" +
                ...
                (gr.mkt != null ? gr.mkt.toFixed(dp) : "—") +
                "</td>" +
                '<td style="padding:6px 10px;color:#f5a623">' +
                zoneStr +
                "</td>" +
                '<td style="padding:6px 10px;font-family:monospace">' +
                (gr.bestZone ? gr.bestDist.toFixed(0) + "p" : "—") +
                "</td>" +
                '<td style="padding:6px 10px">' +
                gateStr +
                "</td>" +
    ...
    var html =
        ...
        "<title>SMC SuperFIB Signal Plan - Day " +
        day +
        " - " +
        ts +
        "</title>" +
        ...
        '<h1 style="font-family:monospace;font-size:20px;font-weight:700;margin-bottom:4px">SIGNAL PLAN - DAY ' +
        day +
        "</h1>" +
        '<div style="color:#8899b0;font-size:12px;margin-bottom:20px">Generated: ' +
        ts +
        " SAST - Account: 1220086 - Kudzanai Lloyd Taruvinga - Phase 1 Recovery - 20p/50p tolerance</div>" +
        '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px">' +
        '<div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">EQUITY</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#1fdb7a">' +
        equity.toLocaleString() +
        '</div><div style="font-size:11px;color:#8899b0">USC - R' +
        (equity * ZAR).toFixed(0) +
        "</div></div>" +
        '<div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">1% RISK</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#f5a623">' +
        P.riskPerLadder.toFixed(2) +
        '</div><div style="font-size:11px;color:#8899b0">USC per ladder</div></div>' +
        '<div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">LADDERS</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#3d8ef5">' +
        P.ladders.length +
        '</div><div style="font-size:11px;color:#8899b0">Valid signal plans</div></div>' +
        '<div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">COLLECTION</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#f5a623">DAY ' +
        day +
        '</div><div style="font-size:11px;color:#8899b0">of 90 target</div></div>' +
        "</div>" +
        '<div style="border-radius:8px;padding:16px 20px;margin-bottom:20px;border:2px solid ' +
        verdictBorder +
        ";background:" +
        verdictBg +
        '">' +
        '<div style="font-family:monospace;font-size:18px;font-weight:700;letter-spacing:2px;color:' +
        verdictBorder +
        ';margin-bottom:8px">' +
        P.verdict +
        "</div>" +
        ...
        '<div style="background:#111620;border:1px solid #1e2738;border-radius:6px;margin-bottom:20px;overflow:hidden">' +
        checklistHTML +
        "</div>" +
        ...
        '<thead><tr style="background:#161d2a">' +
        ["Pair", "Regime", "Market Price", "Nearest Zone", "Dist to Zone", "Gate"]
            .map(function (h) {
                return (
                    '<th style="padding:7px 10px;font-family:monospace;font-size:9px;letter-spacing:1px;text-transform:uppercase;color:#4a5568;text-align:left;border-bottom:1px solid #28334a">' +
                    h +
                    "</th>"
                );
            })
...
function handleHistoryFile(f) {
    if (!f || !f.name.match(/\.xlsx?$/i)) {
        xtoast("⚠ WARNING Upload broker history as .xlsx", "warn");
        return;
    }
    ...
    r.onload = function (e) {
        try {
            ...
            document.getElementById("hist-stat").textContent =
                "History parse failed: " + err.message;
...
function buildHistoryHeaderMap(row) {
    ...
    row.forEach(function (cell, idx) {
        ...
        if (
            map.ticket == null &&
            (h === "ticket" || h === "position" || h === "position ticket" || h === "order")
        )
            map.ticket = idx;
        else if (map.symbol == null && (h === "symbol" || h === "instrument" || h === "pair"))
            map.symbol = idx;
        else if (map.type == null && (h === "type" || h === "side" || h === "direction"))
            map.type = idx;
        else if (map.lots == null && (h === "lots" || h === "volume" || h === "size"))
            map.lots = idx;
        else if (map.openTime == null && (h === "open time" || h === "time open" || h === "opened"))
            map.openTime = idx;
        else if (
            map.closeTime == null &&
            (h === "close time" || h === "time close" || h === "closed")
        )
            map.closeTime = idx;
        else if (
            map.openPrice == null &&
            (h === "open price" || h === "price open" || h === "entry")
        )
            map.openPrice = idx;
        else if (
            map.closePrice == null &&
            (h === "close price" || h === "price close" || h === "exit")
        )
            map.closePrice = idx;
        ...
        else if (map.profit == null && (h === "profit" || h === "p l" || h === "pl"))
            map.profit = idx;
    });
    return map.ticket != null &&
        map.symbol != null &&
        map.profit != null &&
        (map.closeTime != null || map.closePrice != null)
        ? map
        : null;
}
...
function classifyExactClose(sig, trade) {
    ...
    var be =
        (sl && Math.abs(sl - trade.entry) < (isJPY ? 0.02 : 0.00002)) ||
        Math.abs(trade.profit) < 0.5;
    ...
    sig.closePips =
        trade.dir === "SELL"
            ? +((trade.entry - closePrice) * mult).toFixed(1)
            : +((closePrice - trade.entry) * mult).toFixed(1);
    sig.awaitingHistory = false;
    if (be) sig.outcome = "BE";
    else if (trade.profit > 0 && tp && Math.abs(closePrice - tp) * mult < 10)
        sig.outcome = "WIN-TP";
    else if (trade.profit < 0 && sl && Math.abs(closePrice - sl) * mult < 10)
        sig.outcome = "LOSS-SL";
...
function processHistoryTrades(trades, fname) {
    ...
    closedTrades = Object.keys(tradeMap)
        .map(function (k) {
            return tradeMap[k];
        })
        .sort(function (a, b) {
...
function handleFile(f) {
    if (!f || !f.name.match(/\.xlsx?$/i)) {
        xtoast("Upload .xlsx from IFX Brokers", "warn");
        return;
    }
    ...
    r.onload = function (e) {
        try {
            var wb = XLSX.read(e.target.result, { type: "array" });
            parseRep(
                XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, defval: null }),
                f.name,
            );
        } catch (err) {
            xtoast("Parse error: " + err.message, "err");
        }
    };
    r.readAsArrayBuffer(f);
}
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (mode === "pos" && r[0] && r[0].indexOf(".c") > -1 && r[1] && !isNaN(r[1])) {
            snap.positions.push({
                ...
                pair: np(r[0]),
                ot: r[2],
                dir: r[3] && r[3].toLowerCase() === "buy" ? "BUY" : "SELL",
                lots: parseFloat(r[4]) || 0,
                entry: parseFloat(r[5]) || 0,
                sl: parseFloat(r[6]) || 0,
                tp: parseFloat(r[7]) || 0,
                mkt: parseFloat(r[8]) || 0,
                swap: parseFloat(r[9]) || 0,
                profit: parseFloat(r[10]) || 0,
            });
        }
        if (mode === "ord" && r[0] && r[0].indexOf(".c") > -1 && r[1] && !isNaN(r[1])) {
            snap.orders.push({
                ...
                ot: r[2],
                type: r[3],
                lots: parseFloat(String(r[4]).split("/")[0]) || 0,
                entry: parseFloat(r[5]) || 0,
                sl: parseFloat(r[6]) || 0,
                tp: parseFloat(r[7]) || 0,
                mkt: parseFloat(r[8]) || 0,
...
function processSnap(snap, fname) {
    if (!baseline) {
        ...
        persistSettingsLocal();
        xtoast(
            "Baseline set from " + fname + " — " + snap.positions.length + " positions logged",
            "ok",
        );
    }
    var prev = snaps.length
        ? snaps[snaps.length - 1]
        : { positions: baseline ? baseline.positions.slice() : [], orders: [] };
    snaps.push(snap);
    ...
    var newPos = snap.positions.filter(function (p) {
        return !prevSet[p.id];
    });
    var closedPos = prev.positions.filter(function (p) {
        return !currSet[p.id];
    });
    var heldPos = snap.positions.filter(function (p) {
        return !!prevSet[p.id];
    });
    ...
    newPos.forEach(function (p) {
        if (
            ...
            signals.push(buildSig(p));
    });
    closedPos.forEach(function (p) {
        var sigs = signals.filter(function (s) {
            return s.posId === p.id;
        });
    ...
    var slm = [];
    heldPos.forEach(function (cur) {
        var pp = prevSet[cur.id];
        if (pp && Math.abs(pp.sl - cur.sl) > 0.00001) {
            slm.push(cur.pair + " #" + cur.id + ": " + pp.sl + " to " + cur.sl);
            var sigs = signals.filter(function (s) {
                return s.posId === cur.id;
            });
            if (sigs.length) {
                sigs[0].currentSL = cur.sl;
                sigs[0].currentPL = cur.profit;
            }
        }
    });
    persistTradesLocal();
    ...
    document.getElementById("upstat").textContent =
        "OK " +
        fname +
        " " +
        sess +
    ...
    xtoast(
        sess +
            ...
            " new signals, " +
            closedPos.length +
            " left open book · import history to confirm closes",
        "ok",
    );
    ...
    if (acct && acct.equity)
        document.getElementById("plan-equity-note").textContent =
            "Equity: " +
            acct.equity.toLocaleString() +
            " USC - 1% risk = " +
            (acct.equity * 0.01).toFixed(2) +
            " USC per ladder";
    var hasPrices = Object.keys(savedPrices).some(function (k) {
        return savedPrices[k] > 0;
    });
...
function buildSig(p) {
    var isJPY = p.pair && p.pair.indexOf("JPY") > -1;
    var mult = isJPY ? 100 : 10000;
    var tol = isJPY ? TOL_JPY : TOL_USD;
    var dp = isJPY ? 2 : 5;
    ...
    for (var i = 0; i < levels.length; i++) {
        var lv = levels[i];
        var d = Math.abs(p.entry - lv.price) * mult;
        if (d < bestD) {
            bestD = d;
            best = lv;
        }
    }
    var zm = best && bestD <= tol;
    var ch = getChopBand(p.pair);
    var inChop = ch && p.entry >= ch.lo && p.entry <= ch.hi;
    var rg = savedRegimes[p.pair] || "UNKNOWN";
    var rgOk =
        rg !== "RANGING" &&
        ((p.dir === "SELL" && (rg === "TREND DOWN" || rg === "REVERSAL ZONE")) ||
            (p.dir === "BUY" && (rg === "TREND UP" || rg === "REVERSAL ZONE")));
    var liveSig = getBestLiveSignalForPair(p.pair, p.dir);
    var smcOk = !!(
        liveSig &&
        liveSig.setup_class &&
        liveSig.setup_class !== "BLOCKED" &&
        liveSig.structure &&
        liveSig.structure.internal_shift !== false &&
        liveSig.structure.major_bos !== false &&
        liveSig.poi &&
        liveSig.poi.type
    );
    var zoneStars = best ? getStarsForLevel(p.pair, best) : null;
    var slP = p.sl ? +(Math.abs(p.entry - p.sl) * mult).toFixed(1) : null;
    var tp1P = p.tp ? +(Math.abs(p.tp - p.entry) * mult).toFixed(1) : null;
    var rr1 = slP && tp1P ? +(tp1P / slP).toFixed(2) : null;
    var pv = getPipValueAccount(p.pair, true) || 0.1;
    var riskUSC = slP ? +(slP * (p.lots / 0.01) * pv).toFixed(2) : null;
    var riskZAR = riskUSC ? +(riskUSC * ZAR).toFixed(2) : null;
    var verdict = inChop
        ? "CHOP BLOCK"
        : !zm
          ? "OFF ZONE"
    ...
    return {
        ...
        riskUSC: riskUSC,
        riskZAR: riskZAR,
        zonePct: best ? best.pct : null,
        zonePrice: best ? best.price : null,
        zoneSide: best ? best.side : null,
        zoneStars: zoneStars,
        zoneFib: best ? best.fib : null,
        zoneDistPips: best ? +bestD.toFixed(1) : null,
        zoneMatch: zm,
        inChop: inChop,
        regimeGate: rg,
        regimeOk: rgOk,
        verdict: verdict,
        smcGrade: liveSig && liveSig.setup_class ? liveSig.setup_class : null,
        smcSetupQuality: liveSig && liveSig.setup_quality != null ? liveSig.setup_quality : null,
        outcome: "OPEN",
...
function classifyOC(sig, p, snap) {
    sig.closeDate = snap.time.slice(0, 10);
    var isJPY = p.pair && p.pair.indexOf("JPY") > -1;
    var mult = isJPY ? 100 : 10000;
    var be =
        (p.sl && Math.abs(p.sl - p.entry) < (isJPY ? 0.02 : 0.00002)) || Math.abs(p.profit) < 0.5;
    var pv = getPipValueAccount(p.pair, true) || 0.1;
    var expLoss = sig.slPips ? -(sig.slPips * (p.lots / 0.01) * pv) : null;
    sig.closePL = p.profit;
    sig.closePips =
        p.dir === "SELL"
            ? +((p.entry - p.mkt) * mult).toFixed(1)
            : +((p.mkt - p.entry) * mult).toFixed(1);
    if (be) {
        sig.outcome = "BE";
    } else if (p.profit > 0 && p.tp && Math.abs(p.mkt - p.tp) * mult < 10) {
        sig.outcome = "WIN-TP";
    } else if (p.profit < 0 && expLoss && Math.abs(p.profit - expLoss) < 5) {
        sig.outcome = "LOSS-SL";
    } else if (p.profit > 0) {
        sig.outcome = "WIN-MANUAL";
    } else {
        sig.outcome = "MANUAL-CLOSE";
    }
}
...
function showDelta(snap, newPos, closedPos, heldPos, prev) {
    ...
    var heldPLnow = heldPos.reduce(function (s, p) {
        return s + (p.profit || 0);
    }, 0);
    var heldPLprev = prev.positions
        .filter(function (p) {
            return heldPos.some(function (h) {
                return h.id === p.id;
            });
        })
        .reduce(function (s, p) {
    ...
    document.getElementById("delcards").innerHTML =
        mk(
            ...
            '<div class="cval ' +
                ...
                '">' +
                newPos.length +
                '</div><div class="csub">Auto-matched to zones</div>',
        ) +
        mk(
            "rb",
            "LEFT OPEN BOOK",
            '<div class="cval ' +
                (closedPos.length > 0 ? "neg" : "") +
                '">' +
                closedPos.length +
                '</div><div class="csub">' +
                (closedPos.length
                    ? "Awaiting exact history confirmation"
                    : "No exits this upload") +
                "</div>",
        ) +
        mk(
            "bb",
            "HELD",
            '<div class="cval" style="color:var(--bl)">' +
                heldPos.length +
                '</div><div class="csub" style="color:' +
                heldCol +
                '">' +
                (heldDiff !== 0
                    ? heldSign + heldDiff.toFixed(2) + " USC on open book"
                    : "Book unchanged") +
                "</div>",
        );
    if (newPos.length) {
        var h =
            '<div style="font-family:var(--mo);font-size:9px;letter-spacing:2px;color:var(--gr);text-transform:uppercase;margin-bottom:8px">New Entries</div>' +
        ...
        newPos.forEach(function (p) {
            ...
            if (!sig) {
                h +=
                    '<tr class="rg" style="opacity:.45"><td class="mn">' +
                    p.id +
                    '</td><td class="mn"><strong>' +
                    pair +
                    '</strong></td><td colspan="8" style="color:var(--mu);font-family:var(--mo);font-size:10px">NO SIGNAL MATCH — position not in signals log</td></tr>';
                return;
            }
            var isJPY = pair.indexOf("JPY") > -1,
                dp = isJPY ? 2 : 5;
            var vc =
                sig.verdict === "VALID ENTRY"
                    ? "pos"
                    : sig.verdict === "CHOP BLOCK" || sig.verdict === "REGIME BLOCK"
                      ? "neg"
                      : "wrn";
            var sd =
                sig.zoneStars === 3
            ...
            h +=
                ...
                "<td>" +
                (p.dir === "BUY"
                    ? '<span class="pg2 pill">BUY</span>'
                    : '<span class="pr2 pill">SELL</span>') +
                "</td>" +
                '<td class="mn">' +
                p.entry.toFixed(dp) +
                "</td>" +
                '<td class="mn wrn">' +
                (sig.zonePct || "OFF ZONE") +
                "</td>" +
                '<td class="mn">' +
                (sig.zoneDistPips != null ? sig.zoneDistPips + "p" : "-") +
                "</td>" +
                "<td>" +
                sd +
                "</td>" +
...
function renderHeader() {
    ...
    if (nameEl) {
        if (acctInfo.name)
            nameEl.innerHTML =
                '<div class="smc-sb-meta-row"><span class="smc-sb-meta-value">' +
                acctInfo.name +
                "</span></div>";
        else if (isLoggedIn && user.display_name)
            nameEl.innerHTML =
                '<div class="smc-sb-meta-row"><span class="smc-sb-meta-value">' +
                user.display_name +
                "</span></div>";
    ...
    if (acctEl) {
        if (acctInfo.account)
            acctEl.innerHTML =
                '<div class="smc-sb-meta-row"><span class="smc-sb-meta-label">Account:</span><span class="smc-sb-meta-value">' +
                acctInfo.account +
                "</span></div>" +
                '<div class="smc-sb-meta-row"><span class="smc-sb-meta-value">' +
                (acctInfo.company || "IFX Brokers") +
                "</span></div>" +
                '<div class="smc-sb-meta-row"><span class="smc-sb-meta-value">USC</span></div>';
        else if (isLoggedIn && user.email)
            acctEl.innerHTML =
                '<div class="smc-sb-meta-row"><span class="smc-sb-meta-label">User:</span><span class="smc-sb-meta-value">' +
                user.email +
                "</span></div>";
    ...
    if (navUser)
        navUser.textContent = isLoggedIn && user.display_name ? user.display_name : "Session";
...
function renderAcct() {
    ...
    if (!acct) {
        var emptySub =
            API.USER && API.USER.id
                ? "Upload report to load broker data"
                : "Login to load broker data";
        el.innerHTML =
            ...
            '<div class="card"><div class="clbl">Balance</div><div class="cval" style="color:var(--mu)">--</div><div class="csub">' +
            emptySub +
            "</div></div>" +
            '<div class="card"><div class="clbl">Floating P/L</div><div class="cval" style="color:var(--mu)">--</div><div class="csub">' +
            emptySub +
            "</div></div>" +
            '<div class="card"><div class="clbl">Margin Level</div><div class="cval" style="color:var(--mu)">--</div><div class="csub">' +
            emptySub +
            "</div></div>";
        return;
    }
    ...
    function chg(cur, old, isAbsolute) {
        ...
        if (isAbsolute)
            return (
                '<span style="color:' +
                col +
                ';font-size:10px;font-family:var(--mo)"> ' +
                sign +
                diff.toFixed(2) +
                " vs last</span>"
            );
    ...
    el.innerHTML =
        ...
        '<div class="cval" style="color:' +
        flColor +
        '">' +
        (a.floatingPL >= 0 ? "+" : "") +
        a.floatingPL.toFixed(2) +
        "</div>" +
        '<div class="csub">USC' +
        flChg +
        "</div>" +
        ...
        "%</div>" +
        '<div class="csub">' +
        (ml > 5000 ? "Strong" : ml > 500 ? "Floor OK" : "EMERGENCY") +
        mlChg +
        "</div>" +
        "</div>";
}
function renderSigs() {
    ...
    var d0ids = {};
    ((baseline && baseline.positions) || []).forEach(function (d) {
        d0ids[d.id] = true;
    });
    var post = signals.filter(function (s) {
        return !d0ids[s.posId];
    });
    if (!post.length) {
        var sumEmpty = document.getElementById("sig-summary");
        if (sumEmpty) sumEmpty.style.display = "none";
        el.innerHTML =
            '<div style="text-align:center;color:var(--mu);padding:30px;font-family:var(--mo);font-size:11px">No signals yet - upload AM report - signals auto-generated - Day 0 legacy book excluded</div>';
        return;
    }
    var noFills = post.filter(function (s) {
        return s.outcome === "NO-FILL";
    });
    var awaitingHistory = post.filter(function (s) {
        return s.outcome === "AWAIT_HISTORY";
    });
    var openCount = post.filter(function (s) {
        return s.outcome === "OPEN";
    }).length;
    var closed = post.filter(function (s) {
        return (
            s.outcome !== "OPEN" &&
            s.outcome !== "INVALID" &&
            s.outcome !== "NO-FILL" &&
            s.outcome !== "AWAIT_HISTORY"
        );
    });
    var wins = closed.filter(function (s) {
        return s.outcome.indexOf("WIN") === 0;
    });
    var losses = closed.filter(function (s) {
        return s.outcome === "LOSS-SL";
    });
    var be = closed.filter(function (s) {
        return s.outcome === "BE";
    });
    var totalPL = closed.reduce(function (a, s) {
        return a + (s.closePL || 0);
    }, 0);
    ...
    if (sumEl) {
        ...
        sumEl.innerHTML =
            '<div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;padding:10px 14px;background:var(--bg2);border:1px solid var(--bd);border-radius:6px;font-family:var(--mo);font-size:11px">' +
            '<span style="color:var(--mu)">Total: <strong style="color:var(--bl)">' +
            post.length +
            "</strong></span>" +
            '<span style="color:var(--mu)">·</span>' +
            '<span class="pg2 pill">W ' +
            wins.length +
            "</span>" +
            '<span class="pr2 pill">L ' +
            losses.length +
            "</span>" +
            '<span class="pa2 pill">BE ' +
            be.length +
            "</span>" +
            '<span class="pb2 pill">AH ' +
            awaitingHistory.length +
            "</span>" +
            '<span class="pp2 pill">NF ' +
            noFills.length +
            "</span>" +
            '<span style="color:var(--mu)">·</span>' +
            '<span style="color:var(--mu)">Win Rate: <strong style="color:' +
            (closed.length && wins.length / closed.length >= 0.6
                ? "var(--gr)"
                : closed.length && wins.length / closed.length >= 0.45
                  ? "var(--am)"
                  : "var(--re)") +
            '">' +
            ...
            "</strong></span>" +
            '<span style="color:var(--mu)">·</span>' +
            '<span style="color:var(--mu)">Fill Rate: <strong style="color:' +
            (resolved && closed.length / resolved >= 0.7
                ? "var(--gr)"
                : resolved && closed.length / resolved >= 0.5
                  ? "var(--am)"
                  : "var(--re)") +
            '">' +
            fillRate +
            "</strong></span>" +
            '<span style="color:var(--mu)">·</span>' +
            '<span>Net P/L: <strong style="color:' +
            (totalPL >= 0 ? "var(--gr)" : "var(--re)") +
            '">' +
            (totalPL >= 0 ? "+" : "") +
            totalPL.toFixed(2) +
            ' USC</strong> <span style="color:var(--dm);font-size:10px">' +
            plZAR(totalPL) +
            "</span></span>" +
            "</div>";
    }
    el.innerHTML = post
        ...
        .reverse()
        .map(function (sig, i) {
            var isJPY = sig.pair && sig.pair.indexOf("JPY") > -1,
                dp = isJPY ? 2 : 5;
            var oc = sig.outcome || "OPEN";
            var cls =
                oc === "OPEN"
                    ? "so"
                    : oc === "NO-FILL"
                ...
                oc === "OPEN" || oc === "AWAIT_HISTORY"
                    ...
                    : oc.indexOf("WIN") === 0
                      ...
                      : oc === "BE"
                        ? "var(--am)"
                        : "var(--re)";
            var sd = sig.zoneStars === 3 ? "3-STAR" : sig.zoneStars === 2 ? "2-STAR" : "1-STAR";
            var sdCls = sig.zoneStars === 3 ? "s3" : sig.zoneStars === 2 ? "s2" : "s1";
            var rg = sig.regimeGate;
            var rgCls =
                rg === "TREND DOWN"
                    ? "rdn"
                    : rg === "TREND UP"
            ...
            return (
                ...
                (sig.dir === "BUY"
                    ? '<span class="pg2 pill">BUY</span>'
                    : '<span class="pr2 pill">SELL</span>') +
                '</div><div style="font-family:var(--mo);font-size:10px;color:var(--mu);margin-top:3px">' +
                (sig.openTime || "") +
                '</div></div><div class="sb2"><span class="' +
                sdCls +
                '" style="font-family:var(--mo);font-weight:700">' +
                sd +
                '</span><span class="' +
                ...
                getOutcomeBadge(oc) +
                (sig.closePips != null
                    ? '<span style="font-family:var(--mo);font-size:11px;color:' +
                      (sig.closePips > 0 ? "var(--gr)" : "var(--re)") +
                      '">' +
                      (sig.closePips > 0 ? "+" : "") +
                      sig.closePips +
                      "p</span>"
                ...
                "</div></div>" +
                '<div class="sg">' +
                sf("Zone", sig.zonePct || "OFF ZONE", sig.zoneMatch ? "pos" : "neg") +
                sf("Zone Price", sig.zonePrice ? sig.zonePrice.toFixed(dp) : "-", "") +
                sf("Entry", sig.entry ? sig.entry.toFixed(dp) : "-", "") +
                sf("SL", sig.sl ? sig.sl.toFixed(dp) : "-", "neg") +
                sf("TP", sig.tp ? sig.tp.toFixed(dp) : "-", "pos") +
                sf("Lots", sig.lots, "") +
                sf("SL Pips", sig.slPips ? sig.slPips + "p" : "-", "") +
                sf("Risk USC", sig.riskUSC ? sig.riskUSC + " USC" : "-", "wrn") +
                sf("Risk ZAR", sig.riskZAR ? "R" + sig.riskZAR : "-", "") +
                sf(
                    "R:R",
                    sig.rr1
                        ? (function () {
                              var rr = sig.rr1;
                              var rrClass = rr >= 2.0 ? "pos" : rr >= 1.5 ? "wrn" : "neg";
                              return (
                                  '<span class="mn ' + rrClass + '">1:' + rr.toFixed(1) + "</span>"
                              );
                          })()
                        : "-",
                    "",
                ) +
                sf("Fib Source", sig.zoneFib || "-", "") +
                sf("Dist to Zone", sig.zoneDistPips != null ? sig.zoneDistPips + "p" : "-", "") +
                sf("SMC Grade", sig.smcGrade || "-", "") +
                sf("Verdict", sig.verdict, vc) +
                (sig.closePL != null
                    ? sf("Close P/L", plBoth(sig.closePL), sig.closePL >= 0 ? "pos" : "neg")
                    : "") +
                (sig.awaitingHistory ? sf("Close Status", "History Import Needed", "wrn") : "") +
                (sig.noFillDate ? sf("No Fill Date", sig.noFillDate, "") : "") +
                "</div>" +
                '<div style="margin-top:10px;display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap"><button class="btn btg bts" onclick="openSignalLogChart(\'' +
                String(sig.posId || "").replace(/'/g, "\\'") +
                "')\">Open Chart</button>" +
                (oc === "OPEN"
                    ? '<button class="btn btp bts" onclick="markNoFill(\'' +
                      sig.posId +
                      "')\">Mark No Fill</button>"
                    : "") +
                "</div>" +
...
function sf(label, val, cls) {
    return (
        '<div class="sf"><div class="sfl">' +
        label +
        '</div><div class="sfv ' +
        (cls || "") +
        '">' +
...
function renderAnalytics() {
    ...
    var pipArr = wins
        .filter(function (s) {
            return s.closePips != null;
        })
        .map(function (s) {
            return s.closePips;
        });
    var avgP = pipArr.length
        ? (
              pipArr.reduce(function (a, b) {
                  return a + b;
              }, 0) / pipArr.length
          ).toFixed(0) + "p"
        : "-";
    var totalPL_USC = closed
        .filter(function (s) {
            return s.closePL != null;
        })
        .reduce(function (a, s) {
            return a + s.closePL;
        }, 0);
    ...
    document.getElementById("an-ts").textContent =
        openCount +
        " open, " +
        awaitingHistory.length +
        " awaiting history, " +
    ...
    document.getElementById("an-pl").innerHTML =
        '<div class="cval ' +
        (totalPL_USC >= 0 ? "pos" : "neg") +
        '">' +
        (totalPL_USC >= 0 ? "+" : "") +
        totalPL_USC.toFixed(2) +
        " USC</div>" +
        '<div class="csub" style="color:' +
        (totalPL_USC >= 0 ? "var(--gr)" : "var(--re)") +
        '">' +
        plZAR(totalPL_USC) +
        "</div>";
    ...
    if (statsDiv) {
        ...
        var _fillRate = _resolved
            ? _resolved > 0
                ? ((closed.length / _resolved) * 100).toFixed(1) + "%"
                : "—"
            : "—";
        statsDiv.innerHTML =
            '<div class="clbl">Win/Loss Summary</div>' +
            '<div style="display:flex; flex-wrap:wrap; gap:12px; margin-top:8px">' +
            '<div><span class="pos">Wins:</span> <strong>' +
            wins.length +
            "</strong></div>" +
            '<div><span class="neg">Losses:</span> <strong>' +
            losses.length +
            "</strong></div>" +
            '<div><span class="wrn">BE:</span> <strong>' +
            be.length +
            "</strong></div>" +
            '<div><span style="color:var(--bl)">Awaiting History:</span> <strong>' +
            awaitingHistory.length +
            "</strong></div>" +
            '<div><span style="color:var(--pu)">No Fill:</span> <strong>' +
            noFills.length +
            "</strong></div>" +
            "<div>Win Rate: <strong>" +
            (closed.length ? ((wins.length / closed.length) * 100).toFixed(1) + "%" : "—") +
            "</strong></div>" +
            '<div>Fill Rate: <strong style="color:' +
            (_resolved && closed.length / _resolved >= 0.7
                ? "var(--gr)"
                : _resolved && closed.length / _resolved >= 0.5
                  ? "var(--am)"
                  : "var(--re)") +
            '">' +
            _fillRate +
            "</strong></div>" +
            '<div style="width:100%;margin-top:6px;padding-top:6px;border-top:1px solid var(--bd);font-family:var(--mo);font-size:11px">' +
            'Net P/L: <span style="color:' +
            (totalPL_USC >= 0 ? "var(--gr)" : "var(--re)") +
            '">' +
            plUSC(totalPL_USC) +
            '</span> &nbsp;·&nbsp; <span style="color:var(--dm)">' +
            plZAR(totalPL_USC) +
            "</span></div>" +
            "</div>";
    }
    var nt = '<div style="color:var(--mu);font-size:12px;padding:8px 0">No closed tests yet</div>';
    function atbl(headers, rows) {
        if (!rows) return nt;
        return (
            '<table style="width:100%;font-size:12px"><thead>   th' +
            headers
                .map(function (h) {
                    return (
                        '<th style="padding:4px 8px;background:var(--bg3);color:var(--mu);font-family:var(--mo);font-size:9px;letter-spacing:1px">' +
                        h +
                        "</th>"
                    );
                })
    ...
    if (!closed.length) {
        ["an-star", "an-zone", "an-regime", "an-pair"].forEach(function (id) {
    ...
    [1, 2, 3, 4, 5].forEach(function (s) {
        var sc = closed.filter(function (x) {
            return x.zoneStars === s;
        });
        if (!sc.length) return;
        var sw = sc.filter(function (x) {
            return x.outcome.indexOf("WIN") === 0;
        });
        var wr = ((sw.length / sc.length) * 100).toFixed(0);
        var ap = sw
            .filter(function (x) {
                return x.closePips;
            })
        ...
        var avg = ap.length
            ? (
                  ap.reduce(function (a, b) {
                      return a + b;
                  }, 0) / ap.length
              ).toFixed(0) + "p"
            : "-";
        var c = wr >= 60 ? "var(--gr)" : wr >= 45 ? "var(--am)" : "var(--re)";
        sr +=
            '<tr><td style="padding:5px 8px;border-bottom:1px solid var(--bd)">' +
            s +
            '-star</td><td style="padding:5px 8px;border-bottom:1px solid var(--bd)">' +
            sc.length +
            '</td><td style="padding:5px 8px;border-bottom:1px solid var(--bd);color:' +
            c +
            ';font-weight:700">' +
            wr +
            '%</td><td style="padding:5px 8px;border-bottom:1px solid var(--bd)">' +
            avg +
            "</td></tr>";
    });
    document.getElementById("an-star").innerHTML = atbl(
        ["Stars", "Tests", "Hold Rate", "Avg Pips"],
        sr || null,
    );
    var zm = {};
    closed.forEach(function (x) {
        if (x.zonePct) {
            if (!zm[x.zonePct]) zm[x.zonePct] = [];
            zm[x.zonePct].push(x);
        }
    });
    var zr = "";
    Object.keys(zm).forEach(function (z) {
        var zc = zm[z];
        var zw = zc.filter(function (x) {
            return x.outcome.indexOf("WIN") === 0;
        });
        var wr = ((zw.length / zc.length) * 100).toFixed(0);
        var c = wr >= 60 ? "var(--gr)" : wr >= 45 ? "var(--am)" : "var(--re)";
        zr +=
            '<tr><td style="padding:5px 8px;border-bottom:1px solid var(--bd)">' +
            z +
    ...
    document.getElementById("an-zone").innerHTML = atbl(["Zone", "Tests", "Hold Rate"], zr || null);
    var rm = {};
    closed.forEach(function (x) {
        if (x.regimeGate) {
            if (!rm[x.regimeGate]) rm[x.regimeGate] = [];
            rm[x.regimeGate].push(x);
        }
    });
    var rr = "";
    Object.keys(rm).forEach(function (rg) {
        var rc = rm[rg];
        var rw = rc.filter(function (x) {
            return x.outcome.indexOf("WIN") === 0;
        });
        var wr = ((rw.length / rc.length) * 100).toFixed(0);
        var c = wr >= 60 ? "var(--gr)" : wr >= 45 ? "var(--am)" : "var(--re)";
        rr +=
            '<tr><td style="padding:5px 8px;border-bottom:1px solid var(--bd)">' +
            rg +
    ...
    document.getElementById("an-regime").innerHTML = atbl(
        ["Regime", "Tests", "Hold Rate"],
        rr || null,
    );
    var pr = "";
    PAIRS.forEach(function (p) {
        var pc = closed.filter(function (x) {
            return x.pair === p;
        });
        if (!pc.length) return;
        var pw = pc.filter(function (x) {
            return x.outcome.indexOf("WIN") === 0;
        });
        var wr = ((pw.length / pc.length) * 100).toFixed(0);
        var pl = pc
            .filter(function (x) {
                return x.closePL != null;
            })
            .reduce(function (a, x) {
                return a + x.closePL;
            }, 0)
        ...
        pr +=
            ...
            '">' +
            pl +
            ' USC</td><td style="padding:5px 8px;border-bottom:1px solid var(--bd);color:var(--dm);font-size:11px">' +
            plZAR(parseFloat(pl)) +
            "</td></tr>";
    });
    document.getElementById("an-pair").innerHTML = atbl(
        ["Pair", "Tests", "Hold Rate", "P/L USC", "P/L ZAR"],
        pr || null,
    );
}
function renderBook() {
    ...
    if (!curPos || !curPos.length) {
        tb.innerHTML =
            '<tr><td colspan="14" style="text-align:center;color:var(--mu);padding:20px;font-family:var(--mo);font-size:11px">No open positions - upload report to load book</td></tr>';
        return;
    }
    tb.innerHTML = curPos
        .map(function (p, i) {
            ...
            var mult = isJPY ? 100 : 10000,
                dp = isJPY ? 2 : 5;
            var slP = p.sl ? (Math.abs(p.entry - p.sl) * mult).toFixed(0) : "-";
            var toSL =
                p.sl && p.mkt
                    ? (p.dir === "SELL" ? (p.sl - p.mkt) * mult : (p.mkt - p.sl) * mult).toFixed(0)
                    : "-";
            var be = p.sl && Math.abs(p.sl - p.entry) < (isJPY ? 0.02 : 0.00002);
            var slSt = be ? "BE OK" : parseFloat(toSL) < 100 ? "WATCH" : "OK";
            var slC = be ? "var(--gr)" : parseFloat(toSL) < 100 ? "var(--am)" : "var(--mu)";
            return (
                '<tr><td class="mn" style="color:var(--mu)">' +
                (i + 1) +
                '</td><td class="mn" style="color:var(--mu);font-size:10px">' +
                p.id +
                '</td><td style="color:var(--mu);font-size:10px">' +
                (p.ot || "").slice(0, 10) +
                '</td><td class="mn"><strong>' +
                (p.pair || "—") +
                "</strong></td><td>" +
                ...
                p.entry.toFixed(dp) +
                '</td><td class="mn neg">' +
                (p.sl ? p.sl.toFixed(dp) : "-") +
                '</td><td class="mn">' +
                p.mkt.toFixed(dp) +
                '</td><td class="mn">' +
                slP +
                'p</td><td class="mn" style="color:' +
                (parseFloat(toSL) < 100 ? "var(--am)" : "var(--mu)") +
                '">' +
                toSL +
                'p</td><td class="mn" style="color:var(--mu)">' +
                p.swap.toFixed(2) +
                '</td><td class="mn" style="color:' +
                (p.profit >= 0 ? "var(--gr)" : "var(--re)") +
                '">' +
                (p.profit >= 0 ? "+" : "") +
                p.profit.toFixed(2) +
                '</td><td style="color:' +
                slC +
                ';font-weight:600;font-family:var(--mo);font-size:10px">' +
                slSt +
                "</td></tr>"
            );
        })
...
function renderOrders(orders) {
    ...
    if (!orders || !orders.length) {
        tb.innerHTML =
            '<tr><td colspan="11" style="text-align:center;color:var(--mu);padding:20px;font-family:var(--mo);font-size:11px">No pending orders in this report</td></tr>';
        return;
    }
    tb.innerHTML = orders
        .map(function (o) {
            var isJPY = o.pair && o.pair.indexOf("JPY") > -1;
            var mult = isJPY ? 100 : 10000,
                dp = isJPY ? 2 : 5;
            var buy = o.type && o.type.indexOf("buy") > -1;
            var dist = o.mkt ? ((buy ? o.mkt - o.entry : o.entry - o.mkt) * mult).toFixed(0) : "-";
            var slP = o.sl && o.entry ? Math.abs(o.entry - o.sl) * mult : null;
            var pv = getPipValueAccount(o.pair, true) || 0.1;
            var risk = slP ? (slP * (o.lots / 0.01) * pv).toFixed(2) : "-";
            var rr =
                slP && o.tp
                    ? "1:" + (Math.abs(o.tp - o.entry) / Math.abs(o.entry - o.sl)).toFixed(1)
                    : "-";
            var ch = getChopBand(o.pair);
            var chopF = ch
                ? o.entry >= ch.lo && o.entry <= ch.hi
                    ? '<span class="pr2 pill">CHOP</span>'
                    : '<span class="pg2 pill">Clear</span>'
                : '<span class="pgy pill">-</span>';
            var noSL = !o.sl ? '<span class="pr2 pill">NO SL</span>' : o.sl.toFixed(dp);
            var tstr = o.type ? o.type.toUpperCase() : "";
            return (
                '<tr><td class="mn" style="color:var(--mu)">' +
                o.id +
                '</td><td class="mn"><strong>' +
                o.pair +
                "</strong></td><td>" +
                (buy ? '<span class="pg2 pill">' : '<span class="pr2 pill">') +
                tstr +
                '</span></td><td class="mn">' +
                o.lots +
                '</td><td class="mn">' +
                (o.entry ? o.entry.toFixed(dp) : "-") +
                '</td><td class="mn neg">' +
                noSL +
                '</td><td class="mn pos">' +
                (o.tp ? o.tp.toFixed(dp) : "-") +
                '</td><td class="mn" style="color:var(--dm)">' +
                dist +
                'p</td><td class="mn wrn">' +
                risk +
                ' USC</td><td class="mn">' +
                rr +
                "</td><td>" +
...
function renderProgress() {
    ...
    document.getElementById("pprog").innerHTML = PAIRS.map(function (p) {
        var n = post.filter(function (s) {
            return s.pair === p;
        }).length;
        var pct = Math.min((n / TARGET) * 100, 100);
        var col = pct >= 100 ? "pfg" : pct >= 60 ? "pfa" : "pfb";
        return (
            '<div class="prow"><div class="pnm">' +
            p +
            '</div><div style="flex:1"><div class="pr_b"><div class="pf ' +
            col +
            '" style="width:' +
            pct +
            '%"></div></div></div><div style="font-family:var(--mo);font-size:11px;font-weight:700;width:36px;text-align:right;color:' +
            (pct >= 100 ? "var(--gr)" : "var(--dm)") +
            '">' +
            n +
            '</div><div style="font-family:var(--mo);font-size:9px;color:var(--mu);width:42px"> / ' +
            TARGET +
            (pct >= 100 ? " OK" : "") +
            "</div></div>"
        );
    ...
    var days = getElapsedDays();
    var pct = Math.min((days / TDAYS) * 100, 100);
    var col = pct >= 100 ? "pfg" : pct >= 50 ? "pfa" : "pfb";
    var csKv = document.getElementById("collection-start-kv");
    if (csKv) {
        var startDate = lsGet("sn_start") || "—";
        csKv.textContent = startDate;
    }
    document.getElementById("tprog").innerHTML =
        '<div style="display:flex;justify-content:space-between;font-family:var(--mo);font-size:10px;margin-bottom:4px"><span style="color:var(--mu)">Days elapsed</span><span style="color:' +
        (pct >= 100 ? "var(--gr)" : "var(--dm)") +
        '">Day ' +
        days +
        " / " +
        TDAYS +
        (pct >= 100 ? " DONE" : "") +
        '</span></div><div class="pr_b" style="height:8px"><div class="pf ' +
        col +
        '" style="width:' +
        pct.toFixed(1) +
        '%"></div></div><div style="font-family:var(--mo);font-size:10px;color:var(--mu);margin-top:4px">' +
        Math.max(TDAYS - days, 0) +
        " days remaining</div>";
}
function exportReportHtml() {
    ...
    if (!post.length) {
        xtoast("No signals to export", "warn");
        return;
    }
    var closed = post.filter(function (s) {
        return s.outcome !== "OPEN" && s.outcome !== "AWAIT_HISTORY";
    });
    var wins = closed.filter(function (s) {
        return s.outcome.indexOf("WIN") === 0;
    });
    ...
    var totalPL_USC = closed.reduce(function (s, sig) {
        return s + (sig.closePL || 0);
    }, 0);
    ...
    var sigRows = post
        .map(function (s, i) {
            var isJPY = s.pair && s.pair.indexOf("JPY") > -1,
                dp = isJPY ? 2 : 5;
                ...
                oc.indexOf("WIN") === 0
                    ...
                    : oc === "BE"
                      ...
                      : oc.indexOf("LOSS") === 0
                        ...
                        : "color:#3d8ef5";
            return (
                '<tr style="border-bottom:1px solid #1e2738"><td style="padding:6px 9px;font-family:monospace">' +
                (i + 1) +
                '</td><td style="padding:6px 9px;font-family:monospace">' +
                s.posId +
                '</td><td style="padding:6px 9px">' +
                (s.openTime || "").slice(0, 10) +
                '</td><td style="padding:6px 9px;font-weight:700">' +
                s.pair +
                '</td><td style="padding:6px 9px;color:' +
                (s.dir === "BUY" ? "#1fdb7a" : "#f5414f") +
                '">' +
                s.dir +
                '</td><td style="padding:6px 9px;font-family:monospace">' +
                (s.entry ? s.entry.toFixed(dp) : "-") +
                '</td><td style="padding:6px 9px;font-family:monospace;color:#f5414f">' +
                (s.sl ? s.sl.toFixed(dp) : "-") +
                '</td><td style="padding:6px 9px;color:#f5a623">' +
                (s.zonePct || "OFF ZONE") +
                '</td><td style="padding:6px 9px">' +
                (s.zoneStars ? s.zoneStars + "-star" : "-") +
                '</td><td style="padding:6px 9px">' +
                (s.slPips ? s.slPips + "p" : "-") +
                '</td><td style="padding:6px 9px;color:#f5a623">' +
                (s.riskUSC ? s.riskUSC + " USC" : "-") +
                '</td><td style="padding:6px 9px">' +
                (s.rr1 ? "1:" + s.rr1 : "-") +
                '</td><td style="padding:6px 9px">' +
                (s.regimeGate || "-") +
                ...
                '">' +
                oc +
                '</td><td style="padding:6px 9px;font-family:monospace;color:' +
                (s.closePips > 0 ? "#1fdb7a" : s.closePips < 0 ? "#f5414f" : "#8899b0") +
                '">' +
                (s.closePips != null ? (s.closePips > 0 ? "+" : "") + s.closePips + "p" : "-") +
                '</td><td style="padding:6px 9px;font-family:monospace;color:' +
                (s.closePL != null ? (s.closePL >= 0 ? "#1fdb7a" : "#f5414f") : "#8899b0") +
                '">' +
                (s.closePL != null ? plBothText(s.closePL) : "-") +
                "</td></tr>"
            );
        })
        .join("");
    var html =
        '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Sniper 4a Report Day ' +
        days +
        '</title></head><body style="background:#080b10;color:#dde4f0;font-family:DM Sans,sans-serif;padding:24px;font-size:13px"><div style="font-family:monospace;font-size:10px;letter-spacing:3px;color:#e8a020;margin-bottom:6px">SNIPER v12.0.9.1 PHASE 4a REPORT</div><h1 style="font-family:monospace;font-size:20px;margin-bottom:4px">Level Test Report Day ' +
        days +
        '</h1><div style="color:#8899b0;font-size:12px;margin-bottom:20px">Account 1220086 - Kudzanai Lloyd Taruvinga - ' +
        new Date().toISOString().slice(0, 16).replace("T", " ") +
        ' SAST</div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px"><div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">EQUITY</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#1fdb7a">' +
        eq +
        '</div><div style="font-size:11px;color:#8899b0">USC</div></div><div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">SIGNALS</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#3d8ef5">' +
        post.length +
        '</div><div style="font-size:11px;color:#8899b0">' +
        closed.length +
        ' closed</div></div><div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">HOLD RATE</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#1fdb7a">' +
        (closed.length ? ((wins.length / closed.length) * 100).toFixed(1) + "%" : "-") +
        '</div><div style="font-size:11px;color:#8899b0">' +
        wins.length +
        "W / " +
        closed.length +
        ' closed</div></div><div style="background:#111620;border:1px solid #1e2738;border-radius:6px;padding:14px"><div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#4a5568;margin-bottom:6px">DAY</div><div style="font-family:monospace;font-size:22px;font-weight:700;color:#f5a623">' +
        days +
        '</div><div style="font-size:11px;color:#8899b0">of 90</div></div></div><div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:11.5px"><thead><tr style="background:#161d2a">' +
        [
            "Num",
            "Pos ID",
            "Date",
            ...
            .map(function (h) {
                return (
                    '<th style="padding:7px 9px;font-family:monospace;font-size:9px;letter-spacing:1px;text-transform:uppercase;color:#4a5568;text-align:left;border-bottom:1px solid #28334a">' +
                    h +
                    "</th>"
                );
            })
        ...
        sigRows +
        "</tbody>" +
        '<tfoot><tr style="border-top:2px solid #28334a; background:#111620"><td colspan="14" style="text-align:right; font-weight:700">Totals:</td><td style="color:' +
        (totalPL_USC >= 0 ? "#1fdb7a" : "#f5414f") +
        '">' +
        (totalPL_USC >= 0 ? "+" : "") +
        totalPL_USC.toFixed(2) +
        ' USC<br><span style="font-size:10px;color:#4a5568">' +
        plZAR(totalPL_USC) +
        "</span></td><td></td></tr>" +
        '<tr><td colspan="14" style="text-align:right">Wins: ' +
        wins.length +
        " | Losses: " +
        losses.length +
        " | BE: " +
    ...
    a.href = "data:text/html;charset=utf-8," + encodeURIComponent(html);
    a.download =
        "sniper_4a_report_day" + days + "_" + new Date().toISOString().slice(0, 10) + ".html";
    a.click();
    xtoast("Report exported", "ok");
}
...
function exportCSV() {
    ...
    var rows = post.map(function (s, i) {
        return [
            i + 1,
            s.posId,
            s.openTime,
    ...
    a.download = "sniper_4a_day" + days + "_" + new Date().toISOString().slice(0, 10) + ".csv";
    a.click();
    xtoast("CSV exported", "ok");
}
...
function updateResetBtn() {
    ...
    if (rem > 0) {
        btn.disabled = true;
        btn.title =
            "Reset locked - Phase 4a test active (" +
            rem +
            " day" +
            (rem === 1 ? "" : "s") +
            " remaining)";
...
function resetAll() {
    ...
    if (rem > 0) {
        xtoast(
            "Reset locked - Phase 4a test ends in " +
                rem +
                " day" +
                (rem === 1 ? "" : "s") +
                ". Data must be preserved for the full 90-day window.",
            "warn",
        );
        return;
    }
    ...
    document.getElementById("upstat").textContent =
        "All data reset. Upload a report to establish new baseline.";
    document.getElementById("hist-stat").textContent =
        "No history file loaded yet - exact close analytics stay pending until history is imported";
        ...
        if (shouldScroll) {
            ...
            try {
                if (scrollContainer) {
                    ...
                    var nextTop =
                        scrollContainer.scrollTop + (focusRect.top - containerRect.top) - 8;
    ...
    wrap.innerHTML = entries
        ...
        .map(function (entry, idx) {
            ...
            return (
                '<button class="btn ' +
                (active ? "bta" : "btg") +
                ' bts" type="button" data-chart-symbol="' +
                String(entry.symbol).replace(/"/g, "&quot;") +
                '" onclick="showChart(\'' +
                String(entry.symbol).replace(/'/g, "\\'") +
                "',this)\">" +
                entry.pair +
...
function getTradeQueueItemForPair(pair, direction) {
    ...
    rows.sort(function (a, b) {
        return (
            new Date(b.updated_at || b.generated_at || 0) -
            new Date(a.updated_at || a.generated_at || 0)
        );
    });
    return rows[0] || null;
}
...
function collectChartEntryLevels(pair, seed, live, queue, computed, history) {
    ...
    if (seed && Array.isArray(seed.entry_levels)) {
        seed.entry_levels.forEach(function (level, idx) {
            if (level && typeof level === "object")
                pushLevel(level.label || "E" + (idx + 1), level.price, level.status);
    ...
    if (queue && queue.risk_breakdown && Array.isArray(queue.risk_breakdown.stages)) {
        ...
        queue.risk_breakdown.stages.forEach(function (stage, idx) {
            pushLevel(
                stageLabels[idx] || "E" + (idx + 1),
                stage.entry,
                stage.lot === 0 ? "Too small" : "",
            );
        });
    }
    if (history && history.entry != null) pushLevel("Entry", history.entry, history.outcome || "");
    if (!levels.length && computed && computed.entry_zone_price != null)
        pushLevel("Zone", computed.entry_zone_price, computed.sequence_status || "");
    return levels;
}
function buildChartContext(seed) {
    seed = seed && typeof seed === "object" ? seed : { pair: seed };
    var pair =
        toPairDisplay(seed.pair || seed.instrument_id || seed.symbol) ||
    ...
    var sourceKind =
        seed.sourceKind ||
        (queue ? "queue" : live ? "live" : computed ? "computed" : history ? "signals" : "chart");
    return {
        pair: pair,
        symbol: symbol,
        ...
        regime: firstNonEmptyText([
            seed.regime,
            live && live.regime,
            queue && queue.regime,
            computed && computed.regime,
            history && history.regimeGate,
        ]),
        sequence_status: firstNonEmptyText([
            seed.sequence_status,
            live && live.sequence_status,
            computed && computed.sequence_status,
        ]),
        signal_state: firstNonEmptyText([
            seed.signal_state,
            live && live.signal_state,
            queue && queue.signal_state,
            computed && computed.signal_state,
        ]),
        setup_class: firstNonEmptyText([
            seed.setup_class,
            live && live.setup_class,
            queue && queue.setup_class,
            computed && computed.setup_class,
            history && history.smcGrade,
        ]),
        market_price: firstFiniteNumber([
            seed.market_price,
            live && live.market_price,
            queue && queue.market_price,
        ]),
        zone_price: firstFiniteNumber([
            seed.zone_price,
            seed.entry_zone_price,
            live && live.zone_price,
            queue && queue.zone_price,
            history && history.zonePrice,
            computed && computed.entry_zone_price,
        ]),
        sl: firstFiniteNumber([seed.sl, live && live.sl, queue && queue.sl, history && history.sl]),
        tp1: firstFiniteNumber([
            seed.tp1,
            queue && queue.tp1,
            live && live.tp1,
            computed && computed.tp1,
            history && history.tp,
        ]),
        tp2: firstFiniteNumber([
            seed.tp2,
            queue && queue.tp2,
            live && live.tp2,
            computed && computed.tp2,
        ]),
        tp: firstFiniteNumber([
            seed.tp,
            queue && queue.tp,
            live && live.tp,
            computed && computed.tp,
            history && history.tp,
        ]),
        updated_at: firstNonEmptyText([
            seed.updated_at,
            live && live.updated_at,
            queue && queue.updated_at,
            queue && queue.generated_at,
            history && history.closeDate,
            history && history.openTime,
            computed && (computed.freshness_ts || computed.generated_at),
        ]),
        blocked_reason: firstNonEmptyText([
            seed.blocked_reason,
            live && live.blocked_reason,
            queue && queue.blocked_reason,
        ]),
        source_detail: firstNonEmptyText([
            seed.provenance,
            seed.source,
            live && (live.provenance || live.source),
            queue && (queue.provenance || queue.source),
...
function renderChartContextPanel() {
    ...
    var context = buildChartContext(
        CHART_CONTEXT || {
            pair: _tvCurrentPair || chartPairFromSymbol(_tvCurrentSymbol),
    ...
    var regimeClass =
        context.regime === "TREND UP"
            ? "rup"
            : context.regime === "TREND DOWN"
              ? "rdn"
              : context.regime === "REVERSAL ZONE"
                ? "rrv"
                : "rrg";
    ...
    function pushMetric(label, value, className) {
        ...
        grid.push(
            '<div class="sfx-chart-context__kv"><span class="sfx-chart-context__label">' +
                escapeHtmlAttr(label) +
                "</span>" +
                '<span class="sfx-chart-context__value' +
                (className ? " " + className : "") +
                '">' +
                value +
                "</span></div>",
    ...
    if (context.market_price != null)
        pushMetric(
            "Market",
            escapeHtmlAttr(context.market_price.toFixed(isPairJPY(context.pair) ? 2 : 5)),
            "",
        );
    if (context.zone_price != null)
        pushMetric(
            "Zone",
            escapeHtmlAttr(context.zone_price.toFixed(isPairJPY(context.pair) ? 2 : 5)),
            "wrn",
        );
    if (context.sl != null)
        pushMetric(
            "SL",
            escapeHtmlAttr(context.sl.toFixed(isPairJPY(context.pair) ? 2 : 5)),
            "neg",
        );
    if (context.tp1 != null)
        pushMetric(
            "TP1",
            escapeHtmlAttr(context.tp1.toFixed(isPairJPY(context.pair) ? 2 : 5)),
            "pos",
        );
    if (context.tp2 != null)
        pushMetric(
            "TP2",
            escapeHtmlAttr(context.tp2.toFixed(isPairJPY(context.pair) ? 2 : 5)),
            "pos",
        );
    if (context.tp != null)
        pushMetric(
            "Final TP",
            escapeHtmlAttr(context.tp.toFixed(isPairJPY(context.pair) ? 2 : 5)),
            "pos",
        );
    if (context.setup_class)
        pushMetric("Setup", escapeHtmlAttr(cleanDisplayText(context.setup_class)), "");
    if (context.updated_at)
        pushMetric("Updated", escapeHtmlAttr(formatSastDateTime(context.updated_at)), "");
    var meta = [];
    meta.push(
        '<span class="pgy pill">' +
            escapeHtmlAttr(chartSourceLabel(context.sourceKind)) +
            "</span>",
    );
    if (context.direction)
        meta.push(
            '<span class="' + dirClass + ' pill">' + escapeHtmlAttr(context.direction) + "</span>",
        );
    ...
    if (context.regime)
        meta.push(
            '<span class="' + regimeClass + '">' + escapeHtmlAttr(context.regime) + "</span>",
        );
    var levelsHtml = context.entry_levels.length
        ? '<div class="sfx-chart-context__levels">' +
          context.entry_levels
              .map(function (level) {
                  var suffix = level.status
                      ? '<div class="sfx-chart-context__foot" style="margin-top:6px;padding-top:0;border-top:none">' +
                        escapeHtmlAttr(cleanDisplayText(level.status)) +
                        "</div>"
                      : "";
                  return (
                      '<div class="sfx-chart-context__level"><strong>' +
                      escapeHtmlAttr(level.label) +
                      '</strong><span class="sfx-chart-context__value">' +
                      escapeHtmlAttr(Number(level.price).toFixed(isPairJPY(context.pair) ? 2 : 5)) +
                      "</span>" +
                      suffix +
                      "</div>"
                  );
    ...
    var foot = context.blocked_reason
        ? '<div class="sfx-chart-context__foot">Blocked reason: ' +
          escapeHtmlAttr(cleanDisplayText(context.blocked_reason)) +
          "</div>"
    ...
    el.innerHTML =
        '<div class="sfx-card__hd"><div><div class="sfx-card__title">Chart Context</div><div class="sfx-card__meta">' +
        escapeHtmlAttr(context.symbol || chartSymbolForPair(context.pair)) +
        "</div></div></div>" +
        '<div class="sfx-chart-context__summary"><div class="sfx-chart-context__pair">' +
        escapeHtmlAttr(context.pair) +
        "</div></div>" +
        '<div class="sfx-chart-context__meta">' +
        meta.join("") +
        "</div>" +
        (grid.length
            ? '<div class="sfx-chart-context__grid">' + grid.join("") + "</div>"
            : '<div class="sfx-chart-context__empty">No live level values are available for this selection yet.</div>') +
        levelsHtml +
        foot;
}
...
function showChart(symbol, btn) {
    ...
    if (
        ...
        CHART_CONTEXT.pair !== _tvCurrentPair
    ) {
...
function xtoast(msg, type) {
    ...
    clearTimeout(xtoast._t);
    xtoast._t = setTimeout(function () {
...
function tick() {
    ...
    if (nyH >= 2 && nyH < 5) {
        kz = "LONDON KZ ACTIVE";
        kzLabel = "London Kill Zone";
    }
    if (nyH >= 8 && nyH < 11) {
        kz = "NEW YORK KZ ACTIVE";
        kzLabel = "New York Kill Zone";
    }
    if (nyH >= 13.5 && nyH < 16) {
        kz = "NY PM KZ ACTIVE";
        kzLabel = "NY PM Kill Zone";
    }
    ...
    if (kzChip) {
        if (kzLabel) {
            ...
            for (var ki = 0; ki < kzSchedule.length; ki++) {
                if (nyH < kzSchedule[ki].start) {
                    nextKzObj = kzSchedule[ki];
                    break;
                }
            }
...
function buildLegacyStagesFinal(pair, direction, entriesArr, stageSlsArr, tp1, tp2, acctProfile) {
    ...
    var acctBalance =
        acctProfile &&
        (acctProfile.balance != null ? acctProfile.balance : acctProfile.balance_usc);
...
function buildRiskBreakdown(
    ...
    var balance =
        acct_profile &&
        (acct_profile.balance != null ? acct_profile.balance : acct_profile.balance_usc);
    ...
    if (pipVal == null) {
        return {
            available: false,
            reason: "Risk calculation unavailable for unsupported pair " + pair,
        };
    }
    ...
    entriesArr.forEach(function (entryPrice, i) {
        ...
        var stageSl =
            finalStage && finalStage.sl != null
                ? Number(finalStage.sl)
                : Array.isArray(stageSlsArr) && stageSlsArr[i] != null
                  ? Number(stageSlsArr[i])
                  : Number(slPrice);
        var slPips =
            finalStage && finalStage.sl_pips != null
                ? Number(finalStage.sl_pips)
                : priceToPips(pair, entryPrice, stageSl);
        var stageRisk = totalRisk * stageWeights[i];
        var lot =
            Array.isArray(stageLotsOverride) && stageLotsOverride[i] != null
    ...
    for (var j = 1; j < stages.length; j++) {
        if (stages[j - 1].lot > stages[j].lot) {
            ...
            stages[j - 1].riskAmount =
                Math.round(stages[j - 1].lot * stages[j - 1].slPips * pipVal * 100 * 100) / 100;
        }
    }
    totalRiskIfAll = stages.reduce(function (sum, s) {
        return sum + (Number(s.riskAmount) || 0);
    }, 0);
    ...
    return {
        ...
        currency: currency,
        stages: stages,
        stage_lots: stages.map(function (s) {
            return Number(s.lot) || 0;
        }),
        monotonic_lot_pass: monotonicLotPass,
        ...
        ddWarningMsg: ddWarning
            ? "⚠ Total risk " +
              currency +
              " " +
              Math.round(totalRiskIfAll) +
              " exceeds max drawdown cap of " +
              currency +
              " " +
              Math.round(maxDd)
            : null,
...
function getPipValueUsd(pair) {
    ...
    if (q === "CHF") {
        rate = savedPrices["USD/CHF"] || savedPrices["USDCHF"] || 0.9;
        return (cs * ps) / rate;
    }
    if (q === "GBP") {
    ...
    if (q === "NZD") {
        rate = savedPrices["NZD/USD"] || savedPrices["NZDUSD"] || 0.6;
        return cs * ps * rate;
    }
...
async function runSignalEngine(runOptions, legacyForceRun, legacyManual) {
    ...
    if (DEBUG_TRACE)
        console.log("[ENGINE_TRACE:START]", {
            force: !!force,
            manual: !!manual,
    ...
    if (!MARKET_DATA_READY) {
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:EARLY_RETURN]", { reason: "market_data_not_ready" });
        signalEngineStatus = "OFFLINE";
    ...
    if (signalEngineStatus === "COMPUTING") {
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:EARLY_RETURN]", { reason: "already_computing" });
        return;
    }
    var now = Date.now();
    ...
    if (
        !force &&
        !canBypassCooldown &&
        lastSignalEngineRunAt &&
        now - lastSignalEngineRunAt < cooldownMs
    ) {
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:EARLY_RETURN]", { reason: "cooldown_throttle_active" });
        return;
    }
    clearPendingEngineRetry();
    var results = {};
    ...
    for (var offset = 0; offset < eligiblePairs.length; offset += MAX_ENGINE_CONCURRENCY) {
        ...
        var settled = await Promise.allSettled(
            batch.map(async function (pair) {
                ...
                if (!candles) {
                    if (DEBUG_TRACE)
                        console.log("[ENGINE_TRACE:CANDLES]", pair, {
                            hasCandles: false,
                            duration_ms: fetchMs,
                            symbol: symbol || null,
                        });
                ...
                if (DEBUG_TRACE)
                    console.log("[ENGINE_TRACE:PAIR]", pair, {
                        hasCandles: true,
                ...
                if (DEBUG_TRACE)
                    console.log("[ANCHOR_TRACE:ENGINE]", pair, {
                        ...
                        levelsCount:
                            snapshot && Array.isArray(snapshot.levels) ? snapshot.levels.length : 0,
                    });
                return true;
            }),
        );
        settled.forEach(function (item, index) {
            if (item.status === "rejected") {
                ...
                if (code === "TD_DEFERRED") {
                    ...
                    if (DEBUG_TRACE)
                        console.log("[ENGINE_TRACE:PAIR_DEFERRED]", pairKey, {
                            retry_after_seconds: item.reason.retry_after_seconds || null,
                        });
                    return;
                }
                console.warn(
                    "[ENGINE_TRACE:PAIR_ERROR]",
                    pairKey,
    ...
    if (!anySuccess && hasProcessablePrice) {
        var deferredOnly =
            Object.keys(deferredPairs).length > 0 &&
            Object.keys(deferredPairs).length === eligiblePairs.length;
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:EARLY_RETURN]", { reason: "all_candle_fetches_failed" });
        ...
        scheduleEngineRetry(
            retrySeconds * 1000,
            deferredOnly ? "candle_retry_td_deferred" : "candle_fetch_retry_15s",
            eligiblePairs,
        );
        return;
    }
    ...
    if (anySuccess) {
        ...
        updateSignalEngineUI();
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:PRE_POST]", {
                pairKeys: Object.keys(results || {}),
                nonNullSignals: Object.keys(results || {}).filter(function (k) {
                    return !!results[k];
                }),
                snapshotKeys: Object.keys(runSnapshots || {}),
            });
        ...
        try {
            ...
            if (engineResp === null || (engineResp && engineResp.ok === false))
                backendSyncOk = false;
            if (backendSyncOk) {
        ...
        if (backendSyncOk && liveOk) {
            try {
                ...
                console.warn(
                    "[SNIPER] generatePlan failed after signal engine run:",
                    ePlan && ePlan.message ? ePlan.message : ePlan,
                );
            }
        }
        try {
            ...
            console.warn(
                "[SNIPER] buildPriceInputs failed after signal engine run:",
                eInputs && eInputs.message ? eInputs.message : eInputs,
            );
        }
    } else {
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:EARLY_RETURN]", { reason: "no_candles_fetched" });
        if (DEBUG_TRACE)
            console.log("[ENGINE_TRACE:EARLY_RETURN]", { reason: "no_signal_results" });
        signalEngineStatus = "STALE";
        lastSignalEngineFinishAt = Date.now();
    }
    ...
    refreshWpSession()
        ...
        .then(function () {
            ...
            switchSection(initialSection, {
                updateHash: window.location.hash !== "#" + initialSection,
                focus: false,
                scroll: false,
            });
            if (MARKET_DATA_READY) {
                Promise.allSettled([fetchPrices(), fetchRegimes(), fetchLiveSignals()]).then(
                    function () {
                ...
                if (priceStatus)
                    priceStatus.textContent = "Log in to enable authenticated market data";
            }
            startAutoRefresh();
            if (!init._engineRefreshInterval) {
                init._engineRefreshInterval = setInterval(function () {
                    ...
                    if (
                        ...
                        runSignalEngine({
                            reason: isVisible ? "silent_refresh_90s" : "silent_refresh_hidden",
                        });
                    }
                }, 30000);
            }
            if (!init._regimeRefreshInterval)
                init._regimeRefreshInterval = setInterval(fetchRegimes, 5 * 60 * 1000);
            if (!init._liveRefreshInterval)
                init._liveRefreshInterval = setInterval(fetchLiveSignals, 60 * 1000);
            ...
            apiGet("user/risk-profile")
                ...
                .catch(function () {
                    /* no risk profile saved yet — ignore */
                });
        });
}
...
window.SniperDashboardCore = Object.assign(window.SniperDashboardCore || {}, {
    ...
    __test__: {
        ...
        setState: function (state) {
            ...
            if (next.savedPrices && typeof next.savedPrices === "object")
                savedPrices = Object.assign({}, next.savedPrices);
            if (next.savedRegimes && typeof next.savedRegimes === "object")
                savedRegimes = Object.assign({}, next.savedRegimes);
            if (next.liveSignalMap && typeof next.liveSignalMap === "object")
                liveSignalMap = Object.assign({}, next.liveSignalMap);
            if (next.efLevels && typeof next.efLevels === "object")
                EF_LEVELS = Object.assign({}, next.efLevels);
            if (next.sflAnchors && typeof next.sflAnchors === "object")
                SFL_ANCHORS = Object.assign({}, next.sflAnchors);
            if (next.computedSnapshots && typeof next.computedSnapshots === "object")
                computedSnapshots = Object.assign({}, next.computedSnapshots);
            ...
            if (typeof next.signalEngineStatus === "string" && next.signalEngineStatus)
                signalEngineStatus = next.signalEngineStatus;
            if (typeof next.lastSignalEngineAttemptAt === "number")
                lastSignalEngineAttemptAt = next.lastSignalEngineAttemptAt;
            if (next.dataHydration && typeof next.dataHydration === "object")
                DATA_HYDRATION = Object.assign({}, DATA_HYDRATION, next.dataHydration);
            if (typeof next.fibTimeframe === "string" && next.fibTimeframe)
                syncRuntimeTimeframe(next.fibTimeframe);
        },
        getState: function () {
            return {
