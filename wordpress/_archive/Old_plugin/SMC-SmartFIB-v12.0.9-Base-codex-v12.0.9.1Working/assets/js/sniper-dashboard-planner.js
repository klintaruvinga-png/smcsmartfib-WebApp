(function (window) {
    ...
    function canonicalState(signal) {
        ...
        var normalized = String(raw || "INVALID")
            .trim()
            .toUpperCase();
        if (["INVALID", "WATCHLIST", "READY", "ACTIVE", "EXPIRED"].indexOf(normalized) > -1)
            return normalized;
        return "INVALID";
    }
    ...
    function scoreSignal(signal) {
        var setupQ = Number(
            signal && (signal.setup_quality != null ? signal.setup_quality : signal.setupQ),
        );
        var execQ = Number(
            signal && (signal.execution_quality != null ? signal.execution_quality : signal.execQ),
        );
    ...
    function renderSignalValidationBadges(signal) {
        var anchors =
            signal && signal.anchors && typeof signal.anchors === "object" ? signal.anchors : {};
        ...
        badges.push(
            '<span class="' +
                (entrySource === "EF" ? "pg2" : fallbackReason ? "pr2" : "pgy") +
                ' pill" title="Entry routing metadata. Execution authority remains backend-owned.">' +
                (entrySource === "EF" ? "EF OK" : fallbackReason ? "EF FALLBACK" : "EF N/A") +
                "</span>",
        );
        [
            ["f1", "F1"],
            ...
            badges.push(
                '<span class="' +
                    (hasFibAnchor(anchors[row[0]]) ? "pg2" : "pgy") +
                    ' pill" title="Canonical audit metadata; not an execution gate.">' +
                    row[1] +
                    " " +
                    (hasFibAnchor(anchors[row[0]]) ? "OK" : "PENDING") +
                    "</span>",
            );
        });
        return badges.join(" ");
    ...
    function renderSignalMeta(signal) {
        var rows = [
            '<div class="kv"><span class="kvl">Entry Source</span><span class="kvv">' +
                String((signal && signal.entry_source) || "FIB") +
                "</span></div>",
            '<div class="kv"><span class="kvl">SL Rule</span><span class="kvv">' +
                String((signal && signal.sl_rule) || "LEGACY_NEXT_LEVEL") +
                "</span></div>",
        ];
        if (signal && signal.fallback_reason) {
            rows.push(
                '<div class="kv"><span class="kvl">Fallback</span><span class="kvv wrn">' +
                    String(signal.fallback_reason) +
                    "</span></div>",
            );
        }
        rows.push(
            '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">' +
                renderSignalValidationBadges(signal) +
                "</div>",
        );
        return rows.join("");
    }
    ...
    function renderFallbackPlan(signals, state) {
        ...
        var rankedSignals = signals.slice().sort(function (a, b) {
        ...
        verdictEl.innerHTML =
            ...
            (actionable.length ? "ok" : "warn") +
            '">' +
            '<div class="verdict-lbl" style="color:' +
            (actionable.length ? "var(--gr)" : "var(--am)") +
            '">' +
        ...
        laddersEl.innerHTML = actionable.length
            ? actionable
                  .map(function (signal) {
                      ...
                      var entryPrice =
                          signal.zone_price != null
                              ? signal.zone_price
                              : signal.entry_zone_price != null
                                ? signal.entry_zone_price
                                : "--";
                      var pill =
                          typeof state.statePill === "function"
                              ? state.statePill(canonicalState(signal))
                              : canonicalState(signal);
                      return (
                          '<div class="ladder-table"><div class="ladder-header">' +
                          '<div><span class="ladder-title" style="color:var(--gr)">' +
                          signal.pair +
                          " " +
                          signal.direction +
                          "</span> " +
                          pill +
                          "</div></div>" +
                          '<div style="padding:14px;font-size:12px;color:var(--tx)">Entry zone: <span class="mn">' +
                          entryPrice +
                          "</span> " +
                          '&nbsp;|&nbsp; SL: <span class="mn">' +
                          (signal.sl != null ? signal.sl : "--") +
                          "</span>" +
                          '&nbsp;|&nbsp; TP1: <span class="mn">' +
                          primaryTp +
                          "</span>" +
        ...
        riskEl.innerHTML =
            '<div class="card"><div class="clbl">Actionable</div><div class="cval pos">' +
            actionable.length +
            '</div><div class="csub">READY or ACTIVE</div></div>' +
            '<div class="card"><div class="clbl">Watchlist</div><div class="cval wrn">' +
            watchlist.length +
            '</div><div class="csub">Visible but gated</div></div>' +
            '<div class="card"><div class="clbl">Expired</div><div class="cval neg">' +
            expired.length +
            '</div><div class="csub">Suppressed from execution</div></div>';

        gatesEl.innerHTML = signals
            .map(function (signal) {
                var signalState = canonicalState(signal);
                if (signalState === "INVALID") return "";
                return (
                    '<div class="kv"><span class="kvl">' +
                    signal.pair +
                    '</span><span class="kvv">' +
                    signalState +
                    " / " +
                    (signal.regime || "NO REGIME") +
                    " / " +
                    (signal.gate || "NO GATE") +
                    "</span></div>"
                );
    ...
    function renderPlanFromState(state) {
        ...
        if (blueprints.length && state && typeof state.renderServerBlueprintPlan === "function") {
            ...
            var filterPlanContextForBlueprints = function (planContext) {
                if (!planContext || !blueprintPairs || !Object.keys(blueprintPairs).length)
                    return planContext;
            ...
            var ctx =
                state.planContext &&
                state.planContext.gateResults &&
                state.planContext.gateResults.length
                    ...
                    : {
                          ...
                          verdictBody:
                              blueprints.length +
                              " backend blueprints available from canonical state.",
                          checklist: [],
                          gateResults: [],
                          ladders: [],
    ...
    function generatePlan(state) {
        ...
        var runtime =
            plannerState.runtime && typeof plannerState.runtime === "object"
                ? plannerState.runtime
                : {};
        var synced =
            plannerState.synced && typeof plannerState.synced === "object"
                ? plannerState.synced
                : {};
        ...
        var canonicalSignals = Array.isArray(plannerState.canonicalSignals)
            ...
            : bridge && typeof bridge.getSignals === "function"
              ? bridge.getSignals()
              : [];
        ...
        try {
            if (
                renderPlanFromState({
                    ...
                    liveSignals: Array.isArray(runtime.liveSignals)
                        ? runtime.liveSignals.slice()
                        : [],
                    blueprints: Array.isArray(runtime.tradeQueue) ? runtime.tradeQueue.slice() : [],
                    acct: runtime.acct || null,
                    prices:
                        synced.prices && typeof synced.prices === "object"
                            ? Object.assign({}, synced.prices)
                            : {},
                    regimes:
                        synced.regimes && typeof synced.regimes === "object"
                            ? Object.assign({}, synced.regimes)
                            : {},
                    computedSignals:
                        runtime.computedSignals && typeof runtime.computedSignals === "object"
                            ? Object.assign({}, runtime.computedSignals)
                            : {},
