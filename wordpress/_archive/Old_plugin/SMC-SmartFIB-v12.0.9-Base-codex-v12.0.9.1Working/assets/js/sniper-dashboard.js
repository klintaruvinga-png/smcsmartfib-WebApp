...
(function (window, document) {
  ...
  function validateBridge() {
    ...
    ['getProfile', 'getSignals', 'getCandidateSymbols', 'getRegime', 'isChopCondition'].forEach(function (method) {
  ...
  function verifyCoreGlobals() {
    ...
    console.error('[SniperDashboard] Compatibility check failed. Missing core globals: ' + missing.join(', '));
    return false;
  }
  ...
  function boot() {
    ...
    if (!window.SniperDashboardCore || typeof window.SniperDashboardCore.init !== 'function') {
      console.error('[SniperDashboard] Compatibility check failed. SniperDashboardCore.init missing.');
      return;
    }
