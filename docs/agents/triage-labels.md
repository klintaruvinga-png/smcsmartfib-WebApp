...
- `ready-for-human` — requires human review or decision.
- `wontfix` — issue will not be addressed.
- `bug` — confirmed defect or incorrect behavior.
- `enhancement` — improvement request or new feature.
...
- `workflow` — pipeline, state, automation, or local workflow issues.
