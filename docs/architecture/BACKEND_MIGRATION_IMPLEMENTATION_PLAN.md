...
export async function validateEaApiKey(apiKey: string) {
  const [user] = await db.select().from(users).where(eq(users.eaApiKey, apiKey)).limit(1);
...
export function extractEaApiKey(request: Request): string | null {
  return request.headers.get("X-EA-API-Key") || request.headers.get("x-ea-api-key") || null;
}
```
...
export const createFibLevelRoute = factory.createHandlers(eaAuthMiddleware, async (c) => {
...
export const getFibLevelsRoute = factory.createHandlers(
  ...
  async (c) => {
    ...
    const fibs: Record<
      string,
      Record<string, { ratio: number; price: number; calculated_at: string }[]>
    > = {};
...
export const getUserRoute = factory.createHandlers(userAuthMiddleware, async (c) => {
...
export const shadowValidationRoute = factory.createHandlers(userAuthMiddleware, async (c) => {
    ...
    for (const [family, levels] of Object.entries(families)) {
      ...
      for (const lvl of levels) {
        const hit = wpLevels.find(
          (w: any) =>
            Number(w.ratio) === Number(lvl.ratio) && Number(w.price) === Number(lvl.price),
        );
      ...
      levels: [
        {
          ...
          ltf_sf: [
            { ratio: 0, price: 1.085 },
            { ratio: 25, price: 1.082 },
            { ratio: 50, price: 1.08 },
            { ratio: 62.5, price: 1.078 },
            { ratio: 75, price: 1.076 },
            { ratio: 100, price: 1.072 },
          ],
          htf_af: [],
        },
      ],
...
export async function seedPhase4TestData() {
  ...
  const testData = [
    { symbol: "EURUSD", timeframe: "H1", price: 1.085 },
    { symbol: "GBPUSD", timeframe: "H1", price: 1.265 },
    { symbol: "USDJPY", timeframe: "H4", price: 149.5 },
  ];
