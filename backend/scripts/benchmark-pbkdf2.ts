...
async function benchmarkIterations(iterations: number, password: string, salt: Uint8Array): Promise<number> {
...
async function main() {
  ...
  const iterationCounts = [100000, 200000, 300000, 400000, 500000, 600000, 700000, 800000, 900000, 1000000];
  ...
  for (const iterations of iterationCounts) {
    ...
    console.log(`${iterations.toLocaleString()} iterations: ${avgTime.toFixed(2)}ms avg, ${maxTime.toFixed(2)}ms max`);
  }
  ...
  if (acceptable.length > 0) {
    ...
    console.log(`Recommended: ${recommended.iterations.toLocaleString()} iterations (${recommended.avgTime.toFixed(2)}ms avg)`);
    console.log(`This provides strong security while keeping login/registration under 500ms`);
  } else {
    const fastest = results[0];
    console.log(`Recommended: ${fastest.iterations.toLocaleString()} iterations (${fastest.avgTime.toFixed(2)}ms avg)`);
