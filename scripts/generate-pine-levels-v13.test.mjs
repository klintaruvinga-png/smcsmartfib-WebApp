...
describe("generate-pine-levels-v13.cjs output contract", () => {
  ...
  test("defaults both MT5 input and Pine output to the same phase4 report directory", () => {
    ...
    expect(source).toMatch(
      /const MT5_FILE\s*=\s*path\.resolve\(getArg\('--mt5-file'\) \|\| path\.join\(REPORTS_DIR, 'mt5-levels\.json'\)\)/,
    );
    expect(source).not.toMatch(
      /const OUTPUT_LEVELS\s*=\s*path\.join\(REPO_ROOT, 'reports', 'phase4-parity', 'pine-levels\.json'\)/,
    );
  });
  ...
  test("writes standalone anchor debug records without changing level rows", () => {
    ...
    const debug = JSON.parse(
      fs.readFileSync(path.join(reportsDir, "pine-anchor-debug.json"), "utf8"),
    );
    ...
    expect(ltf.components[0]).toEqual(
      expect.objectContaining({
