...
describe("run-phase4-parity.ps1 automation contract", () => {
  ...
  test("requires WordPress auth before any dry-run or full-run mode can continue", () => {
    ...
    expect(source.indexOf('Write-Step "Resolving auth"')).toBeLessThan(
      source.indexOf("if (-not $DryRun)"),
    );
  ...
  test("exports MT5 candles before staleness checks unless explicitly skipped", () => {
    ...
    expect(source.indexOf("export-mt5-candles.ps1")).toBeLessThan(
      source.indexOf("Test-CandleFiles $candleRoot"),
    );
  });
  ...
  test("uses M15 as the only exported candle source while preserving output timeframe selection", () => {
    ...
    expect(source).toMatch(
      /\$combined \| Where-Object \{ \(\$gateSymbols -contains \$_.symbol\) -and \(\$timeframes -contains \$_.timeframe\) \}/,
    );
  });
  ...
  test("allows partial Pine output when HTF authority history is optional", () => {
    ...
    expect(source).toContain(
      "Pine levels: $pineRows rows -> $pineLevelsFile (allowed $pineMinimumRows-$pineExpectedRows)",
    );
  ...
  test("package scripts expose short Windows PowerShell parity aliases", () => {
    ...
    expect(pkg.scripts.parity).toBe(
      "powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-phase4-parity.ps1",
    );
    expect(pkg.scripts["parity:dry"]).toBe(
      "powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-phase4-parity.ps1 -DryRun",
    );
    expect(pkg.scripts["parity:expanded"]).toBe(
      "powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-phase4-parity.ps1 -ExpandedAudit",
    );
  });
});
...
describe("Patch 1 — candle lineage columns in anchor debug Markdown report", () => {
  test("New-Mt5AnchorDebugRecord preserves MT5 lineage fields from endpoint debug", () => {
    const source = fs.readFileSync(path.join(__dirname, "run-phase4-parity.ps1"), "utf8");
    expect(source).toContain(
  ...
  test("official Phase 4 gate documents -BackendMode pine_compatible", () => {
    ...
    expect(source).toMatch(
      /-BackendMode\s+pine_compatible.*-NoPrompt|-NoPrompt.*-BackendMode\s+pine_compatible/s,
    );
  });
});
