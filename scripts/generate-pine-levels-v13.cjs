...
const REPORTS_DIR = path.resolve(
  getArg("--reports-dir") || path.join(REPO_ROOT, "reports", "phase4-parity"),
);
...
const SYMBOLS = _symbolsArg
  ? _symbolsArg
      .split(",")
      .map((s) => s.trim().toUpperCase())
      .filter(Boolean)
  : ["EURUSD", "USDJPY", "XAUUSD"];
const TIMEFRAMES = _timeframesArg
  ? _timeframesArg
      .split(",")
...
function buildAnchorDebugRecord(
  symbol,
  timeframe,
  family,
  sessionTf,
...
function checkStaleness(mt5Mtime) {
  ...
  for (const sym of SYMBOLS) {
    ...
    if (!norm.length) {
      console.error(
        `FAIL: no valid candles in ${path.join(CANDLE_DIR, `${sym}_${SOURCE_TIMEFRAME}.json`)}`,
      );
      process.exit(1);
    }
    ...
    console.log(
      `  [staleness OK] ${sym} ${SOURCE_TIMEFRAME}: open=${new Date(lastOpenMs).toISOString()} close=${new Date(lastCloseMs).toISOString()}`,
    );
  }
}
...
function main() {
  ...
  for (const sym of SYMBOLS) {
    ...
    for (const tf of TIMEFRAMES) {
      ...
      if (!htf) {
        ...
        console.warn(
          `WARN: skipping HTF_AF for ${sym} ${tf} - insufficient ${authTf} authority-session history`,
        );
      }
      ...
      for (const [family, anchor] of Object.entries(anchors)) {
        ...
        anchorDebugRows.push(
          buildAnchorDebugRecord(
            sym,
            tf,
            family,
            sessionTf,
  ...
  if (levels.length < EXPECTED_ROWS) {
    console.warn(
      `WARN: partial output - ${levels.length}/${EXPECTED_ROWS} rows (some HTF_AF skipped due to insufficient history)`,
    );
  }
