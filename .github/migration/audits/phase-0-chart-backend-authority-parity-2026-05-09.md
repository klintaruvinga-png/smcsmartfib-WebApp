...
- Previous drift: the Charts route could query the default backend before user backend settings were ready and could not build in production because the local workspace was missing a resolvable `lightweight-charts` install state.
...
- Bundle size and chart interaction performance should continue to be monitored on the restored `lightweight-charts` path, even though the build now passes.
...
- Supporting lockfile sync: `package-lock.json`
