...
3. Validate feed‑key rotation from a stale user feed to a fresher global feed.
   _Tests will use the WordPress test suite fixtures to populate mock rows._
...
| Guard adds network load                             | Measure request count; if >10 % increase, consider a secondary guard that only disables placeholder when _any_ price is stale. |
