# SMC SuperFIB — Copilot Research Report

### 1. Issue classification
- Severity: HIGH
- Category: signal-integrity / data-contract
- Layer(s) affected: Dashboard-JS / REST-API / Engine
- Phase impact: Phase 0 / Cross-phase

### 2. Confirmed evidence
- UI filter and toggle: [src/routes/signals.tsx](src/routes/signals.tsx) — `watchlistOnly` state, toggle button, and backend-driven `uniqueSignals` computed via `watchlistSet.has(signal.symbol)`.
- Canonical watchlist source: [src/hooks/useSniperData.ts](src/hooks/useSniperData.ts) — `useCanonicalWatchlist`, `normalizeWatchlist`, `useWatchlist` and `alignWatchlistItems`.
- Backend snapshot / signals APIs: [src/lib/api/sniperClient.ts](src/lib/api/sniperClient.ts) — `getSnapshot()`, `getLiveSignals()`, and watchlist endpoints `postWatchlistAdd` / `postWatchlistRemove`.
- Polling gating: [src/hooks/useSniperData.ts](src/hooks/useSniperData.ts) — `usePollingQueryState` enables queries only when `backendReady` and `pollMs` are valid; comments note critical gating behavior.
- Account settings handling: [src/routes/account.tsx](src/routes/account.tsx) — settings save flow, watchlist add/remove mutations (`useWatchlistAdd`, `useWatchlistRemove`) and optimistic updates.

No backend logs or snapshot files found in `reports/` that directly prove a backend-side symbol-filtering change; evidence comes from frontend code paths above.

### 3. Root cause hypothesis
- Most likely root cause: Backend/engine is only producing live signal candidates for symbols that have market data active over the weekend (cryptocurrencies). The frontend filters `live-signals` by `watchlistSet.has(signal.symbol)`, so if the engine never emits candidates for non-crypto symbols after market open, the watchlist view appears to only show crypto. (Hypothesis)
- Secondary hypothesis: Symbol format mismatch between backend signal IDs and canonical watchlist entries (e.g., casing, suffixes, or normalized form) causes `watchlistSet.has(signal.symbol)` to fail for many symbols, leaving only crypto symbols that happen to match exactly. This would also make the toggle appear broken. (Hypothesis)
- Toggling UI bug hypothesis: The toggle control in `signals.tsx` is implemented correctly, but if `watchlistSet` is empty or mismatched, toggling will not change results — appearing as a broken toggle. (Hypothesis)
- Less likely: Polling gating prevents `live-signals` from being fetched after settings reload (confirmed gating code), causing stale/missing data until a manual refresh is forced. (Unconfirmed)

Each sub-point above is labelled Hypothesis unless explicitly derived from code evidence.

### 4. Blast radius
- Files likely affected: `src/lib/api/sniperClient.ts`, `src/hooks/useSniperData.ts`, `src/routes/signals.tsx`, `src/routes/account.tsx`, `src/mocks/sniperData.ts` (mock behavior comparison).
- Systems impacted: Dashboard frontend, backend REST `/sniper/v1/snapshot` and `/sniper/v1/live-signals`, engine run process that produces live candidates, user-settings watchlist persistence endpoints.
- Parity surfaces at risk: Backend engine <-> Dashboard signal contract (symbol naming/normalization), snapshot freshness/engine-run sequencing.
- Stale-state risks: optimistic watchlist updates vs backend state, query gating that disables refetch until settings resolve.

### 5. Regression surface
- Patching symbol normalization could break integrations if backend expects unnormalized symbols; must preserve or coordinate normalization across layers.
- Forcing UI to display missing watchlist entries (without backend data) may mask engine failures; avoid hiding the real absence of engine candidates.
- Changing polling gating (enable earlier) could cause spurious queries to an unconfigured backend or duplicate requests; tests guard poll timing.
- Existing optimistic update flow in `useWatchlistAdd`/`Remove` relies on mutation responses; modifying invalidation behavior risks flicker or stale reads.

### 6. Resolution path options
- Path A (narrow): Capture and compare the live JSON payloads returned by `/sniper/v1/live-signals` and `/sniper/v1/snapshot` immediately after market open for a user with the affected watchlist. Verify symbol formats and which symbols the engine emits. If mismatch discovered, normalize `signal.symbol` (or watchlist entries) in the frontend filter before applying `watchlistSet.has()`. Recommended when backend emits correct candidate set but symbol formatting differs.
- Path B (broader): If backend/engine is not emitting candidates for non-crypto symbols (engine-run/sharding/config issue), open a backend/engine investigation to ensure the engine runs against the full watchlist after market open (or ensure a fallback engine run). This may require changes to engine scheduling, snapshot composition, or backend watchlist handling.
- Recommended: Start with Path A (frontend verification and defensive normalization) because it is lower-risk and allows quick validation. If frontend fixes only mask a missing-engine condition, follow up with Path B to restore engine completeness.

### 7. Risk flags
- High-risk system involved: Yes — signal engine and backend snapshot generation. Changes here can affect live trading signals and parity surfaces.
- Requires parity re-validation: Yes — validate Pine <-> Backend <-> Dashboard parity for signals and symbol formats after any fix.
- Migration-blocking: No (not a migration gate blocker), but it is Phase 0 stabilization relevant.
- Human review required before merge: Yes — any change that alters symbol normalization, engine scheduling, or backend query gating must be reviewed by backend/engine owner.

### 8. Handoff package
- Epicentre files to inspect first:
  - src/lib/api/sniperClient.ts (getSnapshot, getLiveSignals)
  - src/hooks/useSniperData.ts (usePollingQueryState, useCanonicalWatchlist, filter helpers)
  - src/routes/signals.tsx (filtering/toggle logic)
  - src/routes/account.tsx (watchlist mutation flow)
- Inputs Codex must verify before planning:
  1. Example JSON returned by `/sniper/v1/live-signals` (post-market-open) for an affected user.
  2. Example JSON returned by `/sniper/v1/snapshot` (post-market-open) for the same user.
  3. The current user-settings GET `/user/settings` payload (watchlist contents and normalization).
  4. Engine health `/health` / lastBatchAt and feedStatus near the incident time.
- Open unknowns that could invalidate the hypothesis:
  - Whether backend emitted candidates for non-crypto symbols and they were dropped by a frontend filter.
  - Whether `signal.symbol` values use a different normalization (suffixes, market codes) than watchlist entries.
  - Whether the engine scheduling/runner failed to run for non-crypto markets after weekend boundaries.
