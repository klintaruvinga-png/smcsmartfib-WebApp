## Intake: SMC Intake - Chart ticker and live polling is broken and charts arent live updating price.

Summary
- Issue validated: single-line intake received.

Findings
- Vite dev logs show repeated transform errors for `src/routes/charts.tsx` (.codex-vite-dev.err.log). This can prevent the charts route from loading in dev and block live updates.
- `src/routes/charts.tsx` uses `react-query` with `refetchInterval` controlled by `usePollMs()` and `useBackendReady()` (see `src/hooks/useSniperData.ts`). If `usePollMs()` returns `null` or `useBackendReady()` is false, polling is intentionally disabled.
- `usePollMs()` returns `null` until `user-settings` are loaded; this is deliberate to avoid orphaned queries but means polling won't start until settings/backend URL are loaded.
- The chart snapshot fetch is implemented in `src/lib/api/sniperClient.ts` as `apiClient.getChartSnapshot()` and calls `/sniper/v1/charts`. The API client supports `MOCK_MODE` which can return static mock data if enabled via `VITE_SNIPER_MOCK_MODE`.
- The rendering leaf `src/components/sniper/TVChart.tsx` updates the series when the `series` prop changes; a working data fetch should update the chart UI.

Likely root causes
- Build-time transform error on `src/routes/charts.tsx` (see logs) preventing the route from loading.
- Dashboard initialization/state: `user-settings` may be missing or backend URL not configured, preventing `useBackendReady()` from enabling polling.
- Environment may be in `MOCK_MODE`, returning static chart data so ticks appear frozen.

Evidence of interest (files)
- `src/routes/charts.tsx`
- `src/hooks/useSniperData.ts`
- `src/lib/api/sniperClient.ts`
- `src/components/sniper/TVChart.tsx`
- `.codex-vite-dev.err.log` and `.codex-vite-dev.log` (dev transform errors)

Next steps to triage (recommended)
1. Check browser devtools console and Vite logs for the exact transform error for `src/routes/charts.tsx` and fix any syntax/TSX issues or mis-configured plugin that may cause parsing failures.
2. Verify `Account` → `backendUrl` is configured in the app (so `useBackendReady()` is true) and `user-settings` are loading successfully.
3. Confirm `VITE_SNIPER_MOCK_MODE` is not forcing mock data in environments intended to show live updates.
4. Manually curl the dashboard backend endpoint `/sniper/v1/charts?symbol=...` to confirm the API returns fresh candle data and `updatedAt` timestamps.
5. If the build/transform error is transient, rebuild the dev server and confirm charts route loads; otherwise fix TSX syntax or Vite transform configuration.
